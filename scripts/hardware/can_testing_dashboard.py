...
class UnifiedCANDashboard(Node):
    ...
    def update_dashboard(self):
        ...
        hw_status = "🟢 CONNECTED" if self.latest_data['can_connected'] else "🔴 MOCK/DISCONNECTED"
        bb_status = "🟢 AVAILABLE" if self.latest_data['blackboard_available'] else "🟡 NOT DETECTED"
        ...
        for topic, count in self.message_counts.items():
            ...
            print(f"│  {topic.ljust(15)}: {str(count).ljust(8)} messages  ({rate:.1f} Hz){'':>32}│")
        print("└" + "─"*78 + "┘")
        print()
        ...
        if total_messages == 0:
            ...
            if self.latest_data['blackboard_available']:
                ...
                print("   To test CAN→blackboard: ./scripts/hardware/test_can_blackboard_full.sh")
            print()
