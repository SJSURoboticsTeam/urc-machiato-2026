    ...
    def simulate_step(self):
        ...
        for name, module in self.modules.items():
            if abs(module.angle - module.target_angle) > 0.01:
                print(f"🔄 {name}: adjusting angle {module.angle:.3f}→{module.target_angle:.3f}")
            if abs(module.speed - module.target_speed) > 0.05:
                print(f"⚡ {name}: adjusting speed {module.speed:.2f}→{module.target_speed:.2f}")
    ...
    async def run_simulation(self):
        ...
        while self.running:
            ...
            if step_count % 50 == 0:
                ...
                print(f"📊 Robot: vx={vx:.2f}m/s, vy={vy:.2f}m/s, ω={omega:.3f}rad/s, speed={speed:.2f}m/s")
                print("   Modules: ", end="")
                for name, module in self.modules.items():
