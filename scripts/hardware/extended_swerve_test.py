...
class ExtendedSwerveTest:
        ...
        for maneuver_name, commands in test_maneuvers:
            ...
            for step in range(100):  # 2 seconds max
                ...
                for motor in self.simulator.modules:
                    angle_error = abs(
                        self.simulator.modules[motor].target_angle
                        - self.simulator.modules[motor].angle
                    )
                    speed_error = abs(
                        self.simulator.modules[motor].target_speed
                        - self.simulator.modules[motor].speed
                    )
            ...
            print(
                f"      ✅ {maneuver_name}: {convergence_time:.2f}s to converge"
                if converged
                else f"❌ {maneuver_name}: failed to converge"
            )

        # Analyze results
        converged_results = [r for r in maneuver_results if r["converged"]]
        avg_convergence = (
            statistics.mean([r["convergence_time"] for r in converged_results])
            if converged_results
            else 0
        )
        success_rate = (
            len(converged_results) / len(maneuver_results) if maneuver_results else 0
        )
    ...
    def run_performance_test(self):
        ...
        for second in range(5):  # 5 second test
            ...
            print(
                f"✅ Performance Test: {command_rate:.1f} commands/sec ({command_rate/400:.1%} of target)"
            )
        ...
        for motor, target in target_commands.items():
            if motor in self.simulator.modules:
                actual = (
                    self.simulator.modules[motor].angle
                    if "steer" in motor
                    else self.simulator.modules[motor].speed
                )
    ...
    async def run_all_tests(self):
        ...
        print(
            f"   Complex maneuvers: {self.test_results['complex_maneuvers']['success_rate']:.1%} success"
        )
        print(
            f"   Performance ratio: {self.test_results['performance']['performance_ratio']:.1%}"
        )
