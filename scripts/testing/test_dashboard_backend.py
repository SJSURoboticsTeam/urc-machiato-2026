...
try:
    ...
    from simulation.can.can_bus_mock_simulator import CANBusMockSimulator, CANMessageType, CANMessagePriority
    ...
    logging.warning(f"Simulation components not available - using basic simulation: {e}")
...
class ROS2BridgeNode(Node if HAS_ROS2 else object):
    ...
    def cmd_vel_callback(self, msg):
        ...
        logger.info(f"Command velocity: linear={msg.linear.x:.2f}, angular={msg.angular.z:.2f}")
    ...
    def publish_sensor_data(self):
        ...
        if int(sim_time) % 5 == 0 and int(sim_time * 10) % 10 == 0:
            print(f"ROS2 PUBLISHING: IMU x={imu_data['linear_acceleration']['x']:.3f}, GPS lat={gps_data['latitude']:.6f}")
...
class TestExecutionEngine:
    ...
    async def _execute_test(self, test_id: str):
        ...
        try:
            ...
            result.message = "Test completed successfully" if success else "Test failed - check logs"
            result.metrics = {
...
class TestDashboardBackend:
    ...
    def __init__(self, host: str = 'localhost', port: int = 8766):
        ...
        if HAS_SIMULATION:
            ...
            logger.warning("Using basic simulation - install simulation package for full features")
    ...
    def _init_simulation(self):
        ...
        try:
            ...
            self.imu_sensor = sensor_factory.create('imu', {
                'name': 'imu_sensor',
                'type': 'imu',
            ...
            self.gps_sensor = sensor_factory.create('gps', {
                'name': 'gps_sensor',
                'type': 'gps',
    ...
    async def update_metrics_loop(self):
        ...
        while True:
            ...
            logger.debug(f"Updated simulation_data with IMU accel_x: {simulation_data.get('imu', {}).get('linear_acceleration', {}).get('x', 'N/A')}")
    ...
    def _generate_simulation_data(self, simulation_time):
        ...
        if HAS_SIMULATION and self.imu_sensor and self.gps_sensor:
            ...
            data['rover_state'] = self.rover.get_state() if hasattr(self.rover, 'get_state') else {'speed': 0.0}
        else:
