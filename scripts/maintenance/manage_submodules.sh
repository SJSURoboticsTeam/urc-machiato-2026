...
update_submodule() {
    ...
    git submodule update --remote "vendor/$submodule"
...
check_status() {
    ...
    echo "Teleoperation: vendor/teleoperation"
    echo "Control Systems: vendor/control-systems"

    echo
    log_info "Submodule README: vendor/README.md"
}
...
show_integration() {
    ...
    echo
    echo "Quick start:"
    echo "  1. Update submodules: ./scripts/maintenance/manage_submodules.sh update"
    echo "  2. Start autonomy: source install/setup.bash && ros2 launch autonomy_core unified.launch.py mode:=simulation"
    echo "  3. Start teleop: ./scripts/maintenance/manage_submodules.sh teleop"
    echo "  4. Legacy full stack (requires matching ROS packages): ros2 launch <repo>/scripts/launch/integrated_system.launch.py"
    echo
    echo "Testing without hardware:"
    echo "  - Prefer: ros2 launch autonomy_core simulation.launch.py"
    echo "  - Legacy sim launch file: <repo>/scripts/launch/rover_simulation.launch.py"
