...
class ServiceHealthMonitor(Node):
    ...
    def _create_health_clients(self):
        ...
        for service_name, config in self._service_configs.items():
            try:
                ...
                self.get_logger().warning(f"Failed to create health client for {service_name}: {e}")
    ...
    def _check_process_health(self, service_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.get_logger().error(f"Process health check error for {service_name}: {e}")
            return {'healthy': False, 'process_running': False}
    ...
    def _restart_service(self, service_name: str):
        ...
        try:
            ...
            self.get_logger().info(f"Service {service_name} restart initiated (attempt {config['restart_attempts']})")
    ...
    def get_overall_health_assessment(self) -> Dict[str, Any]:
        ...
        return {
            'overall_status': self.overall_health_status,
            'healthy_services': sum(1 for s in self._service_configs.values() if s['health_status'] == 'healthy'),
            'total_services': len(self._service_configs),
            'critical_failures': sum(1 for s in self._service_configs.values() if s['critical'] and s['health_status'] != 'healthy'),
            'total_restarts': sum(s['restart_attempts'] for s in self._service_configs.values()),
