...
def generate_charuco_board(
    ...
    board = cv2.aruco.CharucoBoard(
        (cols, rows), checker_size_mm, marker_size_mm, aruco_dict
    )
    ...
    print(
        f"Board physical size: {board_width_inches:.2f} x {board_height_inches:.2f} inches"
    )
    ...
    if (
        board_width_inches > canvas_width_inches - 1.0
    ...
    if start_y < 0:
        board_area_height_px = page_height_px - int(
            0.5 * DPI
        )  # Reduce text space to 0.5 inches
        start_y = (board_area_height_px - board_height_px) // 2
        if start_y < 0:
    ...
    fig.text(
        0.5, 0.03, footer_text, ha="center", va="bottom", fontsize=7, family="monospace"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate ChArUco board patterns for camera calibration."
    )
    parser.add_argument(
        "--rows", type=int, default=7, help="Number of rows of squares (default: 7)"
    )
    parser.add_argument(
        "--cols", type=int, default=5, help="Number of columns of squares (default: 5)"
    )
    parser.add_argument(
        "--checker-size",
        type=float,
        default=30.0,
        help="Size of a single checker square in mm (default: 30.0)",
    )
    parser.add_argument(
        "--marker-size",
        type=float,
        default=18.0,
        help="Size of the ArUco marker in mm (default: 18.0)",
    )
    parser.add_argument(
        "--dict",
        default="DICT_4X4_50",
    ...
    parser.add_argument(
        ...
        help="Output PDF filename (default: charuco_board.pdf)",
    )
