        ...
        if threaded:
            ...
            for camera_index in sorted(self.calibrations.keys()):
                thread = threading.Thread(
                    target=self.process_camera, args=(camera_index, duration, display)
                )
...
Examples:
  ...
  python detect_with_multiple_calibrations.py --calibration-dir ./camera_calibrations --duration 30
    ...
    parser.add_argument(
        "--tag-size", type=int, default=18, help="Tag size in mm (default: 18)"
    )
    ...
    if args.camera is not None:
        ...
        success = detector.process_camera(
            args.camera, duration=args.duration, display=not args.no_display
        )
    else:
        # Process all cameras
