...
Examples:
  ...
  python ../aruco_tags/aruco_validator.py --calibration my_camera.json --tag-size 10.0
    ...
    parser.add_argument(
        "--board-cols", type=int, default=8, help="ChArUco board columns (default: 8)"
    )
    parser.add_argument(
        "--board-rows", type=int, default=6, help="ChArUco board rows (default: 6)"
    )
    ...
    if success:
        ...
        print("   cd ../aruco_tags/")
        print(
            f"   python aruco_validator.py --calibration ../camera/{args.output} --tag-size 10.0"
        )
        return 0
    else:
        print("\n❌ Calibration failed")
