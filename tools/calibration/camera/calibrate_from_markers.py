...
def calibrate_from_markers(
            ...
            if ids is not None and len(ids) > 4:
                try:
                    ...
                    if charuco_corners is not None and charuco_ids is not None:
                        n_corners = (
                            len(charuco_ids)
                            if isinstance(charuco_ids, np.ndarray)
                            else 0
                        )
                        if n_corners > 5:
                    ...
                    if frame_count % 30 == 0:
                        print(
                            f"⏳ Frame {frame_count}: {len(ids) if ids is not None else 0} markers detected..."
                        )
            else:
                if frame_count % 30 == 0:
    ...
    try:
        ...
        for frame_idx, (corners_frame, ids_frame) in enumerate(
            zip(all_corners, all_ids)
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--square-size", type=float, default=0.030, help="Square size in meters"
    )
    parser.add_argument(
        "--marker-size", type=float, default=0.018, help="Marker size in meters"
    )
    parser.add_argument(
        "--duration", type=int, default=30, help="Capture duration in seconds"
    )
    parser.add_argument(
        "--output", "-o", default="calibration.json", help="Output file"
    )
