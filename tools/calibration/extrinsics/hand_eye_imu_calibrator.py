...
class HandEyeCalibrator:
    ...
    def add_pose_observation(
        ...
        # Store pose
        pose = HandEyePose(
            pose_id=pose_id, robot_base_to_hand=robot_base_to_hand, timestamp=timestamp
        )
        ...
        logger.info(
            f"Added pose observation {pose_id} (total: {len(self.poses_collected)})"
        )

    def calibrate(
        self, setup_type: str = "eye_on_hand", method: str = cv2.CALIB_HAND_EYE_TSAI
    ) -> HandEyeResult:
        """
        ...
        logger.info(
            f"Computing hand-eye calibration ({setup_type}) with {len(self.poses_collected)} poses"
        )
        ...
        for i, detection in enumerate(self.image_detections):
            ...
            T_camera_to_board = self._create_transform(
        ...
        try:
            ...
            error = self._calculate_reprojection_error(
                robot_poses, camera_poses, T_hand_eye
            )
    ...
    def save_calibration(
        self, result: HandEyeResult, output_dir: str = "artifacts/extrinsics"
    ) -> Dict[str, str]:
        """Save hand-eye calibration"""
        Path(output_dir).mkdir(parents=True, exist_ok=True)

        base_name = (
            f"hand_eye_{result.setup_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        files_saved = {}
...
class MultiCameraCalibrator:
    ...
    def __init__(
        self, camera1_intrinsics: Dict, camera2_intrinsics: Dict, board_config: Dict
    ):
    ...
    def add_observation(
        ...
        self.pose_pairs.append(
            {"corners1": corners1, "ids1": ids1, "corners2": corners2, "ids2": ids2}
        )
...
class IMUCalibrator:
    ...
    def add_accel_sample(
        self, x: float, y: float, z: float, gravity_magnitude: float = 9.81
    ):
        """
    ...
    def calibrate_accelerometer(self):
        ...
        if len(self.accel_samples) < 6:
            logger.warning(
                f"Insufficient accel samples: {len(self.accel_samples)} (need 6+)"
            )
            return
    ...
    def calibrate_gyroscope(self):
        ...
        if len(self.gyro_samples) < 100:
            logger.warning(
                f"Insufficient gyro samples: {len(self.gyro_samples)} (need 100+)"
            )
            return
