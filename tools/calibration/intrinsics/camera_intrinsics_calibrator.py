...
class CameraIntrinsicsCalibrator:
    ...
    def _setup_detector(self) -> cv2.aruco.ArucoDetector:
        ...
        return cv2.aruco.ArucoDetector(
            self.board_config.aruco_dictionary, cv2.aruco.DetectorParameters()
        )
    ...
    def capture_manual(
        self, target_images: int = 50, output_dir: str = None
    ) -> List[str]:
        ...
        while len(captured_images) < target_images:
            ...
            if ids is not None:
                display_frame = cv2.aruco.drawDetectedMarkers(
                    display_frame, corners, ids
                )
    ...
    def capture_video(
        self, target_images: int = 50, sampling_rate: int = 5, output_dir: str = None
    ) -> List[str]:
        ...
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        logger.info(
            f"Video capture started. Target: {target_images}, sampling: every {sampling_rate}th frame"
        )
        ...
        cap.release()
        logger.info(
            f"Video capture complete: {len(captured_images)}/{target_images} extracted"
        )
        return captured_images
    ...
    def capture_conservative(
        ...
        while True:
            ...
            if ids is None or len(ids) < min_markers:
                ...
                if blur_ratio > max_blur_ratio:
                    ...
                    # Detection rate
                    expected_markers = (
                        self.board_config.size[0] * self.board_config.size[1]
                    )
        ...
        indexed_scores = [
            (i, score) for i, (score, _) in enumerate(frame_quality_scores)
        ]
        indexed_scores.sort(key=lambda x: x[1], reverse=True)

        # Select top N, spread across time
        selected_indices = sorted(
            [idx for idx, _ in indexed_scores[: target_images * 2]]
        )
        ...
        cap.release()
        logger.info(
            f"Conservative capture complete: {len(captured_images)}/{target_images}"
        )
        return captured_images
    ...
    def detect_charuco_corners(
        ...
        for i in range(len(charuco_corners)):
            cv2.cornerSubPix(
                gray, charuco_corners[i : i + 1], (5, 5), (-1, -1), criteria
            )
    ...
    def save_calibration(
        self, result: CalibrationResult, output_dir: str = None
    ) -> Dict[str, str]:
    ...
    def print_results(self, result: CalibrationResult):
        ...
        print(
            f"  FOV: {result.quality_metrics['fov_x_deg']:.1f}° × {result.quality_metrics['fov_y_deg']:.1f}°"
        )
...
if __name__ == "__main__":
    ...
    camera_cfg = CameraConfig(
        name="Test Camera", camera_index=0, resolution=(1920, 1080)
    )
