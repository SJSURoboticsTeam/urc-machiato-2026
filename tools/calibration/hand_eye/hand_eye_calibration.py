...
def generate_sample_data(setup_type="eye_on_hand", num_poses=20):
    ...
    R_hand_to_camera_true = Rotation.from_euler(
        "xyz", [np.pi, 0, np.pi / 2]
    ).as_matrix()
    t_hand_to_camera_true = np.array(
        [0.1, 0.05, 0.05]
    )  # 10cm forward, 5cm left, 5cm up
    T_hand_to_camera_true = create_pose_matrix(
        R_hand_to_camera_true, t_hand_to_camera_true
    )
    ...
    T_base_to_camera_true = create_pose_matrix(
        R_base_to_camera_true, t_base_to_camera_true
    )
    ...
    T_base_to_target_true = create_pose_matrix(
        R_base_to_target_true, t_base_to_target_true
    )
    ...
    for i in range(num_poses):
        ...
        t_base_to_hand = np.array(
            [0.4 + rand_trans[0], 0 + rand_trans[1], 0.3 + rand_trans[2]]
        )
        ...
        if setup_type == "eye_on_hand":
            ...
            T_camera_to_target = (
                np.linalg.inv(T_hand_to_camera_true)
                @ np.linalg.inv(T_base_to_hand)
                @ T_base_to_target_true
            )
            ...
            raise ValueError(
                "Invalid setup type. Must be 'eye_on_hand' or 'eye_to_hand'"
            )
...
def perform_hand_eye_calibration(
    robot_poses, target_poses, setup_type="eye_on_hand", method=cv2.CALIB_HAND_EYE_TSAI
):
    """
...
def collect_hand_eye_data(
    ...
    detector = ChArUcoDetector(
        board_squares[0], board_squares[1], square_size, marker_size
    )
...
def save_calibration_result(
    result_matrix, setup_type, output_file="hand_eye_calibration_result.npz"
):
    """
...
def main():
    ...
    # Data source
    parser.add_argument(
        "--data", type=str, help="Path to .npz file containing calibration data"
    )
    parser.add_argument(
        "--generate-sample",
        action="store_true",
