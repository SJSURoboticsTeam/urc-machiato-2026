...
class PerformanceMonitor:
    ...
    def record_performance_run(
        self, benchmark_data: Dict[str, Any], run_id: Optional[str] = None
    ) -> str:
    ...
    def analyze_performance_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        for benchmark_name in self._get_unique_benchmarks(recent_runs):
            analysis[benchmark_name] = self._analyze_benchmark_trend(
                benchmark_name, recent_runs
            )
    ...
    def detect_regressions(
        self, current_run_id: str, baseline_days: int = 7
    ) -> Dict[str, Any]:
        ...
        if not baseline_runs:
            return {
                "error": f"No baseline data found for the last {baseline_days} days"
            }
        ...
        for benchmark in current_data.get("benchmarks", []):
            ...
            if baseline_times:
                ...
                std_deviations = (
                    abs(regression_pct) / (baseline_std / baseline_mean)
                ...
                regressions[benchmark_name] = {
                    ...
                    "is_regression": regression_pct
                    > self.standards.get(benchmark_name.split("_")[0], {}).get(
                        "regression_threshold", 0.05
                    ),
    ...
    def generate_performance_report(self, output_file: str, days: int = 30):
        ...
        report = {
            ...
            "charts_generated": ["performance_trends.png", "regression_analysis.png"],
        }
    ...
    def _analyze_benchmark_trend(
        self, benchmark_name: str, runs: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        if len(timestamps) > 1:
            ...
            y = np.array(
                [
                    np.mean([t for t in times[i : i + len(times) // len(timestamps)]])
                    for i in range(0, len(times), len(times) // len(timestamps))
                ][: len(timestamps)]
            )
            if len(x) == len(y):
        ...
        compliant = mean_time <= standards.get("max_mean_time", float("inf")) and (
            std_time / mean_time if mean_time > 0 else 0
        ) <= standards.get("max_variability", 1.0)
    ...
    def _calculate_overall_assessment(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        compliant_benchmarks = sum(
            1 for b in analysis.values() if b.get("compliant", False)
        )

        # Trend assessment
        improving_trends = sum(
            1 for b in analysis.values() if b.get("trend_pct", 0) < -0.01
        )  # >1% improvement
        ...
        compliance_rate = (
            compliant_benchmarks / total_benchmarks if total_benchmarks > 0 else 0
        )
        ...
        return {
            ...
            "overall_health": self._assess_overall_health(
                compliance_rate, degrading_trends
            ),
        }

    def _assess_overall_health(
        self, compliance_rate: float, degrading_trends: int
    ) -> str:
        ...
        for benchmark_name, data in analysis.items():
            if not data.get("compliant", True):
                recommendations.append(
                    f"Address performance issues in {benchmark_name}"
                )

            trend = data.get("trend_pct", 0)
            if trend > 0.05:  # 5% degradation
                recommendations.append(
                    f"Investigate performance degradation in {benchmark_name}"
                )
    ...
    def _summarize_regressions(self, regressions: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "total_regressions": len(
                [r for r in regressions.values() if r.get("is_regression")]
            ),
...
def main():
    ...
    parser.add_argument(
        "--analyze", type=int, default=30, help="Analyze trends for N days"
    )
    parser.add_argument("--detect-regressions", help="Detect regressions for run ID")
    parser.add_argument(
        "--baseline-days", type=int, default=7, help="Baseline period in days"
    )
