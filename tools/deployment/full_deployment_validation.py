...
class DeploymentValidator:
        ...
        try:
            ...
            if result.returncode == 0:
                self.results["unit_tests"] = {
                    "status": "PASSED",
                    "details": self._parse_test_results(
    ...
    def _run_performance_tests(self):
        ...
        try:
            ...
            standards_violations = self._validate_performance_standards(
    ...
    def _validate_performance_standards(self, results_file: str) -> List[str]:
        ...
        try:
            ...
            standards = {
                "test_context_evaluation_performance": {
                    "max_time": 0.1,
                    "max_variability": 0.5,
                },
                "test_policy_engine_performance": {
                    "max_time": 0.05,
                    "max_variability": 0.5,
                },
            }
            ...
            for benchmark in data.get("benchmarks", []):
                ...
                if test_name in standards:
                    ...
                    if mean_time > std["max_time"]:
                        violations.append(
                            f"{test_name}: mean time {mean_time:.3f}s exceeds {std['max_time']}s"
                        )
                    if variability > std["max_variability"]:
                        violations.append(
                            f"{test_name}: variability {variability:.2f} exceeds {std['max_variability']}"
                        )
    ...
    def _validate_deployment_scripts(self) -> Dict[str, Any]:
        ...
        return {"passed": len(missing_scripts) == 0, "missing_scripts": missing_scripts}
    ...
    def _calculate_readiness_score(self) -> float:
        ...
        if (
            perf_result.get("status") == "PASSED"
            and perf_result.get("standards_check") == "PASSED"
        ):
            perf_score = 100
        else:
            perf_score = 0
    ...
    def _generate_readiness_recommendations(self, score: float) -> List[str]:
        ...
        if score < 50:
            recommendations.append(
                "CRITICAL: Major test failures detected. Address all failing tests before deployment."
            )
        elif score < 75:
            recommendations.append(
                "HIGH PRIORITY: Several test categories failing. Focus on competition-critical tests."
            )
        elif score < 85:
            recommendations.append(
                "MODERATE: Most tests passing but some issues remain. Verify performance standards."
            )
        else:
            recommendations.append(
    ...
    def _generate_final_report(self) -> Dict[str, Any]:
        ...
        report = {
            "validation_summary": {
                ...
                "overall_readiness_score": self.results.get(
                    "deployment_readiness", {}
                ).get("readiness_score", 0),
                "competition_ready": self.results.get("deployment_readiness", {}).get(
                    "competition_ready", False
                ),
            },
            "test_results": self.results,
            "recommendations": self.results.get("deployment_readiness", {}).get(
                "recommendations", []
            ),
        }
...
def main():
    ...
    print(".1f")
    print(
        f"Competition Ready: {'YES' if report['validation_summary']['competition_ready'] else 'NO'}"
    )
    print(".2f")
