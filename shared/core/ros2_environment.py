...
class ROS2EnvironmentManager:
    ...
    def _detect_environment(self) -> ROS2Environment:
        ...
        logger.info(
            f"ROS2 Environment: available={ros2_available}, mock_mode={mock_mode}, "
    ...
    def _initialize_mock_implementations(self):
        ...
        self._mock_implementations["ReliabilityPolicy"] = (
            self._create_mock_reliability_policy()
        )
        self._mock_implementations["DurabilityPolicy"] = (
            self._create_mock_durability_policy()
        )
        ...
        self._mock_implementations["LifecycleState"] = (
            self._create_mock_lifecycle_state()
        )
        self._mock_implementations["TransitionCallbackReturn"] = (
            self._create_mock_transition_callback()
        )

        # Message mocks
        self._mock_implementations["PoseStamped"] = self._create_mock_message(
            "PoseStamped"
        )
        ...
        # Action mocks
        self._mock_implementations["NavigateToPose"] = self._create_mock_action(
            "NavigateToPose"
        )
    ...
    def _mock_navsatfix_message(self):
        ...
        class MockNavSatFix:
            def __init__(self, **kwargs):
                ...
                self.position_covariance_type = kwargs.get(
    ...
    def safe_import(self, module_name: str, fallback: Any = None) -> Any:
        ...
        try:
            ...
            logger.warning(
                f"No implementation available for {module_name}, creating dummy"
            )
        ...
        class DummyModule:
            def __getattr__(self, name):
                ...
                def dummy_function(*args, **kwargs):
                    logger.debug(
                        f"Dummy {module_name}.{name} called with {args} {kwargs}"
                    )
                    return None
    ...
    def get_optimized_imports(self) -> Dict[str, Any]:
        ...
        if "QoSProfile" not in imports:
            ...
            imports["ReliabilityPolicy"] = self._mock_implementations[
                "ReliabilityPolicy"
            ]
        ...
        # Messages
        imports["PoseStamped"] = self.safe_import(
            "geometry_msgs.msg", self._mock_implementations["PoseStamped"]
        )
        imports["Twist"] = self.safe_import(
            "geometry_msgs.msg", self._mock_implementations["Twist"]
        )
        imports["Imu"] = self.safe_import(
            "sensor_msgs.msg", self._mock_implementations["Imu"]
        )
        imports["NavSatFix"] = self.safe_import(
            "sensor_msgs.msg", self._mock_implementations["NavSatFix"]
        )
        imports["String"] = self.safe_import(
            "std_msgs.msg", self._mock_implementations["String"]
        )

        # Services
        imports["Trigger"] = self.safe_import(
            "std_srvs.srv", self._mock_implementations["Trigger"]
        )

        # Actions
        imports["NavigateToPose"] = self.safe_import(
            "autonomy_interfaces.action", self._mock_implementations["NavigateToPose"]
        )
