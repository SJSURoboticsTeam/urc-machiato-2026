    ...
    def __init__(self, max_size: int = 10000):
        ...
        if POLARS_AVAILABLE and pl:
            ...
            self.df = pl.DataFrame(
                {"timestamp": [], "measurement": [], "fields": [], "tags": []}
            )
            ...
            if self.using_polars:
                if POLARS_AVAILABLE and pl:
                    ...
                    if PANDAS_AVAILABLE and pd:
                        ...
                        self.df = pl.concat(
                            [self.df, pl.from_pandas(row_df)], how="diagonal"
                        )
                    else:
    ...
    def get_data(self, measurement=None, start_time=None, end_time=None):
        ...
        with self.lock:
            if not self.using_polars:
                ...
                if measurement:
                    filtered_data = [
                        d for d in filtered_data if d.get("measurement") == measurement
                    ]
                ...
                if start_time:
                    filtered_data = [
                        ...
                        if d.get("timestamp", datetime.min) >= start_time
                    ]
                ...
                if end_time:
                    filtered_data = [
                        d
                        for d in filtered_data
                        if d.get("timestamp", datetime.min) <= end_time
                    ]
...
def record_metric(
    measurement: str, value: Union[int, float], tags: Optional[Dict[str, str]] = None
):
...
def log_event(event_type: str, description: str, tags: Optional[Dict[str, str]] = None):
    """Log an event."""
    telemetry = get_telemetry_system()
    telemetry.add_point(f"event_{event_type}", {"description": description}, tags)
