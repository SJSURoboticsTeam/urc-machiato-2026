#!/usr/bin/env python3
"""
StateManager and HierarchicalStateMachine - Dashboard/CLI State Management.

This module provides state management for DASHBOARD, CLI, and LEGACY code paths.
This is NOT the runtime ROS2 state machine - see adaptive_state_machine.py.

Components:
- HierarchicalStateMachine: FSM using transitions library
- StateManager: Unified manager for dashboard state tracking
- StateSynchronizationManager: Component state coordination
- BehaviorTree: Python wrapper for BT blackboard access (requires ROS2 node)

Use Cases:
- Dashboard UI state representation
- CLI commands and state queries
- Development/testing state simulation
- Legacy code paths

For production runtime state, use AdaptiveStateMachine (ROS2 LifecycleNode).
...
import threading
import json
from typing import Dict, List, Any, Optional, Callable, Union, Type, Set, TYPE_CHECKING
...
class BehaviorTree:
    ...
    def __init__(
        ...
        try:
            from shared.core.unified_blackboard_client import UnifiedBlackboardClient

            self.blackboard = UnifiedBlackboardClient(node)
            logger.info(f"BehaviorTree '{name}' using unified blackboard")
        except ImportError as e:
            raise ImportError(
                "UnifiedBlackboardClient not available. "
    ...
    def set_blackboard_value(self, key: str, value: Any) -> bool:
        """Set value in unified blackboard."""
        return self.blackboard.set(key, value)
