...
class PerformancePhysicsEngine:
    ...
    def __init__(
        self, performance_level: SimulationPerformance = SimulationPerformance.BALANCED
    ):
    ...
    def _update_rover_kinematics(self, dt: float):
        ...
        q_change = np.array([0, 0, np.sin(angle_change / 2), np.cos(angle_change / 2)])
        ...
        position_change = (
            linear_vel * dt * np.array([np.cos(heading), np.sin(heading), 0])
        )
        ...
        self.world_state["rover_velocity"] = np.array(
            [linear_vel * np.cos(heading), linear_vel * np.sin(heading), 0]
        )
    ...
    def _update_battery_model(self, dt: float):
        ...
        self.world_state["battery_voltage"] = max(
            10.0, self.world_state["battery_voltage"]
        )
...
class FailureInjectionFramework:
    ...
    def apply_failure_effects(
        self, sensors: AnalogousSensor, physics_engine: PerformancePhysicsEngine
    ):
    ...
    def _apply_motor_stall(
        self, physics_engine: PerformancePhysicsEngine, failure, config, age
    ):
    ...
    def _apply_getting_stuck(
        self, physics_engine: PerformancePhysicsEngine, failure, config, age
    ):
        """Apply getting stuck effects."""
        velocity_magnitude = np.linalg.norm(
            physics_engine.world_state["rover_velocity"]
        )
...
class PerformanceSimulationNode(Node):
    ...
    def __init__(
        ...
        self.simulation_timer = self.create_timer(
            1.0 / 1000.0, self._simulation_step  # 1kHz physics updates
        )

        # Status publishing
        self.status_timer = self.create_timer(
            0.1, self._publish_status  # 10Hz status publishing
        )
    ...
    def _simulation_step(self):
        ...
        self.sensors.update_true_value(
            "wheel_velocity", world_state["wheel_velocities"]
        )
    ...
    def _cmd_vel_callback(self, msg: Twist):
        ...
        self.sensors.update_true_value(
            "wheel_velocity", [left_wheel_vel, right_wheel_vel]
        )
...
if __name__ == "__main__":
    ...
    for level in [
        SimulationPerformance.FAST,
        SimulationPerformance.BALANCED,
        SimulationPerformance.REALISTIC,
    ]:
