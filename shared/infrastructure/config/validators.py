...
class URCConfigValidator:
    ...
    def _validate_safety_constraints(self):
        ...
        if env_value == "competition":
            ...
            if not safety.emergency_stop_enabled:
                self.errors.append("Emergency stop must be enabled in competition mode")
    ...
    def _validate_hardware_consistency(self):
        ...
        if not hardware.use_mock:
            ...
            if not hardware.camera_rear_url:
                self.warnings.append("camera_rear_url not configured for real hardware")
    ...
    def _validate_mission_parameters(self):
        ...
        if mission.max_mission_duration < 300:  # 5 minutes
            self.warnings.append(
                f"Mission duration ({mission.max_mission_duration}s) " "is very short"
            )

        if mission.max_mission_duration > 7200:  # 2 hours
            self.warnings.append(
                f"Mission duration ({mission.max_mission_duration}s) " "is very long"
            )
    ...
    def _validate_waypoints(self):
        ...
        if duplicates:
            self.errors.append(f"Duplicate waypoint names: {duplicates}")
        ...
        for i, wp in enumerate(waypoints):
            ...
            if wp.tolerance < 0.1:
                self.warnings.append(
                    f"Waypoint '{wp.name}' tolerance ({wp.tolerance}m) " "is very tight"
                )
