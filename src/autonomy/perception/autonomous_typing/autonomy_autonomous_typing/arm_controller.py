...
class SimpleInverseKinematics:
    ...
    def forward_kinematics(self, joint_angles: np.ndarray) -> np.ndarray:
        ...
        reach = self.link_lengths[1] * np.cos(q1 + q2) + self.link_lengths[2] * np.cos(
            q1 + q2 + q3
        )

        x = reach * np.cos(q1)
        y = reach * np.sin(q1)
        z = self.link_lengths[1] * np.sin(q1 + q2) + self.link_lengths[2] * np.sin(
            q1 + q2 + q3
        )
...
class ArmController:
    ...
    def press_key(
        ...
        try:
            ...
            approach_pos = np.array(
                [key_world_pos[0], key_world_pos[1], key_world_pos[2] + approach_height]
            )
            ...
            press_pos = np.array(
                [key_world_pos[0], key_world_pos[1], key_world_pos[2] - press_depth]
            )
    ...
    def _plan_trajectory(
        self, target_angles: np.ndarray, duration: float, num_waypoints: int = 10
    ) -> JointTrajectory:
        """
    ...
    def is_ready(self) -> bool:
        ...
        return (
            self.current_joint_positions is not None
            and self.current_state == ArmState.IDLE
        )
...
if __name__ == "__main__":
    ...
    if angles is not None:
        self.get_logger().info(f"  Solved angles: {angles}")
        # Verify with FK
        fk_result = ik.forward_kinematics(angles)
        self.get_logger().info(f"  FK verification: {fk_result}")
        self.get_logger().info(f"  Error: {np.linalg.norm(fk_result - target):.4f}m")
    else:
        self.get_logger().info("  IK solver failed")
# =============================================================================
...
class RRTStarPlanner:
    ...
    def plan_trajectory(
        ...
        for iteration in range(self.max_iterations):
            ...
            for goal_node in self.goal_nodes:
                if (
                    self._collision_free(q_new, goal_node)
                    and q_new.distance_to(goal_node) < self.goal_tolerance
                ):
                    # Connect to goal
    ...
    def _sample_configuration(self) -> RRTNode:
        ...
        if random.random() < self.goal_bias and self.goal_nodes:
            ...
            config = np.clip(
                goal.configuration + perturbation,
                self._get_min_limits(),
                self._get_max_limits(),
            )
        else:
            # Sample uniformly in joint space
            config = np.random.uniform(
                self._get_min_limits(), self._get_max_limits(), self.num_joints
            )
