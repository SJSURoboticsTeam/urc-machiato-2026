        ...
        try:
            ...
            if not satisfied:
                ...
                self.violation_history.append(
                    {"timestamp": time.time(), "system_state": system_state.copy()}
                )
...
class SafetyMonitor(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                (
                    ...
                    5.0,
                ),  # Seconds before communication loss alert
        ...
        self.violation_alert_threshold = self.get_parameter(
        ...
        self.emergency_response_time = self.get_parameter(
    ...
    def _define_safety_properties(self) -> Dict[str, SafetyProperty]:
        ...
        def communication_evaluator(state):
            ...
            return time_since_comm < (
                self.communication_timeout * 2
            )  # More lenient when idle
        ...
        self.system_state.update(
            {
                ...
                "system_errors": list(msg.system_errors)
                if hasattr(msg, "system_errors")
                else [],
            }
        )
    ...
    def _alert_check_callback(self):
        ...
        time_since_comm = current_time - self.system_state.get(
            "last_communication", current_time
        )
    ...
    def _handle_safety_violation(self, violation: Dict[str, Any]):
        ...
        if severity == SafetyLevel.EMERGENCY.value:
            self.get_logger().fatal(
                f"EMERGENCY SAFETY VIOLATION: {property_name} - {details}"
            )
        elif severity == SafetyLevel.CRITICAL.value:
            self.get_logger().error(
                f"CRITICAL SAFETY VIOLATION: {property_name} - {details}"
            )
        elif severity == SafetyLevel.WARNING.value:
            self.get_logger().warn(
                f"WARNING SAFETY VIOLATION: {property_name} - {details}"
            )
        else:
    ...
    def _handle_get_safety_status(self, request, response):
        ...
        response.monitoring_stats.total_evaluations = self.monitoring_stats[
            "total_evaluations"
        ]
        response.monitoring_stats.total_violations = self.monitoring_stats[
            "total_violations"
        ]
        response.monitoring_stats.evaluation_rate = self.monitoring_stats[
            "evaluation_rate"
        ]
    ...
    def _calculate_overall_safety_level(self) -> str:
        ...
        if any(
            alert["severity"] == SafetyLevel.EMERGENCY.value
            for alert in self.monitoring_stats["active_alerts"]
        ):
            return SafetyLevel.EMERGENCY.value
        elif any(
            alert["severity"] == SafetyLevel.CRITICAL.value
            for alert in self.monitoring_stats["active_alerts"]
        ):
            return SafetyLevel.CRITICAL.value
        elif any(
            alert["severity"] == SafetyLevel.WARNING.value
            for alert in self.monitoring_stats["active_alerts"]
        ):
