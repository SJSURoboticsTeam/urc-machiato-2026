...
from autonomy_interfaces.msg import ContextState, AdaptiveAction as AdaptiveActionMsg, SystemState, ContextUpdate
from autonomy_interfaces.srv import GetContext, GetAdaptationHistory, GetSystemState

from .error_handling import handle_service_error, error_boundary, MonitoringError
from .config import Timing, MonitoringConfig
...
class MonitoringService(Node):
    ...
    def __init__(self) -> None:
        ...
        # Data storage
        self.context_history: deque = deque(maxlen=5000)  # Store last 5000 context readings
        self.adaptation_history: deque = deque(maxlen=1000)  # Store last 1000 adaptations
    ...
    def _setup_timers(self) -> None:
        ...
        self.dashboard_timer = self.create_timer(
            1.0 / self.dashboard_rate,
            self._dashboard_update_callback
        )

        # Analytics computation
        self.analytics_timer = self.create_timer(
            1.0 / self.analytics_rate,
            self._analytics_computation_callback
        )
    ...
    def _context_callback(self, msg: ContextState) -> None:
        ...
        try:
            ...
            handle_service_error(self.get_logger(), e, "context update", "MonitoringService")
    ...
    def _adaptation_callback(self, msg: AdaptiveActionMsg) -> None:
        ...
        try:
            ...
            if msg.priority >= 80:
                self._log_system_event('HIGH_PRIORITY_ADAPTATION',
    ...
    def _state_change_callback(self, msg: SystemState) -> None:
        ...
        try:
            self._log_system_event('STATE_CHANGE',
                                 f"State changed to {msg.current_state}")
    ...
    def _dashboard_update_callback(self) -> None:
        ...
        try:
            ...
            # Available actions
            dashboard_msg.available_actions = self._get_available_actions(latest_context)
    ...
    def _compute_performance_metrics(self) -> None:
        ...
        try:
            ...
            self.performance_metrics['battery_trend'] = self._calculate_trend(battery_levels)
            self.performance_metrics['battery_volatility'] = self._calculate_volatility(battery_levels)
            ...
            self.performance_metrics['avg_memory_usage'] = sum(memory_usage) / len(memory_usage)
            self.performance_metrics['cpu_volatility'] = self._calculate_volatility(cpu_usage)

            # Mission efficiency
            mission_progress = [ctx['context'].mission_progress for ctx in recent_contexts]
            if mission_progress:
                self.performance_metrics['mission_progress_rate'] = self._calculate_trend(mission_progress)
    ...
    def _compute_policy_effectiveness(self) -> None:
        ...
        try:
            ...
            for adaptation in recent_adaptations:
                ...
                policy_stats[action_type]['avg_priority'] += adaptation['action'].priority
    ...
    def _calculate_alert_level(self, context: ContextState) -> str:
        ...
        if (context.battery_critical or
            context.temperature > self.alert_thresholds['temperature_critical'] or
            context.cpu_usage > self.alert_thresholds['cpu_critical']):
            return "CRITICAL"

        if (context.battery_warning or
            context.obstacle_detected or
            not context.communication_active or
            context.safety_active):
            return "WARNING"
    ...
    # Service Callbacks
    def _context_query_callback(self, request: GetContext.Request, response: GetContext.Response) -> None:
    ...
    def _analytics_callback(self, request: GetAdaptationHistory.Request, response: GetAdaptationHistory.Response) -> None:
    ...
    def _health_callback(self, request: GetSystemState.Request, response: GetSystemState.Response) -> None:
    ...
    def _calculate_safety_frequency(self) -> float:
        ...
        safety_count = sum(1 for entry in self.context_history
                          if entry['context'].safety_active)
    ...
    def _cleanup_old_data(self) -> None:
        ...
        while (self.context_history and
               current_time - self.context_history[0]['timestamp'] > max_age):
            self.context_history.popleft()

        # Clean up old adaptation data
        while (self.adaptation_history and
               current_time - self.adaptation_history[0]['timestamp'] > max_age):
            self.adaptation_history.popleft()

        # Clean up old events
        while (self.system_events and
               current_time - self.system_events[0]['timestamp'] > max_age):
            self.system_events.popleft()
