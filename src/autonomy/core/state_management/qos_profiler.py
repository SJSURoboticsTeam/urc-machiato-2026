...
import psutil
import rclpy
from autonomy_interfaces.msg import ContextState, SystemState
from autonomy_interfaces.srv import GetNetworkStats, GetQoSProfile
...
class AdaptiveQoSManager:
    ...
    def adapt_topic_qos(
        self, topic_name: str, current_profile: QoSProfile
    ) -> QoSProfile:
...
class QoSProfiler(Node):
    ...
    def __init__(self):
        ...
        self.bandwidth_interval = self.get_parameter(
            "bandwidth_measurement_interval"
        ).value
    ...
    def _setup_services(self):
        ...
        self.create_service(
            GetQoSProfile, "/qos_profiler/get_profile", self._handle_get_profile
        )
    ...
    def _profiling_callback(self):
        ...
        for topic_name, metrics in self.topic_metrics.items():
            if len(metrics["latency"]) > 0:
                ...
                packet_loss_rate = metrics["packet_loss"] / max(
                    1, metrics["received_count"]
                )
    ...
    def _safety_monitoring_callback(self):
        ...
        if control_topic in self.topic_metrics:
            ...
            if latencies:
                ...
                if (
                    recent_avg
                    > self.safety_properties["control_loop_timing"]["threshold_ms"]
                ):
    ...
    def _adapt_telemetry_rate(self):
        ...
        if (
            now - self.adaptive_params["last_adaptation"] < 5.0
        ):  # Minimum 5 seconds between adaptations
            return
        ...
        avg_bandwidth = (
            statistics.mean(bandwidth_up) + statistics.mean(bandwidth_down)
        ) / 2.0
        max_allowed = (
            self.urc_band_config["900mhz"]["max_bandwidth_mhz"] or 100.0
        )  # Fallback for 2.4GHz
        ...
        # If conditions are good, can increase rate
            ...
            and avg_latency < self.adaptive_params["latency_target"] * 0.5
        ):
            target_rate *= 1.05  # Increase by 5%

        # Apply bounds
        target_rate = max(
            self.adaptive_params["min_telemetry_rate"],
        ...
        new_rate = (
            self.adaptive_params["current_telemetry_rate"] * (1 - adaptation_factor)
            + target_rate * adaptation_factor
        )

        if (
            abs(new_rate - self.adaptive_params["current_telemetry_rate"]) > 0.1
    ...
    def _check_band_compliance(self):
        ...
        total_bandwidth = sum(self.network_metrics["bandwidth_up"]) / len(
            self.network_metrics["bandwidth_up"]
        ) + sum(self.network_metrics["bandwidth_down"]) / len(
            self.network_metrics["bandwidth_down"]
        )
    ...
    def _handle_get_profile(self, request, response):
        ...
        for topic_name, metrics in self.topic_metrics.items():
            profile = {
                ...
                "avg_latency_ms": statistics.mean(metrics["latency"])
                if metrics["latency"]
                else 0.0,
                "jitter_ms": statistics.stdev(metrics["latency"])
                if len(metrics["latency"]) > 1
                else 0.0,
                "packet_loss_rate": metrics["packet_loss"]
                / max(1, metrics["received_count"]),
