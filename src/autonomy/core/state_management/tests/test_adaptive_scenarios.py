...
class TestMissionScenarios:
    ...
    def test_sample_collection_mission_scenario(self, adaptive_system):
        ...
        scenario = MissionScenario(
            ...
            expected_transitions=[
                {'from': RoverState.READY, 'to': RoverState.AUTO, 'reason': 'mission_start'},
            ],
    ...
    def test_equipment_servicing_with_obstacles(self, adaptive_system):
        ...
        scenario = MissionScenario(
            ...
            expected_transitions=[
                {'from': RoverState.READY, 'to': RoverState.AUTO, 'reason': 'mission_start'},
                {'from': RoverState.AUTO, 'to': RoverState.PAUSED, 'reason': 'critical_obstacle'},
            ],
            adaptive_expectations=[
    ...
    def test_emergency_battery_scenario(self, adaptive_system):
        ...
        scenario = MissionScenario(
            ...
            expected_transitions=[
                {'from': RoverState.AUTO, 'to': RoverState.READY, 'reason': 'complete_and_return'},
            ],
            adaptive_expectations=[
    ...
    def test_multi_condition_crisis_scenario(self, adaptive_system):
        ...
        scenario = MissionScenario(
            ...
            expected_transitions=[
                {'from': RoverState.AUTO, 'to': RoverState.ESTOP, 'reason': 'multiple_critical_conditions'},
            ],
            adaptive_expectations=[
    ...
    def _execute_scenario(self, system: Dict, scenario: MissionScenario):
        ...
        for action in actions:
            self.get_logger().info(f"Generated action: {action.action_type.value} "
    ...
    def _verify_scenario_expectations(self, scenario: MissionScenario,
                                    actions: List[AdaptiveAction], final_state: RoverState):
        ...
        if "Battery" in scenario_name and "Emergency" in scenario_name:
            ...
            assert len(complete_actions) > 0, f"Expected complete and return action in {scenario_name}"
            # Check that it's a reasonable priority for the situation
            assert complete_actions[0].priority >= 60, f"Expected reasonable priority action in {scenario_name}"

        elif "Obstacle" in scenario_name:
            intervention_actions = [a for a in actions if a.action_type == AdaptiveActionType.REQUEST_HUMAN_INTERVENTION]
            assert len(intervention_actions) > 0, f"Expected human intervention action in {scenario_name}"

        elif "Communication" in scenario_name:
            # Communication loss triggers appropriate adaptive actions
            abort_actions = [a for a in actions if a.action_type == AdaptiveActionType.MISSION_ABORT]
            safe_mode_actions = [a for a in actions if a.action_type == AdaptiveActionType.COMMUNICATION_SAFE_MODE]
            assert len(abort_actions) > 0 or len(safe_mode_actions) > 0, f"Expected abort or safe mode action in {scenario_name}"

        elif "System Overload" in scenario_name:
            throttle_actions = [a for a in actions if a.action_type == AdaptiveActionType.SYSTEM_THROTTLE]
            assert len(throttle_actions) > 0, f"Expected throttle action in {scenario_name}"

        # Verify appropriate priority actions are present for critical scenarios
        if any(keyword in scenario_name.lower() for keyword in ['emergency', 'critical', 'crisis']):
            ...
            if "Battery" in scenario_name and scenario.initial_context.get('mission_progress', 0) > 0.9:
                appropriate_actions = [a for a in actions if a.priority >= 60]
            else:
                appropriate_actions = [a for a in actions if a.priority >= 80]
            assert len(appropriate_actions) > 0, f"Expected appropriate priority actions in {scenario_name}"
...
class TestAdaptiveLearning:
    ...
    def test_policy_effectiveness_improvement(self, node):
        ...
        test_actions = [
            AdaptiveAction(AdaptiveActionType.EMERGENCY_RETURN, 100, {}, "", 100.0, ContextState()),
            AdaptiveAction(AdaptiveActionType.OBSTACLE_AVOIDANCE, 80, {}, "", 60.0, ContextState()),
            AdaptiveAction(AdaptiveActionType.REDUCE_POWER, 70, {}, "", 120.0, ContextState()),
        ]
    ...
    def test_context_pattern_recognition(self, node):
        ...
        for i in range(50):
            ...
            # Create patterns
            context_entry['context'].battery_level = 80.0 - (i % 10) * 2  # Declining pattern
            context_entry['context'].cpu_usage = 40.0 + (i % 5) * 3      # Oscillating pattern
...
class TestURCCompetitionScenarios:
    ...
    def test_judge_transparency_features(self, node):
        ...
        with patch('rclpy.node.Node.create_timer'), \
            ...
            required_fields = [
                'context_readings', 'adaptation_actions', 'system_events',
                'performance_metrics', 'policy_effectiveness'
            ]
