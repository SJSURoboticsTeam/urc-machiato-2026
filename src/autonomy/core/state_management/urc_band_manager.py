...
class URCBandManager(Node):
    ...
    def __init__(self):
        ...
        self.interference_monitoring = self.get_parameter(
            "interference_monitoring"
        ).value
        self.interference_check_rate = self.get_parameter(
            "interference_check_rate"
        ).value
    ...
    def set_band(self, band: str, subband: Optional[str] = None) -> bool:
        ...
        if (
            self.band_switching_enabled
            and time.time() - self.last_band_switch < self.band_switch_cooldown
        ):
        ...
        if band == "900mhz":
            if subband and subband in self.band_config["900mhz"]["sub_bands"]:
                ...
                self.band_config["900mhz"]["sub_bands"][subband][
                    "last_used"
                ] = time.time()
            ...
            self.get_logger().info(
                f"Switched to 2.4GHz band (channel {self.band_config['2_4ghz']['current_channel']})"
            )
    ...
    def _interference_check_callback(self):
        ...
        for subband in ["low", "mid", "high"]:
            ...
            self.interference_history["900mhz"][subband].append(
                {"timestamp": time.time(), "level": interference}
            )
        ...
        self.interference_history["2_4ghz"].append(
            {"timestamp": time.time(), "level": interference_2_4}
        )
        ...
        if current_band == "900mhz":
            ...
            if current_interference > self.interference_threshold:
                ...
                # If all 900MHz subbands have high interference, consider switching to 2.4GHz
                    ...
                    interference_levels.get("2_4ghz", 1.0) < self.interference_threshold
                ):
                    self.get_logger().warn(
    ...
    def _switch_2_4ghz_channel(self):
        ...
        current_channel = self.band_config["2_4ghz"]["current_channel"]
        available_channels = [
            ch for ch in self.band_config["2_4ghz"]["channels"] if ch != current_channel
        ]

        if available_channels:
            new_channel = available_channels[
                0
    ...
    def _frequency_hopping_callback(self):
        ...
        if (
            not self.band_config["900mhz"]["frequency_hopping"]
            or self.current_band != "900mhz"
        ):
            return
        ...
        if self.interference_history["900mhz"][next_subband]:
            ...
            next_interference = sum(m["level"] for m in recent_measurements) / len(
                recent_measurements
            )
    ...
    def _notify_band_change(self, old_band: str, new_band: str, subband: Optional[str]):
        ...
        self.get_logger().info(
            f"Band changed: {old_band} -> {new_band}"
            + (f" ({subband})" if subband else "")
        )
