...
class LEDController(StateMachineNode):
    ...
    def _load_configuration(self) -> dict:
        ...
        config["enable_hardware"] = get_validated_parameter(
            self, "enable_hardware", True, logger=self.logger
        )

        config["debug_mode"] = get_validated_parameter(
            self, "debug_mode", False, logger=self.logger
        )
    ...
    def _initialize_led_hardware(self) -> None:
        ...
        if error:
            self.logger.error(
                "Failed to initialize LED hardware, using simulation mode", error=error
            )
            return
    ...
    def led_info_callback(self, msg: String) -> None:
        ...
        if led_info != self.current_led_info:
            ...
            if error:
                self.logger.error(
                    "Failed to process LED info", error=error, led_info=led_info
                )
    ...
    def process_led_info(self, led_info: str) -> None:
        ...
        if led_info == "AUTONOMOUS_RED":
            ...
            self._set_led_state_safe(LEDColor.RED, LEDPattern.FAST_BLINK)
            self.logger.warn(
                "Error LED pattern set", error_type=error_type, led_info=led_info
            )
            ...
            self._start_success_flashing()
            self.logger.info(
                "Success LED pattern set", success_type=success_type, led_info=led_info
            )
    ...
    def _set_led_state_safe(self, color: LEDColor, pattern: LEDPattern) -> None:
        ...
        try:
            ...
            if self.led_hardware:
                ...
                # Software simulation
                self.logger.info(
                    "LED set (simulation)", color=color.value, pattern=pattern.value
                )
