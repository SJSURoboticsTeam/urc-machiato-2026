...
from control.hardware_emergency_stop import (
    HardwareEmergencyStop, EmergencyStopConfig, initialize_emergency_stop
)
...
from core.graceful_degradation import (
    OperationModeManager, OperationMode, create_operation_mode_manager
)
...
class IntegratedCriticalSystems(Node):
        ...
        try:
            motor_config = {
                ...
                'predictive_maintenance': {
                    ...
                    'temperature_trend_threshold': self.config.get('temp_trend_threshold', 0.1)
                },
                'encoders': {
    ...
    def _initialize_graceful_degradation(self):
        ...
        try:
            ...
            self.operation_mode_manager = create_operation_mode_manager(degradation_config)
    ...
    def _emergency_stop_callback(self, status: str, e_stop_info: Dict[str, Any]):
        ...
        if status == "soft_stop_activated" or status == "hard_stop_activated":
            ...
            logger.critical(f"Emergency stop activated: {e_stop_info.get('stop_reason', 'unknown')}")
    ...
    def _update_performance_metrics(self):
        ...
        self.performance_metrics['mode_switches'] = motor_status.get('maintenance_alerts', {})
    ...
    def _check_system_health(self):
        ...
        failed_sensors = [sid for sid, info in sensor_status.items() 
    ...
    def run_competition_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_results['emergency_stop'] = (
                e_stop_status['gpio_initialized'] and
                e_stop_status['watchdog_enabled']
            )
            
            # Test motor control
            motor_status = self.motor_controller.get_advanced_status()
            validation_results['motor_control'] = (
                motor_status['control_active'] and
                len(motor_status['rms_errors']) > 0
            )
            ...
            validation_results['graceful_degradation'] = (
                mode_status['current_mode'] in [mode.value for mode in OperationMode]
            )
...
# Factory function
def create_integrated_critical_systems(config: Dict[str, Any] = None) -> IntegratedCriticalSystems:
