...
describe('MessageTester Component', () => {
  ...
  test('handles CAN message sending', async () => {
    render(<MessageTester {...mockProps} />);
  ...
  test('renders message history correctly', () => {
    const testHistory = [
      { id: 'msg_1', content: 'test message 1', target: 'websocket', status: 'sent' },
      { id: 'msg_2', content: 'test message 2', target: 'ros2', status: 'delivered' },
      { id: 'msg_3', content: 'test message 3', target: 'can', status: 'error' }
    ];

    render(<MessageTester {...mockProps} messageHistory={testHistory} />);

    // Check that messages are displayed (should show last 5, but we have 3)
