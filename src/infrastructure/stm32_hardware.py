...
class STM32CANBridge:
    ...
    def send_motor_command(self, motor_id: int, speed: float, torque: float = 0.0) -> bool:
    ...
    def _process_priority_queue(self) -> bool:
        ...
        for priority in sorted(self.queues.keys()):
            ...
            while queue and self.connected:
                ...
                # Track latency
                if success:
                    latency = (time.time() - message['timestamp']) * 1000000  # microseconds
                    self.performance_stats['can_latency_us'].append(latency)
                    
                    # Keep only last 1000 latency measurements
                    if len(self.performance_stats['can_latency_us']) > 1000:
                        self.performance_stats['can_latency_us'] = self.performance_stats['can_latency_us'][-1000:]
            ...
            while self.control_loop_running:
                ...
                if current_time - last_time >= target_period:
                    ...
                    if iterations % 100 == 0:  # Every 100 iterations
                        actual_freq = 100.0 / (current_time - (last_time - target_period))
                        self.performance_stats['control_loop_frequency'] = actual_freq
                else:
                    # Sleep to maintain frequency
                    time.sleep(0.001)  # 1ms sleep
        
        self.control_thread = threading.Thread(target=control_loop, name="STM32_ControlLoop")
    ...
    def _start_telemetry_loop(self):
        ...
        self.telemetry_thread = threading.Thread(target=telemetry_loop, name="STM32_Telemetry")
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if stats['can_latency_us']:
            stats['avg_can_latency_us'] = sum(stats['can_latency_us']) / len(stats['can_latency_us'])
...
class STM32HardwareNode(Node):
    ...
    def __init__(self):
        ...
        self.status_timer = self.create_timer(
            1.0 / 10.0,  # 10Hz status publishing
    ...
    def _publish_performance(self):
        ...
        performance_msg.data = f"CAN Latency: {stats.get('avg_can_latency_us', 0):.1f}μs"
