...
class SafeEvaluator(ast.NodeVisitor):
    ...
    def visit_Name(self, node):
        ...
        if (name in self.config.allowed_functions or 
            name in self.config.allowed_modules):
            if name == 'math':
                return math
...
def safe_eval(
    ...
    if not config.allow_eval:
        raise SecurityError("Expression evaluation not allowed in current configuration")
