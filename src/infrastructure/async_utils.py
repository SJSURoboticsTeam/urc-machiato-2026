...
class AsyncLockManager:
    ...
    async def run_blocking(self, func: Callable[..., T], *args, timeout: Optional[float] = None) -> T:
...
class AsyncPerformanceMonitor:
    ...
    async def monitor_operation(self, operation_name: str, operation: Awaitable[T]) -> T:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if attempt < max_attempts - 1:
                ...
                import logging
                logger = logging.getLogger(__name__)
                logger.warning(f"Operation failed (attempt {attempt + 1}/{max_attempts}), retrying in {wait_time:.1f}s: {e}")
...
async def async_gather_with_errors(*operations: Awaitable[T]) -> tuple[list[T], list[Exception]]:
...
def sync_to_async(func: Callable[..., T]) -> Callable[..., Awaitable[T]]:
    ...
    async def wrapper(*args, **kwargs):
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(_async_manager.executor, func, *args, **kwargs)
    return wrapper
...
def performance_monitor(operation_name: str):
    ...
    def decorator(func: Callable[..., Awaitable[T]]) -> Callable[..., Awaitable[T]]:
        async def wrapper(*args, **kwargs):
            return await _perf_monitor.monitor_operation(operation_name, func(*args, **kwargs))
        return wrapper
    return decorator
