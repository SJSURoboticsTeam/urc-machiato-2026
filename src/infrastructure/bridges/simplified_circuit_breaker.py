        ...
        with self._lock:
            ...
            if self.is_open:
                self._logger.warning(
                    f"Circuit breaker OPEN - rejecting call to {func.__name__}"
                )
                return None
            ...
            try:
                ...
                if self._state == CircuitState.HALF_OPEN:
                    if self._stats.success_count >= self.success_threshold:
                        self._state = CircuitState.CLOSED
                        self._logger.info(
                            f"Circuit breaker CLOSED after {self._stats.success_count} successes"
                        )
                ...
                if self._stats.failure_count >= self.failure_threshold:
                    ...
                    self._logger.error(
                        f"Circuit breaker OPEN after {self._stats.failure_count} failures"
                    )
                elif self._state == CircuitState.CLOSED:
                    self._state = CircuitState.HALF_OPEN
                    self._logger.warning(
                        f"Circuit breaker HALF_OPEN after {self._stats.failure_count} failures"
                    )
            ...
            return {
                ...
                "stats": {
                    ...
                    "time_since_last_failure": (
                        ...
                        if self._stats.last_failure_time > 0
                        else 0
                    ),
                    "config": {
