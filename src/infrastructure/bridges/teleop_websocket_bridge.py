...
Classes
-------
TeleopConfig : dataclass
    Configuration for teleoperation bridge.
TeleopWebSocketBridge : Node
    Main bridge class for bidirectional ROS2-Socket.IO communication.

Author
------
URC 2026 Bridge Integration Team
"""
...
class TeleopConfig:
    ...
    ----------
    server_url : str
        URL of the teleoperation Socket.IO server.
        Default is "http://localhost:5000".
    reconnect_delay : float
        Delay in seconds between reconnection attempts.
        Default is 2.0 seconds.
    status_publish_rate : float
        Rate at which status updates are sent to frontend (Hz).
        Default is 10.0 Hz.
    command_timeout : float
        Timeout in seconds for command safety check.
...
class TeleopWebSocketBridge(Node):
    ...
    sio : socketio.AsyncClient
        Socket.IO client for frontend communication.
    is_connected : bool
        Connection status to teleoperation server.
    connection_attempts : int
        Number of connection attempts made.
    commands_received : int
        Total commands received from frontend.
    commands_published : int
        Total commands published to ROS2.
    status_sent : int
        Total status messages sent to frontend.
    errors : int
        Total errors encountered.
    latest_data : dict
        Latest sensor and system data for status publishing.

    Notes
    -----
    This bridge enables bidirectional communication between the frontend
    ...
    def _setup_socketio_handlers(self) -> None:
        ...
        Registers event handlers for connection, disconnection, and
        drive command events from the frontend.

        Notes
        -----
        Event handlers registered:
        - `connect`: Handles successful connection to server
    ...
    def _setup_ros2_publishers(self) -> None:
        ...
        - Teleoperation velocity commands (`/cmd_vel/teleop`)
        - Homing requests (`/hardware/homing_request`)
    ...
    def _setup_ros2_subscribers(self) -> None:
        ...
        Creates subscribers for:
        - Velocity feedback (`/hardware/velocity_feedback`)
        - Battery status (`/battery_state`)
        - IMU data (`/imu/data`)
        - Diagnostics (`/diagnostics`)
        - System state (`/adaptive_state_machine/state`)
