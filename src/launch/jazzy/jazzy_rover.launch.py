...
def generate_launch_description():
    ...
    bt_orchestrator = LifecycleNode(
        ...
        parameters=[{
            'bt_file': os.path.join(
                ...
                'behavior_trees', 'main_mission.xml'
            ),
    ...
    performance_profiler = Node(
        ...
        parameters=[{
            ...
            'metrics': ['cpu_usage', 'memory_usage', 'network_latency', 'bt_execution_time']
        }],
        condition=IfCondition(LaunchConfiguration('monitoring'))
    )
    ...
    logging_config = ExecuteProcess(
        cmd=[
            'ros2', 'param', 'set', '/jazzy_component_manager',
            'log_level', 'info'
        ],
    ...
    topic_monitor = ExecuteProcess(
        cmd=[
            'ros2', 'topic', 'hz', '/jazzy_component_manager/health',
            '--window', '-1'  # Monitor continuously
        ],
        name='topic_monitor',
        output='screen',
        condition=IfCondition(LaunchConfiguration('monitoring'))
    ...
    ld = LaunchDescription([
        ...
        *set_jazzy_env,
        ...
        TimerAction(
            period=2.0,
            actions=[configure_component_manager]
        ),

        TimerAction(
            period=3.0,
            actions=[configure_state_machine]
        ),

        TimerAction(
            period=4.0,
            actions=[configure_bt_orchestrator]
        ),

        # Activate components in sequence
        TimerAction(
            period=6.0,
            actions=[activate_component_manager]
        ),
        ...
        TimerAction(
            period=8.0,
            actions=[activate_bt_orchestrator]
        ),

        # Start monitoring
        TimerAction(
            period=10.0,
            actions=[topic_monitor]
        ),
