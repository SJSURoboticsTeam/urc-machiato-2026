...
class FaultInjectionFramework(Node):
    ...
    def clear_all_faults(self):
        ...
        for fault_id in fault_ids:
            clear_msg = String()
            clear_msg.data = json.dumps(
                {"command": "clear_fault", "fault_id": fault_id, "immediate": True}
            )
            self.fault_control_pub.publish(clear_msg)
    ...
    def get_fault_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fault_types_tested": list(
                set(f.fault_type.value for f in self.fault_history)
            ),
            "active_faults": len(self.active_faults),
...
class FaultInjectionNode(Node):
    ...
    def __init__(self):
        ...
        self.imu_pub = self.create_publisher(
            String, "/imu_fault_injected", 10
        )  # Placeholder for actual IMU msg
...
def run_fault_injection_demo():
    ...
    try:
        ...
        self.get_logger().info("Injecting random faults for chaos testing...")
        for _ in range(3):
        ...
        self.get_logger().info(f"  Total Injections: {stats['total_injections']}")
        self.get_logger().info(f"  Active Faults: {stats['active_faults']}")
        self.get_logger().info(f"  Recovery Rate: {stats.get('recovery_rate', 0):.1f}")
        print(f"  Fault Types Tested: {', '.join(stats['fault_types_tested'])}")
        print("\nFAULT INJECTION FRAMEWORK DEMONSTRATION COMPLETE")
        self.get_logger().info(
            "System robustness tested with multiple failure scenarios!"
        )
