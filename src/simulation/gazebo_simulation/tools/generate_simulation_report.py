...
class SimulationReportGenerator:
    ...
    def generate_report(self) -> SimulationAdequacyReport:
        ...
        self.get_logger().info("Generating simulation adequacy report...")
        # Load test results
    ...
    def load_test_results(self) -> Dict[str, Any]:
        ...
        for file_path in self.results_directory.glob("*_results_*.json"):
            try:
                ...
                self.get_logger().info(f"Warning: Could not load {file_path}: {e}")
        return test_results
    ...
    def load_sensor_quality_data(self) -> Optional[Dict[str, Any]]:
        ...
        sensor_quality_file = (
            ...
            / f"sensor_quality_report_{int(self.report_timestamp)}.json"
        )
        ...
        if sensor_quality_file.exists():
            try:
                ...
                self.get_logger().info(
                    f"Warning: Could not load sensor quality data: {e}"
                )
        return None

    def analyze_slam_performance(
        ...
        if gps_denied_test:
            ...
            if drift_metrics.get("total_drift", 0) > 0:
                metrics.pose_drift = max(
                    metrics.pose_drift, drift_metrics["total_drift"]
                )

        return metrics

    def analyze_navigation_capability(
        self, test_results: Dict[str, Any]
    ) -> NavigationCapabilityMetrics:
        ...
        if waypoint_test:
            ...
            metrics.waypoint_success_rate = (
                waypoints_completed / total_waypoints
            ) * 100.0

        # Analyze obstacle avoidance results
        obstacle_test = test_results.get("dynamic_obstacle_avoidance", {})
        if obstacle_test:
            metrics.obstacle_avoidance_success = obstacle_test.get(
                "obstacle_avoidance_success", 0.0
            )
        ...
        if endurance_test:
            metrics.real_time_performance = endurance_test.get(
    ...
    def analyze_sensor_fidelity(
        self, sensor_quality: Optional[Dict[str, Any]]
    ) -> SensorFidelityMetrics:
        ...
        if sensor_quality:
            ...
            metrics.imu_fidelity = sensor_reports.get("imu", {}).get(
                "fidelity_score", 0.0
            )
            metrics.gps_fidelity = sensor_reports.get("gps", {}).get(
                "fidelity_score", 0.0
            )
            metrics.lidar_fidelity = sensor_reports.get("lidar", {}).get(
                "fidelity_score", 0.0
            )
            metrics.camera_fidelity = sensor_reports.get("camera", {}).get(
                "fidelity_score", 0.0
            )
            metrics.depth_camera_fidelity = sensor_reports.get("depth_camera", {}).get(
                "fidelity_score", 0.0
            )
    ...
    def identify_simulation_limitations(
        self, test_results: Dict[str, Any], sensor_quality: Optional[Dict[str, Any]]
    ...
    def calculate_adequacy(
        ...
        if navigation_capability.waypoint_success_rate > 90.0:  # High success rate
            nav_score += 0.4
        if (
            navigation_capability.obstacle_avoidance_success > 90.0
        ...
        if score >= 0.9:
            grade = "A"
            recommendation = (
                "Proceed with Development - High confidence in simulation adequacy"
            )
        elif score >= 0.8:
            grade = "B"
    ...
    def generate_recommendations(
        ...
        if slam_performance.pose_drift > 2.0:
            critical_tests.append(
                "SLAM pose estimation accuracy validation on real hardware"
            )
        ...
        if sensor_fidelity.imu_fidelity < 0.8:
            parameter_adjustments.append(
                "Adjust IMU noise parameters to better match real sensors"
            )
    ...
    def save_report(self, report: SimulationAdequacyReport):
        ...
        report_file = (
            self.results_directory
            / f"simulation_adequacy_report_{int(self.report_timestamp)}.json"
        )
    ...
    def print_summary(self, report: SimulationAdequacyReport):
        ...
        self.get_logger().info("SIMULATION ADEQUACY ASSESSMENT REPORT")

        self.get_logger().info(
            f"Overall Adequacy Score: {report.overall_adequacy_score:.2f}"
        )
        self.get_logger().info(f"Grade: {report.adequacy_grade}")
        self.get_logger().info(f"Recommendation: {report.recommendation}")
        self.get_logger().info(f"Test Coverage: {report.coverage_percentage:.1f}%")
        self.get_logger().info("\nSLAM Performance:")
        self.get_logger().info(
            f"  Pose Drift: {report.slam_performance.pose_drift:.3f}m"
        )
        self.get_logger().info(
            f"  Loop Closure Success: {report.slam_performance.loop_closure_success:.1f}%"
        )
        self.get_logger().info(
            f"  Waypoint Accuracy: {report.slam_performance.waypoint_accuracy:.3f}m"
        )
        self.get_logger().info(
            f"  Confidence: {report.slam_performance.confidence:.2f}"
        )
        self.get_logger().info("\nNavigation Capability:")
        self.get_logger().info(
            f"  Waypoint Success Rate: {report.navigation_capability.waypoint_success_rate:.1f}%"
        )
        self.get_logger().info(
            f"  Obstacle Avoidance Success: {report.navigation_capability.obstacle_avoidance_success:.1f}%"
        )
        self.get_logger().info(
            f"  Real-time Performance: {'Yes' if report.navigation_capability.real_time_performance else 'No'}"
        )
        self.get_logger().info(
            f"  Collision Count: {report.navigation_capability.collision_count}"
        )
        self.get_logger().info("\nSensor Fidelity:")
        self.get_logger().info(
            f"  Overall Fidelity: {report.sensor_fidelity.overall_fidelity:.2f}"
        )
        self.get_logger().info(
            f"  IMU Fidelity: {report.sensor_fidelity.imu_fidelity:.2f}"
        )
        self.get_logger().info(
            f"  GPS Fidelity: {report.sensor_fidelity.gps_fidelity:.2f}"
        )
        self.get_logger().info(
            f"  LiDAR Fidelity: {report.sensor_fidelity.lidar_fidelity:.2f}"
        )
        self.get_logger().info(
            f"  Camera Fidelity: {report.sensor_fidelity.camera_fidelity:.2f}"
        )
        self.get_logger().info("\nKey Limitations:")
        for limitation in report.limitations.missing_physical_effects[:3]:
            self.get_logger().info(f"  - {limitation}")
        if len(report.limitations.missing_physical_effects) > 3:
            self.get_logger().info(
                f"  - ... and {len(report.limitations.missing_physical_effects) - 3} more"
            )
        ...
        if len(report.critical_tests_for_hardware) > 3:
            self.get_logger().info(
                f"  - ... and {len(report.critical_tests_for_hardware) - 3} more"
            )
