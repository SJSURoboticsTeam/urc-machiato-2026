...
class WaypointNavigationTester(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f"Testing {len(self.waypoints)} waypoints over {self.max_test_time/60:.1f} minutes"
        )
    ...
    def slam_pose_callback(self, msg):
        ...
        self.slam_poses.append(
            {"pose": pose, "timestamp": time.time(), "frame_id": msg.header.frame_id}
        )
    ...
    def laser_callback(self, msg):
        ...
        self.get_logger().debug(
            f"Laser scan: {valid_ranges}/{len(msg.ranges)} valid ranges"
        )
    ...
    def calculate_test_results(self):
        ...
        if len(self.slam_poses) > 1:
            ...
            results["slam_pose_frequency"] = len(self.slam_poses) / (
    ...
    def log_test_results(self):
        ...
        if "loop_closure_error" in self.test_results:
            ...
            if lce < 1.0:
                self.get_logger().info(
                    f"[SUCCESS] Loop closure error: {lce:.3f}m (< 1m required)"
                )
            else:
                self.get_logger().warn(
                    f"[ERROR] Loop closure error: {lce:.3f}m (>= 1m)"
                )
        ...
        if "slam_pose_frequency" in self.test_results:
            ...
            if freq > 5.0:
                self.get_logger().info(
                    f"[SUCCESS] SLAM frequency: {freq:.1f} Hz (> 5Hz required)"
                )
            else:
                self.get_logger().warn(f"[ERROR] SLAM frequency: {freq:.1f} Hz (< 5Hz)")
