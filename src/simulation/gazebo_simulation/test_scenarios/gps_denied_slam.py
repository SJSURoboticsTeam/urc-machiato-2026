...
class GPSDeniedSLAMTest(Node):
    ...
    def test_loop(self):
        ...
        if self.status == TestStatus.GPS_AVAILABLE:
            if phase_duration >= self.gps_available_phase_duration:
                ...
                self.gps_denied_start_pose = (
                    self.slam_poses[-1] if self.slam_poses else None
                )
    ...
    def update_metrics(self):
        ...
        if self.gps_denied_start_pose and len(self.slam_poses) > 0:
            ...
            drift_per_minute = (
                total_drift / (gps_denied_duration / 60.0)
                if gps_denied_duration > 0
                else 0.0
            )
        ...
        if self.visual_features_data:
            recent_features = [
                f["feature_count"] for f in self.visual_features_data[-10:]
            ]
            self.slam_drift_metrics.visual_features_count = int(
                np.mean(recent_features)
            )

        # Update depth quality
        if hasattr(self, "depth_quality_samples") and self.depth_quality_samples:
            self.slam_drift_metrics.depth_quality = np.mean(
                self.depth_quality_samples[-10:]
            )
    ...
    def complete_test(self):
        ...
        self.get_logger().info(
            f"GPS-denied duration: {self.gps_denied_phase_duration:.2f}s"
        )
        self.get_logger().info(
            f"Total drift: {self.slam_drift_metrics.total_drift:.3f}m"
        )
        self.get_logger().info(
            f"Drift per minute: {self.slam_drift_metrics.drift_per_minute:.3f}m/min"
        )
        self.get_logger().info(
            f"Visual features: {self.slam_drift_metrics.visual_features_count}"
        )
        self.get_logger().info(
            f"Depth quality: {self.slam_drift_metrics.depth_quality:.2f}"
        )
    ...
    def calculate_final_metrics(self):
        ...
        if self.gps_recovery_start_time and self.gps_available:
            self.slam_drift_metrics.gps_reacquisition_time = (
                time.time() - self.gps_recovery_start_time
            )
        ...
        if len(self.slam_poses) > 10:
            ...
            for i in range(1, len(self.slam_poses)):
                interval = (
                    self.slam_poses[i]["timestamp"]
                    - self.slam_poses[i - 1]["timestamp"]
                )
            ...
            self.slam_drift_metrics.navigation_continuity = (
                max_interval < 2.0
            )  # Less than 2s gap
