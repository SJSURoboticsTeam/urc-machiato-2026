...
class DynamicObstacleAvoidanceTester(Node):
    ...
    def complete_test(self):
        ...
        avg_clearance = (
            np.mean(self.clearance_distances) if self.clearance_distances else 0.0
        )
        avg_detection_time = (
        ...
        completion_msg.data = (
            f"OBSTACLE_AVOIDANCE_TEST_COMPLETE|Success:{result.navigation_success}"
        )
        self.test_status_pub.publish(completion_msg)

        self.get_logger().info("Dynamic Obstacle Avoidance Test completed")
        self.get_logger().info(
            f"Results saved to /tmp/dynamic_obstacle_test_results.json"
        )

        # Print summary

        self.get_logger().info("DYNAMIC OBSTACLE AVOIDANCE TEST RESULTS")

        self.get_logger().info(f"Obstacles Encountered: {obstacles_encountered}")
        self.get_logger().info(f"Obstacles Avoided: {obstacles_avoided}")
        self.get_logger().info(f"Collisions: {len(self.collision_events)}")
        self.get_logger().info(f"Path Replans: {self.path_replans}")
        self.get_logger().info(
            f"Navigation Success: {' YES' if result.navigation_success else ' NO'}"
        )
