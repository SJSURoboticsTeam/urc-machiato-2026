...
class SensorTimestampProvider:
    ...
    SENSOR_LATENCIES = {
        SensorType.IMU: SensorLatencyProfile(
            typical_latency_ms=5.0, max_latency_ms=15.0, jitter_ms=2.0,
            calibration_required=True, health_check_interval_ms=1000
        ),
        SensorType.GPS: SensorLatencyProfile(
            typical_latency_ms=100.0, max_latency_ms=200.0, jitter_ms=20.0,
            calibration_required=True, health_check_interval_ms=5000
        ),
        SensorType.SLAM: SensorLatencyProfile(
            typical_latency_ms=50.0, max_latency_ms=100.0, jitter_ms=10.0,
            calibration_required=True, health_check_interval_ms=2000
        ),
        SensorType.CAMERA: SensorLatencyProfile(
            typical_latency_ms=33.0, max_latency_ms=66.0, jitter_ms=5.0,  # 30Hz frame rate
            calibration_required=False, health_check_interval_ms=1000
        ),
        SensorType.WHEEL_ENCODER: SensorLatencyProfile(
            typical_latency_ms=1.0, max_latency_ms=5.0, jitter_ms=0.5,
            calibration_required=False, health_check_interval_ms=1000
        ),
        SensorType.BATTERY: SensorLatencyProfile(
            typical_latency_ms=10.0, max_latency_ms=50.0, jitter_ms=5.0,
            calibration_required=False, health_check_interval_ms=10000
        ),
        SensorType.TEMPERATURE: SensorLatencyProfile(
            typical_latency_ms=100.0, max_latency_ms=500.0, jitter_ms=50.0,
            calibration_required=False, health_check_interval_ms=30000
        ),
        SensorType.LIDAR: SensorLatencyProfile(
            typical_latency_ms=20.0, max_latency_ms=50.0, jitter_ms=5.0,
            calibration_required=True, health_check_interval_ms=1000
        )
    }
    ...
    def __init__(self):
        ...
        for sensor_type in SensorType:
            self._latency_calibration[sensor_type] = self.SENSOR_LATENCIES[sensor_type].typical_latency_ms
    ...
    def tag_sensor_data(self, sensor_type: SensorType, raw_data: Dict[str, Any]) -> TimestampedSensorData:
        """
        ...
        with self._lock:
            ...
            calibrated_latency_ms = self._latency_calibration.get(sensor_type,
                self.SENSOR_LATENCIES[sensor_type].typical_latency_ms)

            # Calculate measurement timestamp (reception - latency)
            measurement_timestamp_ns = reception_timestamp_ns - int(calibrated_latency_ms * 1_000_000)
            ...
            self._sequence_counters[sensor_type] = (sequence_number + 1) % 2**32  # Wrap at 4 billion

            # Calculate quality score based on latency
            quality_score = self._calculate_quality_score(sensor_type, latency_ms)

            # Update health monitoring
            self._update_sensor_health(sensor_type, latency_ms, quality_score, reception_timestamp_ns)
    ...
    def _calculate_quality_score(self, sensor_type: SensorType, latency_ms: float) -> float:
        """
        ...
        if latency_ms <= profile.max_latency_ms:
            ...
            degradation_factor = (latency_ms - profile.typical_latency_ms) / (profile.max_latency_ms - profile.typical_latency_ms)
            return max(0.1, 1.0 - degradation_factor * 0.8)  # Never go below 0.1

        # Poor quality if beyond max latency
        return 0.1

    def _update_sensor_health(self, sensor_type: SensorType, latency_ms: float,
                            quality_score: float, timestamp_ns: int):
        ...
        health['quality_score'] = alpha * quality_score + (1 - alpha) * health['quality_score']
    ...
    def calibrate_sensor_latency(self, sensor_type: SensorType, measured_latency_ms: float):
        """
        ...
        with self._lock:
            ...
            current_calibration = self._latency_calibration.get(sensor_type,
                self.SENSOR_LATENCIES[sensor_type].typical_latency_ms)

            new_calibration = alpha * measured_latency_ms + (1 - alpha) * current_calibration
            self._latency_calibration[sensor_type] = new_calibration

            logger.info(f"Calibrated {sensor_type.value} latency to {new_calibration:.1f}ms")
    ...
    def get_sensor_health(self, sensor_type: SensorType) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if health['latency_measurements']:
                health['avg_latency_ms'] = sum(health['latency_measurements']) / len(health['latency_measurements'])
            ...
            health['time_since_update_ms'] = (time.time_ns() - health['last_update_ns']) / 1_000_000

            return health

    def get_all_sensor_health(self) -> Dict[SensorType, Dict[str, Any]]:
        """Get health status for all sensors."""
        return {sensor_type: self.get_sensor_health(sensor_type) for sensor_type in SensorType}
    ...
    def validate_timestamp_synchronization(self, sensor_data_list: List[TimestampedSensorData],
                                         max_time_diff_ms: float = 50.0) -> Dict[str, Any]:
        """
    ...
    def export_latency_calibration(self) -> Dict[str, float]:
        ...
        return {sensor_type.value: latency_ms for sensor_type, latency_ms in self._latency_calibration.items()}
...
def tag_sensor_data(sensor_type: SensorType, raw_data: Dict[str, Any]) -> TimestampedSensorData:
