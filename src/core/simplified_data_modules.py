...
class TelemetryProcessor:
    ...
    def get_average_value(self, field: str, count: int = 100) -> Optional[float]:
        ...
        values = [d.get(field) for d in recent_data if isinstance(d.get(field), (int, float))]
...
def validate_mission_data(mission: Dict[str, Any]) -> ValidationResult:
    ...
    if timeout and (not isinstance(timeout, (int, float)) or timeout <= 0 or timeout > 3600):
