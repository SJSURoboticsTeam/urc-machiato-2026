...
class ComponentRegistry:
    ...
    def __init__(self):
        self.components = {
            ...
            'emergency_stop': ComponentResources(1.0, 10.0, 0.1, ResourcePriority.CRITICAL, True),
            'safety_monitor': ComponentResources(2.0, 20.0, 0.2, ResourcePriority.CRITICAL, True),
            'collision_detection': ComponentResources(5.0, 50.0, 0.5, ResourcePriority.CRITICAL, True),
            
            # Core control (high priority)
            'motor_control': ComponentResources(3.0, 30.0, 0.3, ResourcePriority.HIGH, True),
            'state_estimation': ComponentResources(8.0, 80.0, 0.8, ResourcePriority.HIGH, True),
            'basic_navigation': ComponentResources(5.0, 40.0, 0.6, ResourcePriority.HIGH, True),
            
            # Perception systems (medium priority)
            'sensor_fusion': ComponentResources(10.0, 100.0, 1.0, ResourcePriority.MEDIUM, False),
            'obstacle_detection': ComponentResources(15.0, 120.0, 1.5, ResourcePriority.MEDIUM, False),
            'visual_odometry': ComponentResources(20.0, 150.0, 2.0, ResourcePriority.MEDIUM, False),
            
            # Advanced features (low priority)
            'slam': ComponentResources(25.0, 200.0, 2.5, ResourcePriority.LOW, False),
            'object_recognition': ComponentResources(30.0, 250.0, 3.0, ResourcePriority.LOW, False),
            'path_planning': ComponentResources(15.0, 100.0, 1.0, ResourcePriority.LOW, False),
            
            # Optional features (optional priority)
            'science_analysis': ComponentResources(20.0, 180.0, 2.0, ResourcePriority.OPTIONAL, False),
            'detailed_logging': ComponentResources(5.0, 50.0, 0.5, ResourcePriority.OPTIONAL, False),
            'telemetry_streaming': ComponentResources(8.0, 80.0, 1.2, ResourcePriority.OPTIONAL, False),
        }
        
        self.active_components = set(self.components.keys())
        self.disabled_components = set()
    
    def get_component_resources(self, component_id: str) -> Optional[ComponentResources]:
...
class ResourceManager:
    ...
    def _get_system_resources(self) -> SystemResources:
        ...
        try:
            ...
            network_usage = min((network.bytes_sent + network.bytes_recv) / 1024 / 1024, 100.0)  # MB
    ...
    def _apply_high_resource_management(self, conditions: List[str]):
        ...
        self.component_registry.disable_components_by_priority(ResourcePriority.OPTIONAL)
...
class OperationModeManager:
    ...
    def start_mode_management(self):
        ...
        self.mode_thread = threading.Thread(target=self._mode_management_loop, daemon=True)
    ...
    def _evaluate_mode_switching(self):
        ...
        if recommended_mode != self.current_mode:
            ...
            if time_in_current_mode >= self.stability_requirement:
                ...
                logger.debug(f"Mode switch delayed for stability: {time_in_current_mode:.1f}s")
    ...
    def _determine_recommended_mode(self, resources: SystemResources) -> OperationMode:
        ...
        if resources.cpu_usage > 95 or resources.memory_usage > 98 or resources.temperature > 90:
            return OperationMode.EMERGENCY_SHUTDOWN
        
        # Check for critical conditions
        if resources.cpu_usage > 85 or resources.memory_usage > 90 or resources.temperature > 80:
            return OperationMode.SAFE_STOP
        
        # Check for high resource usage
        if resources.cpu_usage > 75 or resources.memory_usage > 80 or resources.temperature > 75:
    ...
    def _apply_mode_resource_allocation(self, mode: OperationMode):
        ...
        if mode == OperationMode.FULL_COMPETITION:
            ...
            self.resource_manager.component_registry.enable_components_by_priority(ResourcePriority.OPTIONAL)
        
        elif mode == OperationMode.DEGRADED_AUTONOMY:
            # Disable optional and low priority components
            self.resource_manager.component_registry.disable_components_by_priority(ResourcePriority.LOW)
            self.resource_manager.component_registry.enable_components_by_priority(ResourcePriority.MEDIUM)
        
        elif mode == OperationMode.SURVIVAL_MODE:
            # Keep only critical and high priority components
            self.resource_manager.component_registry.disable_components_by_priority(ResourcePriority.MEDIUM)
            self.resource_manager.component_registry.enable_components_by_priority(ResourcePriority.HIGH)
        
        elif mode == OperationMode.SAFE_STOP:
            # Keep only critical components
            self.resource_manager.component_registry.disable_components_by_priority(ResourcePriority.HIGH)
            self.resource_manager.component_registry.enable_components_by_priority(ResourcePriority.CRITICAL)
        
        elif mode == OperationMode.EMERGENCY_SHUTDOWN:
            # Keep only emergency stop and safety monitor
            for component in list(self.resource_manager.component_registry.active_components):
                if component not in ['emergency_stop', 'safety_monitor']:
                    self.resource_manager.component_registry.active_components.remove(component)
                    self.resource_manager.component_registry.disabled_components.add(component)
    ...
    def set_mission_context(self, phase: str, time_remaining: float, score: float = 0):
        ...
        logger.info(f"Mission context: phase={phase}, time_remaining={time_remaining:.1f}s, score={score}")
...
# Factory function
def create_operation_mode_manager(config: Dict[str, Any] = None) -> OperationModeManager:
