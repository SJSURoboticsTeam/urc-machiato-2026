...
class ObservabilitySystem:
    ...
    def __init__(
        self,
        service_name: str = "urc_2026_rover",
    ...
    def _init_metrics(self):
        ...
        self.active_connections = Gauge(
            "urc_active_connections", "Number of active connections"
        )
        ...
        self.request_duration = Histogram(
            "urc_request_duration_seconds",
            "Request duration in seconds",
            ["method", "endpoint"],
        )

        # ROS2 specific metrics
        self.ros2_topics_active = Gauge(
            "urc_ros2_topics_active", "Number of active ROS2 topics"
        )
        self.ros2_nodes_active = Gauge(
            "urc_ros2_nodes_active", "Number of active ROS2 nodes"
        )
        self.ros2_message_rate = Counter(
            "urc_ros2_messages_total", "Total ROS2 messages published", ["topic"]
        )

        # Mission metrics
        self.mission_duration = Histogram(
            "urc_mission_duration_seconds",
            "Mission duration in seconds",
            ["mission_type"],
        )
        self.mission_success = Counter(
            "urc_mission_success_total",
            "Mission success/failure count",
            ["mission_type", "result"],
        )

        # Safety metrics
        self.safety_violations = Counter(
            "urc_safety_violations_total",
            "Safety violations detected",
            ["violation_type"],
        )
        self.emergency_stops = Counter(
            "urc_emergency_stops_total", "Emergency stops triggered"
        )
    ...
    def _check_system_resources(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                status=status, message=message, timestamp=time.time(), details=details
            )
        except Exception as e:
            return HealthCheckResult(
    ...
    def record_mission_end(self, mission_type: str, success: bool, duration: float):
        ...
        if self.enable_tracing and self.tracer:
            with self.tracer.start_as_current_span(
                f"mission_complete_{mission_type}"
            ) as span:
                span.set_attribute("mission.type", mission_type)
