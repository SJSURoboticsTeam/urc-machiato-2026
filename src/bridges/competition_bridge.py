...
class CompetitionBridge(Node):
    ...
    def __init__(self):
        ...
        self.emergency_communicator = EmergencyCommunicator(
            self, self.logger, self.telemetry_manager.telemetry_data
        )
        ...
        self.websocket_port = self.parameter_manager.get_parameter(
            "websocket_port", DEFAULT_WEBSOCKET_PORT
        )
        self.telemetry_rate = self.parameter_manager.get_parameter(
            "telemetry_rate_hz", DEFAULT_TELEMETRY_RATE_HZ
        )
        self.max_clients = self.parameter_manager.get_parameter(
            "max_websocket_clients", 10
        )
        self.redundancy_role = self.parameter_manager.get_parameter(
            "redundancy_role", "primary"
        )
        self.primary_domain_id = self.parameter_manager.get_parameter(
            "primary_domain_id", DEFAULT_DDS_DOMAIN_ID
        )
        self.log_file = self.parameter_manager.get_parameter(
            "competition_log_file", DEFAULT_COMPETITION_LOG_FILE
        )
        self.enable_logging = self.parameter_manager.get_parameter(
        ...
        self.logger.info(
            "Competition Bridge initialized with extracted manager classes"
        )
    ...
    def _initialize_websocket_manager(self) -> None:
        ...
        self.websocket_manager.register_command_handler(
            "start_mission", self._handle_start_mission
        )
        self.websocket_manager.register_command_handler(
            "stop_mission", self._handle_stop_mission
        )
        self.websocket_manager.register_command_handler(
            "emergency_stop", self._handle_emergency_stop
        )
        self.websocket_manager.register_command_handler(
            "teleoperation_enable", self._handle_teleoperation_enable
        )
        self.websocket_manager.register_command_handler(
            "teleoperation_disable", self._handle_teleoperation_disable
        )
        self.websocket_manager.register_command_handler(
            "start_autonomous_navigation", self._handle_start_autonomous_navigation
        )
        self.websocket_manager.register_command_handler(
            "target_reached", self._handle_target_reached
        )
        self.websocket_manager.register_command_handler(
            "abort_to_previous", self._handle_abort_to_previous
        )
        self.websocket_manager.register_command_handler(
            "highlight_object", self._handle_highlight_object
        )
        self.websocket_manager.register_command_handler(
            "equipment_servicing_command", self._handle_equipment_command
        )
        self.websocket_manager.register_command_handler(
            "get_compliance_status", self._handle_get_compliance_status
        )
    ...
    def _sample_status_callback(self, msg: String) -> None:
        ...
        if status and status.get("mission") == "sample_collection":
            ...
            if (
                self.telemetry_manager.telemetry_data.get("current_mission")
                != "keyboard_typing"
            ):
                status_str = status.get("state", "unknown")
                self.telemetry_manager.update_mission_status(
                    "sample_collection", status_str
                )
    ...
    async def _handle_start_autonomous_navigation(
        self, websocket, targets: Dict[str, Any]
    ) -> None:
    ...
    async def _handle_target_reached(
        self, websocket, target_type: str, target_index: int
    ) -> None:
    ...
    async def _handle_highlight_object(
        self, websocket, object_name: str, confidence: float, bounding_box: List[float]
    ) -> None:
        """Handle highlight object command."""
        self.mission_orchestrator.highlight_object(
            object_name, confidence, bounding_box
        )

    async def _handle_equipment_command(
        self, websocket, subcommand: str, **kwargs
    ) -> None:
