...
class DashboardSimulationBridge:
    ...
    def __init__(
        ...
        self.get_logger().info(
            f"Dashboard Simulation Bridge initialized on port {websocket_port}"
        )

    def start_simulation(self) -> bool:
        """Start the simulation framework."""
        try:
            self.get_logger().info("Initializing simulation framework...")
            if not self.simulation_manager.initialize(self.simulation_config):
                self.get_logger().info("Failed to initialize simulation")
                return False
            ...
            self.get_logger().info("[SUCCESS] Simulation started successfully")
            return True

        except Exception as e:
            self.get_logger().info(f"[ERROR] Failed to start simulation: {e}")
            return False
    ...
    def stop_simulation(self):
        ...
        self.get_logger().info("[SUCCESS] Simulation stopped")
    ...
    def _simulation_loop(self):
        ...
        while self.is_running:
            try:
                ...
                self.get_logger().info(f"Simulation loop error: {e}")
    ...
    def _broadcast_simulation_data(self):
        ...
        for websocket in self.connected_clients:
            try:
                ...
                self.get_logger().info(f"Failed to send data to client: {e}")
    ...
    async def handle_websocket_connection(self, websocket, path):
        ...
        self.get_logger().info(
            f"New dashboard client connected from {websocket.remote_address}"
        )
        ...
        try:
            ...
            async for message in websocket:
                try:
                    ...
                    self.get_logger().info(f"Invalid JSON received: {message}")
                except Exception as e:
                    self.get_logger().info(f"Error handling dashboard message: {e}")
        except Exception as e:
            self.get_logger().info(f"WebSocket connection error: {e}")
        finally:
    ...
    async def handle_dashboard_message(self, websocket, data: Dict[str, Any]):
        ...
        if message_type == "request_state":
            ...
            self.get_logger().info(f"Unknown message type: {message_type}")
    ...
    async def run_websocket_server(self):
        ...
        self.get_logger().info(
            f"[START] Dashboard Simulation Bridge WebSocket server started on ws://0.0.0.0:{self.websocket_port}"
        )
        self.get_logger().info("Connect your dashboard to see real simulation data!")
    ...
    def run(self):
        ...
        if not self.start_simulation():
            self.get_logger().info("[ERROR] Failed to start simulation. Exiting.")
            return
