...
class MapDataBridge(Node):
    ...
    def __init__(self):
        ...
        self.map_data_pub = self.create_publisher(
            String, "/frontend/map_data", qos_reliable
        )
        self.map_metadata_pub = self.create_publisher(
            String, "/frontend/map_metadata", qos_reliable
        )
        self.robot_pose_pub = self.create_publisher(
            String, "/frontend/robot_pose", qos_reliable
        )
        self.map_status_pub = self.create_publisher(
            String, "/frontend/map_status", qos_reliable
        )
        ...
        self.map_update_timer = self.create_timer(
            self.update_interval, self.publish_map_update
        )
    ...
    def map_callback(self, msg: OccupancyGrid):
        ...
        self.get_logger().debug(
            f"Received map update: {msg.info.width}x{msg.info.height}"
        )
    ...
    def prepare_map_data(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            data = np.array(self.current_map.data, dtype=np.int8).reshape(
                (height, width)
            )
            ...
            if len(compressed_data) > self.max_map_size:
                self.get_logger().warning(
                    f"Compressed map too large: {len(compressed_data)} bytes"
                )
                return None
