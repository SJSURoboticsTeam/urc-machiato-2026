...
class ResilientWebSocketClient:
    ...
    def __init__(self, endpoints: List[Dict[str, Any]]):
        ...
        logger.info(
            f"Resilient WebSocket client initialized with {len(endpoints)} endpoints"
        )
    ...
    async def _connect_to_best_endpoint(self):
        ...
        logger.info(
            f"Attempting connection to {best_endpoint.name} ({best_endpoint.url})"
        )
        ...
        try:
            ...
            self.websocket = await asyncio.wait_for(
                websockets.connect(best_endpoint.url), timeout=self.connection_timeout
            )
        ...
        for endpoint in self.endpoints:
            try:
                ...
                websocket = await asyncio.wait_for(
                    websockets.connect(endpoint.url), timeout=1.0
                )
    ...
    def _select_best_endpoint(
        self, available_endpoints: List[EndpointInfo]
    ) -> EndpointInfo:
    ...
    def _monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                if (
                    self.connection_state == ConnectionState.CONNECTED
                    and time.time() - self.last_message_time > self.message_timeout
                ):
    ...
    def _log_connection_event(
        self, event_type: str, endpoint: EndpointInfo, details: str = ""
    ):
    ...
    def _log_status(self):
        ...
        status = {
            ...
            "uptime": time.time()
            - (
                self.connection_history[0]["timestamp"]
    ...
    def get_test_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "message_statistics": {
                "messages_received": self.messages_received,
                "messages_sent": self.messages_sent,
                "average_message_rate": self.messages_received
                / max(
                    1,
                    time.time()
                    - (
            ...
            "endpoint_performance": {
                ep.name: {
                    ...
                    "success_rate": ep.connection_count
                    / max(1, ep.connection_count + ep.failure_count)
                    * 100,
                }
            ...
            "overall_reliability": len(
                [e for e in self.connection_history if e["event"] == "connected"]
            )
            / max(1, len(self.connection_history))
            * 100,
        }
...
class WebSocketRedundancyTester:
    ...
    def _measure_current_performance(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "connection_stable": self.client.connection_state
            == ConnectionState.CONNECTED,
            "current_endpoint": self.client.current_endpoint.name
            if self.client.current_endpoint
            else None,
            "failover_count": self.client.failover_count,
        }

    def _test_endpoint_failure(
        self, endpoint_name: str, failure_duration: int
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "new_endpoint": self.client.current_endpoint.name
            if self.client.current_endpoint
            else None,
            "recovery_time": time.time() - start_time,
            "messages_during_failure": len(
                [
                    r
    ...
    def _analyze_test_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if failover_test.get("failover_triggered"):
            ...
            analysis["data_loss_estimate"] = (
                analysis["average_failover_time"] * 10
            )  # Estimate data loss

        # Generate recommendations
        if analysis["average_failover_time"] > 2.0:
            analysis["recommendations"].append(
                "Consider optimizing failover detection algorithm"
            )

        if analysis["data_loss_estimate"] > 1.0:
            analysis["recommendations"].append(
                "Implement message buffering during failovers"
            )

        if analysis["overall_reliability"] < 99.0:
            analysis["recommendations"].append(
                "Add more redundant endpoints for higher availability"
            )
...
def main():
    ...
    parser.add_argument(
        "--simulate-failures", action="store_true", help="Simulate endpoint failures"
    )
    ...
    logger.info(
        f"Failure simulation: {'enabled' if args.simulate_failures else 'disabled'}"
    )
    ...
    print(f"\nClient Performance:")
    print(
        f"  Messages Received: {client_metrics['message_statistics']['messages_received']}"
    )
    print(f"  Messages Sent: {client_metrics['message_statistics']['messages_sent']}")
    print(
        f"  Average Message Rate: {client_metrics['message_statistics']['average_message_rate']:.1f} msg/s"
    )

    print(f"\nEndpoint Performance:")
    for ep_name, stats in client_metrics["endpoint_performance"].items():
        print(
            f"  {ep_name}: {stats['success_rate']:.1f}% success rate ({stats['connection_attempts']} attempts)"
        )
