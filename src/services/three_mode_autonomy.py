...
class SafetySystem:
    ...
    def _detect_obstacles(self):
        ...
        for i in range(3):  # Simulate up to 3 obstacles
            if np.random.random() < 0.3:  # 30% chance of obstacle
                ...
                obstacle_type = (
                    ObstacleType.HARD_SURFACE
                    if np.random.random() < 0.7
                    else ObstacleType.LIVING_SURFACE
                )
        ...
        self.obstacles = [
            obs for obs in self.obstacles if current_time - obs.timestamp < 10.0
        ...
        if obstacles:
            self.logger.warning(
                ...
                extra={
                    "closest_distance": (
                        obstacles[0].distance if obstacles else float("inf")
                    ),
    ...
    def _assess_emergency_conditions(self):
        ...
        if self.imu_data is not None:
            tilt_angle = abs(self.imu_data.orientation.x) + abs(
...
class AutonomyController:
    ...
    def __init__(self, autonomy_mode: AutonomyMode = AutonomyMode.HYBRID_DEFAULT):
        ...
        self.logger.info(
            f"Autonomy controller initialized in {autonomy_mode.value} mode"
        )
    ...
    def enable_safety_features(self, enabled: bool) -> bool:
        ...
        if (
            self.autonomy_mode == AutonomyMode.FULLY_OPERATOR
            and self.config.operator_override_enabled
        ):
    ...
    def _process_autonomous_mode(
        ...
        if safety_status["obstacle_count"] > 0:
            closest_obstacle = safety_status["obstacles"][
                0
            ...
            if closest_obstacle["distance"] < 3.0:  # Too close
                ...
                safe_speed = min(
                    velocity_magnitude * 0.5, closest_obstacle["distance"] - 1.0
                )
    ...
    def _process_operator_mode(self, position: np.ndarray, velocity: np.ndarray):
        ...
        ] += 1  # Track operator decisions as well
    ...
    def _suggest_alternative_action(self):
        ...
        self.logger.warning(
            ...
            extra={"safety_intervention": True, "obstacle_avoidance": True},
        )
...
class ThreeModeAutonomyNode(Node):
    ...
    def __init__(self):
        ...
        self.mode_service = self.create_service(
            SetAutonomyMode, "/autonomy/set_mode", self._set_mode_callback
        )
        ...
        self.status_timer = self.create_timer(
            1.0, self._publish_status  # 10Hz status publishing
        )
    ...
    def _set_mode_callback(self, request, response):
        ...
        try:
            ...
            response.message = (
                f"Mode set to {mode.value}" if success else "Mode change failed"
            )
    ...
    def _enable_safety_callback(self, request, response):
        ...
        try:
            ...
            validate_input(enabled, schema={"type": "boolean", "name": "enabled"})
