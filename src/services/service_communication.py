...
class ServiceCommunicationBus:
    ...
    async def _process_queue(self, priority: MessagePriority) -> int:
        ...
        processed = 0
        timeout = 0.001 if priority == MessagePriority.CRITICAL else 0.005  # Faster for critical
        ...
        while queue:
            ...
            try:
                ...
                if priority == MessagePriority.CRITICAL and self.shared_memory.shared_memory is not None:
    ...
    def _try_zero_copy_send(self, message: ServiceMessage) -> bool:
        ...
        try:
            ...
            if isinstance(message.data, (str, bytes)):
                message_bytes = message.data.encode() if isinstance(message.data, str) else message.data
            else:
            ...
            offset = hash(message.service_id) % (self.shared_memory.size_bytes - 1024)  # Avoid header
    ...
    def _update_service_health(self, service_id: str, response_time_ms: float):
        ...
        if service_id in self.service_health:
            ...
            health.response_time_ms = (
                (health.response_time_ms * 0.8) + (response_time_ms * 0.2)
            )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        total_capacity = sum(
            len(self.queues[priority]) for priority in MessagePriority
        )
        stats['queue_utilization'] = total_capacity / (len(MessagePriority) * 1000)
        
        # Calculate throughput
        if stats['messages_sent'] > 0:
            stats['throughput_msg_per_sec'] = stats['messages_sent'] / (time.time() - getattr(self, 'start_time', time.time()))
...
class ServiceCommunicationNode(Node):
    ...
    def __init__(self):
        ...
        self.status_publisher = self.create_publisher(
            String,
            '/service_communication/status',
            10  # 10Hz status publishing
        )
        
        self.command_subscriber = self.create_subscription(
            String,
            '/service_communication/command',
            self._command_callback,
            10
        )
        
        self.performance_timer = self.create_timer(
            1.0,  # 1Hz performance publishing
            ...
            if command_type == 'send_message':
                ...
                if message_data:
                    ...
                    async def send_message():
                        ...
                        return {'success': success, 'message_id': message_data.get('id')}
                    ...
                    self.get_logger().info(
                        ...
                        extra={'result': result.get('success'), 'message_id': message_data.get('id')}
                    )
                ...
                if service_id:
                    ...
                    self.get_logger().info(
                        ...
                        extra={'service_id': service_id, 'status': health.status if health else 'not_found'}
                    )
...
if __name__ == "__main__":
    ...
    # Register handlers
    bus.register_message_handler("state_machine.set_mode", lambda msg: f"Mode set to {msg.data}")
    bus.register_message_handler("behavior_tree.execute", lambda msg: f"Executing {msg.data}")
    bus.register_message_handler("hardware_interface.command", lambda msg: f"Hardware command: {msg.data}")
