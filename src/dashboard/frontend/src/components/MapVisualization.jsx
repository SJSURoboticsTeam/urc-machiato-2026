...
const ROSLIB = {
  Ros: class MockRos {
    constructor(config) {
      ...
      console.log(`Mock ROS connection to ${this.url}`);
    }
    connect() {
      this.isConnected = true;
      console.log('Mock ROS connected');
    }
    close() {
      this.isConnected = false;
      console.log('Mock ROS disconnected');
    }
  },
  Topic: class MockTopic {
    constructor(config) {
      this.name = config?.name || 'unknown';
      console.log(`Mock ROS topic: ${this.name}`);
    }
    subscribe(callback) {
      console.log(`Mock subscription to ${this.name}`);
      if (callback) {
        setTimeout(() => callback({data: JSON.stringify({message: 'mock_data'})}), 1000);
      }
    }
    publish(message) {
      console.log(`Mock publish to ${this.name}:`, message);
    }
    unsubscribe() {
      console.log(`Mock unsubscribe from ${this.name}`);
    }
  }
};
