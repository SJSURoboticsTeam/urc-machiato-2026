...
class StandaloneStateMachine(Node):
    ...
    def __init__(self):
        ...
        self.change_state_srv = self.create_service(
            Trigger, "/state_machine/change_state", self.handle_change_state
        )

        self.get_current_state_srv = self.create_service(
            Trigger, "/state_machine/get_current_state", self.handle_get_current_state
        )
        ...
        try:
            ...
            valid_transitions = {
                ...
                "IDLE": [
                    ...
                    "AUTONOMOUS",
                    "SAFETY",
                ],  # Allow direct access to critical states
                ...
                "SAFETY": [
                    ...
                    "TELEOPERATION",
                    "AUTONOMOUS",
                ],  # Allow staying in SAFETY or transitioning out
                "SHUTDOWN": ["BOOT"],
            }
            ...
            if self.current_state in valid_transitions:
                ...
                if len(possible_states) > 1:
                    ...
                    self.transition_counters[state_key] = (current_idx + 1) % len(
                        possible_states
                    )
                else:
                ...
                current_index = (
                    state_sequence.index(self.current_state)
                    if self.current_state in state_sequence
                    else 0
                )
    ...
    def publish_state(self):
        ...
        self.substate_pub.publish(
            String(
                data=json.dumps(
                    {"substate": self.current_substate, "timestamp": time.time()}
                )
            )
        )
