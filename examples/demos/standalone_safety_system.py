...
class EnhancedStandaloneSafetySystem(Node):
    ...
    def __init__(self):
        ...
        self.watchdog_status_pub = self.create_publisher(
            String, "/safety/watchdog_status", 10
        )
        self.sensor_health_pub = self.create_publisher(
            String, "/safety/sensor_health", 10
        )
        ...
        self.emergency_stop_srv = self.create_service(
            Trigger, "/safety/emergency_stop", self.handle_emergency_stop
        )

        self.recover_from_safety_srv = self.create_service(
            Trigger, "/safety/recover_from_safety", self.handle_recover_from_safety
        )

        self.run_diagnostics_srv = self.create_service(
            Trigger, "/safety/run_diagnostics", self.handle_run_diagnostics
        )

        self.watchdog_monitoring_srv = self.create_service(
            Trigger, "/safety/watchdog_monitoring", self.handle_watchdog_monitoring
        )

        self.sensor_health_check_srv = self.create_service(
            Trigger, "/safety/sensor_health_check", self.handle_sensor_health_check
        )
    ...
    def handle_recover_from_safety(self, request, response):
        ...
        if recovery_success:
            ...
            response.success = True
            response.message = (
                f"Safety recovery successful (attempt {self.recovery_attempts})"
            )
    ...
    def handle_watchdog_monitoring(self, request, response):
        ...
        watchdog_status = {
            ...
            "alerts_detected": len(
                [a for a in self.active_alerts if a.get("source") == "watchdog"]
            ),
        }
    ...
    def publish_watchdog_status(self):
        ...
        watchdog_data = {
            ...
            "alerts_since_last_check": len(
                [
                    a
                    for a in self.active_alerts
                    if a.get("timestamp", 0) > time.time() - 3
                ]
            ),
        }
