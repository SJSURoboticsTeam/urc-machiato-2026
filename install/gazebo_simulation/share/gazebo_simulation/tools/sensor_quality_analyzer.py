...
class SensorQualityAnalyzer(Node):
    ...
    def analysis_loop(self):
        ...
        if (
            current_time - self.start_time >= self.analysis_duration
    ...
    def analyze_imu(self):
        ...
        data_completeness = (
            len(self.imu_data) / expected_samples if expected_samples > 0 else 0.0
        )
        ...
        recommendations = self.generate_imu_recommendations(
            characteristics, fidelity_score
        )
        ...
        self.get_logger().info(
            f"IMU Analysis: Fidelity {fidelity_score:.2f}, Grade {report.quality_grade}"
        )
    ...
    def analyze_gps(self):
        ...
        positions = np.array(
            [[d["latitude"], d["longitude"], d["altitude"]] for d in self.gps_data]
        )
        ...
        expected_samples = update_rate * self.analysis_duration
        data_completeness = (
            len(self.gps_data) / expected_samples if expected_samples > 0 else 0.0
        )
        ...
        # Generate recommendations
        recommendations = self.generate_gps_recommendations(
            characteristics, fidelity_score
        )
        ...
        self.get_logger().info(
            f"GPS Analysis: Fidelity {fidelity_score:.2f}, Grade {report.quality_grade}"
        )
    ...
    def analyze_lidar(self):
        ...
        angular_resolution = (
            np.mean(angle_increments) * 180.0 / math.pi
        )  # Convert to degrees
        ...
        expected_samples = update_rate * self.analysis_duration
        data_completeness = (
            len(self.lidar_data) / expected_samples if expected_samples > 0 else 0.0
        )
        ...
        # Generate recommendations
        recommendations = self.generate_lidar_recommendations(
            characteristics, fidelity_score
        )
        ...
        self.get_logger().info(
            f"LiDAR Analysis: Fidelity {fidelity_score:.2f}, Grade {report.quality_grade}"
        )
    ...
    def analyze_camera(self):
        ...
        expected_samples = update_rate * self.analysis_duration
        data_completeness = (
            len(self.camera_data) / expected_samples if expected_samples > 0 else 0.0
        )

        # Create characteristics
        characteristics = SensorCharacteristics(
            update_rate=update_rate, data_completeness=min(data_completeness, 1.0)
        )

        # Calculate fidelity score
        fidelity_score = self.calculate_camera_fidelity(
            characteristics, avg_width, avg_height
        )

        # Generate recommendations
        recommendations = self.generate_camera_recommendations(
            characteristics, fidelity_score
        )
        ...
        self.get_logger().info(
            f"Camera Analysis: Fidelity {fidelity_score:.2f}, Grade {report.quality_grade}"
        )
    ...
    def analyze_depth_camera(self):
        ...
        expected_samples = update_rate * self.analysis_duration
        data_completeness = (
            len(self.depth_data) / expected_samples if expected_samples > 0 else 0.0
        )
        ...
        # Calculate fidelity score
        fidelity_score = self.calculate_depth_camera_fidelity(
            characteristics, depth_quality
        )

        # Generate recommendations
        recommendations = self.generate_depth_camera_recommendations(
            characteristics, fidelity_score
        )
        ...
        self.get_logger().info(
            f"Depth Camera Analysis: Fidelity {fidelity_score:.2f}, Grade {report.quality_grade}"
        )
    ...
    def calculate_lidar_fidelity(self, characteristics: SensorCharacteristics) -> float:
        ...
        resolution_ratio = expected_resolution / max(
            characteristics.angular_resolution, 0.001
        )
    ...
    def calculate_camera_fidelity(
        self, characteristics: SensorCharacteristics, width: float, height: float
    ) -> float:
    ...
    def calculate_depth_camera_fidelity(
        self, characteristics: SensorCharacteristics, depth_quality: float
    ) -> float:
    ...
    def generate_imu_recommendations(
        ...
        if (
            characteristics.update_rate
            < self.real_world_specs["imu"]["update_rate"] * 0.8
        ):
            recommendations.append(
                "Increase IMU update rate to match real-world specifications"
            )

        if (
            characteristics.noise_level
            > self.real_world_specs["imu"]["gyro_noise_density"] * 2
        ):
            recommendations.append(
                "Reduce IMU noise levels to better match real sensors"
            )

        if characteristics.data_completeness < 0.9:
            recommendations.append(
                "Improve IMU data completeness - check for dropped messages"
            )

        if (
            characteristics.bias_drift
            > self.real_world_specs["imu"]["gyro_bias_stability"] * 2
        ):
            recommendations.append(
                "Reduce IMU bias drift to match real-world stability"
            )

        if fidelity_score < 0.7:
            recommendations.append(
                "Overall IMU simulation needs significant improvement for realistic testing"
            )

        return recommendations

    def generate_gps_recommendations(
        self, characteristics: SensorCharacteristics, fidelity_score: float
    ) -> List[str]:
        ...
        if (
            characteristics.update_rate
            < self.real_world_specs["gps"]["update_rate"] * 0.5
        ):
            recommendations.append(
                "Increase GPS update rate to match real-world specifications"
            )

        if (
            characteristics.range_accuracy
            < self.real_world_specs["gps"]["position_accuracy"] * 0.5
        ):
            recommendations.append("GPS accuracy is too good - add realistic noise")
        elif (
            characteristics.range_accuracy
            > self.real_world_specs["gps"]["position_accuracy"] * 2
        ):
    ...
    def generate_lidar_recommendations(
        ...
        if (
            characteristics.update_rate
            < self.real_world_specs["lidar"]["update_rate"] * 0.8
        ):
            recommendations.append(
                "Increase LiDAR update rate to match real-world specifications"
            )

        if (
            characteristics.range_accuracy
            > self.real_world_specs["lidar"]["range_accuracy"] * 2
        ):
            recommendations.append("Improve LiDAR range accuracy")

        if (
            characteristics.angular_resolution
            > self.real_world_specs["lidar"]["angular_resolution"] * 2
        ):
    ...
    def generate_camera_recommendations(
        ...
        if (
            characteristics.update_rate
            < self.real_world_specs["camera"]["update_rate"] * 0.8
        ):
            recommendations.append(
                "Increase camera update rate to match real-world specifications"
            )

        if fidelity_score < 0.7:
            recommendations.append(
                "Camera simulation needs improvement for realistic testing"
            )
    ...
    def generate_depth_camera_recommendations(
        ...
        if (
            characteristics.range_accuracy
            > self.real_world_specs["camera"]["depth_accuracy"] * 2
        ):
    ...
    def generate_overall_report(self):
        ...
        fidelity_scores = [
            report.fidelity_score for report in self.sensor_reports.values()
        ]
        ...
        overall_report = {
            ...
            "sensor_reports": {
                name: asdict(report) for name, report in self.sensor_reports.items()
            },
    ...
    def print_summary(self, report: Dict[str, Any]):
        ...
        self.get_logger().info("SENSOR QUALITY ANALYSIS SUMMARY")

        self.get_logger().info(
            f"Overall Fidelity Score: {report['overall_fidelity_score']:.2f}"
        )
        self.get_logger().info(f"Overall Grade: {report['overall_grade']}")
        self.get_logger().info(f"Test Duration: {report['test_duration']:.1f} seconds")
        self.get_logger().info(f"Sensors Analyzed: {report['sensor_count']}")
        self.get_logger().info("\nSensor Details:")

        for name, sensor_report in report["sensor_reports"].items():
            pass  # Placeholder for sensor details
        ...
        if len(report["overall_recommendations"]) > 5:
            self.get_logger().info(
                f"... and {len(report['overall_recommendations']) - 5} more recommendations"
            )
