...
class URCChallengeSimulator(Node):
    ...
    def __init__(self):
        ...
        self.challenge_timer = self.create_timer(
            self.challenge_duration, self.next_challenge
        )
    ...
    def start_challenge(self):
        ...
        metrics = ChallengeMetrics(
            challenge_type=self.current_challenge, start_time=challenge_start
        )
        ...
        self.publish_challenge_status(
            "STARTED", f"Challenge {self.challenge_index + 1}/7"
        )
    ...
    def check_waypoint_completion(self):
        ...
        if not self.target_waypoints or self.waypoint_index >= len(
            self.target_waypoints
        ):
            return
    ...
    def publish_next_waypoint(self):
        ...
        if self.waypoint_index < len(self.target_waypoints):
            ...
            self.logger.info(
                f" Published waypoint {self.waypoint_index + 1}: ({wp[0]}, {wp[1]})"
            )
    ...
    def update_navigation(self):
        ...
        if self.current_challenge == URCChallenge.FOLLOW_ME:
            ...
            if self.target_waypoints and self.waypoint_index < len(
                self.target_waypoints
            ):
        ...
        status_msg.data = json.dumps(
            {
                ...
                "waypoints_completed": sum(
                    m.waypoints_completed for m in self.challenge_metrics
                ),
            }
        )
    ...
    def complete_current_challenge(self, success: bool = False):
        ...
        if self.challenge_metrics:
            ...
            duration = (
                self.challenge_metrics[-1].end_time
                - self.challenge_metrics[-1].start_time
            )
            self.logger.info(
    ...
    def complete_test(self):
        ...
        test_data = {
            ...
            "overall_metrics": {
                ...
                "average_challenge_duration": total_duration
                / len(self.challenge_sequence),
            },
        }
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if not recommendations:
            recommendations.append(
                "All systems performing well - ready for competition"
            )
    ...
    def print_final_results(self, result: URCChallengeResult, test_data: Dict):
        ...
        self.get_logger().info("URC 2026 COMPLETE CHALLENGE TEST RESULTS")

        self.get_logger().info(f"Test Duration: {result.duration_minutes:.1f} minutes")
        self.get_logger().info(f"Challenges Completed: {result.challenges_completed}/7")
        self.get_logger().info(f"Safety Rating: {result.safety_rating}")
        self.get_logger().info(f"Performance Rating: {result.performance_rating}")
        ...
        for i, metrics in enumerate(self.challenge_metrics):
            challenge_name = challenge_names.get(
                metrics.challenge_type, str(metrics.challenge_type)
            )
            status = "" if metrics.success else ""
            duration = metrics.end_time - metrics.start_time if metrics.end_time else 0

        self.get_logger().info("\nOVERALL METRICS:")
        self.get_logger().info(f"Obstacles Avoided: {self.obstacles_detected}")
        self.get_logger().info(
            f"Safety Violations: {sum(m.safety_violations for m in self.challenge_metrics)}"
        )
        self.get_logger().info("\nRECOMMENDATIONS:")
        ...
        self.get_logger().info("Results saved to /tmp/urc_challenge_test_results.json")
