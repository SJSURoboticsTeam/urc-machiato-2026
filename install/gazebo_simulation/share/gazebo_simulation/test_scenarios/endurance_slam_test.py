...
class EnduranceSLAMTester(Node):
    ...
    def __init__(self):
        ...
        self.test_status_pub = self.create_publisher(
            String, "/endurance_test_status", 10
        )

        # Monitoring timers
        self.monitor_timer = self.create_timer(
            self.monitoring_interval, self.monitor_system_health
        )
    ...
    def complete_test(self):
        ...
        # SLAM performance
        avg_confidence = (
            np.mean(self.confidence_readings) if self.confidence_readings else 0.8
        )
        ...
        self.get_logger().info("ENDURANCE SLAM TEST RESULTS")

        self.get_logger().info(".2f")
        self.get_logger().info(f"Total Distance: {self.total_distance:.1f} meters")
        self.get_logger().info(f"Loop Closures: {self.loop_closures}")
        self.get_logger().info(f"Pose Updates: {self.pose_update_count}")
        self.get_logger().info(f"Map Updates: {self.map_update_count}")

        self.get_logger().info("SYSTEM PERFORMANCE:")
        self.get_logger().info(".1f")
        self.get_logger().info(".1f")
        self.get_logger().info(".1f")
        self.get_logger().info(".1f")

        self.get_logger().info("SLAM PERFORMANCE:")
        self.get_logger().info(".1f")
        self.get_logger().info(f"Average Confidence: {avg_confidence:.2f}")

        self.get_logger().info("TEST VALIDATION:")
        memory_ok = memory_growth < 50.0
        cpu_ok = max_cpu < 80.0
        slam_ok = avg_confidence > 0.7
        self.get_logger().info(
            f"Memory Growth (<50MB/10min): {' PASS' if memory_ok else ' FAIL'}"
        )
        self.get_logger().info(f"CPU Usage (<80%): {' PASS' if cpu_ok else ' FAIL'}")
        self.get_logger().info(
            f"SLAM Stability (>70% conf): {' PASS' if slam_ok else ' FAIL'}"
        )
