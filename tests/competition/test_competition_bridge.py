...
class MockCompetitionBridge:
    ...
    def target_reached(self, target_type, target_index):
        ...
        self.mission_orchestrator['autonomous_navigation']['completed_targets'].append(completed_target)
...
class TestCompetitionBridge:
    ...
    def test_led_status_system(self, competition_bridge_fixture):
        ...
        assert nav_orch['current_mode'] == 'autonomous', "LED status should update navigation mode"

        # Test teleoperation mode LED - should switch to teleoperation
        bridge.update_led_status("autonomous_navigation", "teleoperation")
        assert nav_orch['current_mode'] == 'teleoperation', "LED status should switch to teleoperation"
        ...
        assert nav_orch['last_target_reached'] == True, "LED status should mark target as reached"
        assert nav_orch['current_target_index'] == initial_target_index, "Should not auto-increment on LED status alone"
...
class TestLEDCommandMessages:
    ...
    def test_led_command_serialization(self):
        ...
        assert led_cmd.pattern == "blinking", "Pattern should be blinking for target reached"
        ...
        assert led_cmd.red == 0.0 and led_cmd.green == 1.0 and led_cmd.blue == 0.0, \
        ...
        assert led_cmd.pattern == "blinking" and led_cmd.frequency > 0, \
    ...
    def test_led_command_ros2_compatibility(self):
        ...
        required_fields = ['header', 'status_code', 'red', 'green', 'blue',
                          'pattern', 'frequency', 'priority', 'duration', 'override']
...
class TestVisionDetectionMessages:
    ...
    def test_vision_detection_ros2_compatibility(self):
        ...
        required_fields = ['header', 'class_name', 'confidence', 'class_id', 'detector_type', 'track_id', 'pose', 'size']
        for field in required_fields:
