...
class AutomatedTestPlatform:
    ...
    def _create_test_suites(self) -> Dict[str, TestSuite]:
        ...
        return {
            "aoi_tests": TestSuite(
                ...
                cleanup_commands=["./scripts/system_stop.sh"],
                timeout_seconds=600,
            ...
            "full_system_tests": TestSuite(
                ...
                cleanup_commands=["./scripts/system_stop.sh", "rm -rf test_artifacts/"],
    ...
    def run_automated_test_suite(
        self, suite_name: str, output_dir: Path = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            for cmd in commands:
                self._execute_command(
                    cmd, environment, output_dir / f"{phase_name}.log"
                )
    ...
    def _run_system_validation(self, suite: TestSuite) -> Dict[str, Any]:
        ...
        for service in suite.required_services:
            try:
                result = subprocess.run(
                    ["which", service], capture_output=True, text=True
                )
    ...
    def _execute_command(
        self, command: str, environment: TestEnvironment, log_file: Path
        ...
        if process.returncode != 0:
            raise subprocess.CalledProcessError(
                process.returncode, command, output=f"See log: {log_file}"
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, help="Output directory for test artifacts"
    )
    parser.add_argument(
        "--parallel", action="store_true", help="Run test suites in parallel"
    )
    parser.add_argument(
        "--ci-mode", action="store_true", help="CI/CD mode with JSON output"
    )
