...
class TestAdvancedMotorControl:
    ...
    def test_terrain_classification(self, motor_controller):
        ...
        terrain = terrain_classifier.classify_terrain(
...
class TestGracefulDegradation:
    ...
    def mode_manager(self):
        ...
        config = {
            k: v
            for k, v in TEST_CONFIG["graceful_degradation"].items()
            if k != "sensor_fusion"
        }
        try:
            return OperationModeManager(
                config if config else TEST_CONFIG["graceful_degradation"]
            )
        except TypeError:
    ...
    def test_adaptive_mode_configuration(self, mode_manager):
        ...
        modes = [
            OperationMode.FULL_COMPETITION,
            OperationMode.DEGRADED_AUTONOMY,
...
class TestCompetitionScenarios:
    ...
    def test_emergency_stop_scenario(self):
        ...
        e_stop = HardwareEmergencyStop(
            EmergencyStopConfig(**TEST_CONFIG["emergency_stop"])
        )
