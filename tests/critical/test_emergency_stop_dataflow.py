...
try:
    ...
    pytest.skip(f"Skipping emergency stop tests due to import error: {e}", allow_module_level=True)
...
class TestEmergencyStopDataflow:
    ...
    def test_emergency_stop_sensor_to_motors_latency(
        ...
        while (time.time() - start_time < 1.0 and 
               motor_controller.emergency_received_time is None):
        ...
        assert timing.total_latency < 0.050, f"Emergency stop took {timing.total_latency*1000:.1f}ms, required < 50ms"
        assert timing.safety_latency < 0.020, f"Safety monitor took {timing.safety_latency*1000:.1f}ms, required < 20ms"
        assert timing.state_latency < 0.030, f"State machine took {timing.state_latency*1000:.1f}ms, required < 30ms"
        assert timing.motor_latency < 0.050, f"Motor control took {timing.motor_latency*1000:.1f}ms, required < 50ms"
        
        # Verify motors actually received emergency stop
        assert motor_controller.emergency_received_time is not None, "Motor controller never received emergency stop"
    ...
    def test_emergency_stop_multiple_triggers(self, ros_context):
        ...
        assert motor_controller.emergency_received_time is not None, "Emergency stop not processed with multiple triggers"
    ...
    def test_emergency_stop_system_consistency(
        ...
        safety_monitor = Mock()
        safety_monitor.emergency_stop_callback = Mock(side_effect=lambda msg: received_components.update({'safety': True}))
        
        state_machine = Mock()
        state_machine.emergency_stop_callback = Mock(side_effect=lambda msg: received_components.update({'state': True}))
        
        navigation_node = Mock()
        navigation_node.emergency_stop_callback = Mock(side_effect=lambda msg: received_components.update({'navigation': True}))
        ...
        start_time = time.time()
        while (time.time() - start_time < 1.0 and 
               not all(received_components.values())):
            rclpy.spin_once(ros_context, timeout_sec=0.01)
        ...
        assert all(received_components.values()), f"Not all components received emergency stop: {received_components}"
    ...
    def test_emergency_stop_data_integrity(self, ros_context):
        ...
        safety_monitor = Mock()
        safety_monitor.emergency_stop_callback = Mock(side_effect=lambda msg: trace_message('safety', msg))
        
        # Mock state machine with data checking
        state_machine = Mock()
        state_machine.emergency_stop_callback = Mock(side_effect=lambda msg: trace_message('state', msg))
        
        # Mock motor controller with data checking
        motor_controller = Mock()
        motor_controller.emergency_callback = Mock(side_effect=lambda msg: trace_message('motors', msg))
        ...
        for trace in message_trace:
            assert trace['data'] is True, f"Emergency stop data corrupted at {trace['component']}"
        ...
        assert timestamps == sorted(timestamps), "Messages not processed in chronological order"
        
        print(f"✅ Emergency stop data integrity verified: {len(message_trace)} components")
...
class TestEmergencyStopPerformance:
    ...
    def test_emergency_stop_load_handling(self, ros_context):
        ...
        while (time.time() - load_start < 1.0 and 
               motor_controller.emergency_received_time is None):
            for node in load_nodes:
                rclpy.spin_once(node, timeout_sec=0.001)
            rclpy.spin_once(ros_context, timeout_sec=0.001)
        
        emergency_latency = motor_controller.emergency_received_time - start_time if motor_controller.emergency_received_time else float('inf')
        ...
        assert emergency_latency < 0.100, f"Emergency stop took {emergency_latency*1000:.1f}ms under load, required < 100ms"
    ...
    def test_emergency_stop_trigger_sources(self, ros_context, trigger_type):
        ...
        def simulate_trigger(source):
            if source == "sensor":
                ...
                pub = dashboard_publisher.create_publisher(Bool, '/dashboard/emergency', 10)
                msg = Bool(data=True)
                pub.publish(msg)
