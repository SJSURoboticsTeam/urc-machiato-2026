...
class NavigationLoopMonitor:
    ...
    def record_perception_complete(
        self,
        measurement_id: str,
...
class TestNavigationControlLoop:
    ...
    def test_perception_to_motor_complete_chain(
        ...
        test_imu = {
            "linear_accel": np.array([0.1, 0.0, 9.8]),
            "angular_vel": np.array([0.0, 0.0, 0.01]),
        }
        ...
        sensor_publisher.publish_imu_data(
            test_imu["linear_accel"], test_imu["angular_vel"]
        )

        # Mock perception processing
        def mock_perception_callback():
            perception_time = time.time() - start_time
            navigation_monitor.record_perception_complete(
                test_id, test_image, test_lidar, test_imu
            )
        ...
        assert (
            timing.total_latency < 0.100
        ), f"Total navigation loop took {timing.total_latency*1000:.1f}ms, required < 100ms"
        assert (
            timing.perception_latency < 0.030
        ), f"Perception took {timing.perception_latency*1000:.1f}ms, required < 30ms"
        assert (
            timing.slam_latency < 0.025
        ), f"SLAM took {timing.slam_latency*1000:.1f}ms, required < 25ms"
        assert (
            timing.planning_latency < 0.025
        ), f"Planning took {timing.planning_latency*1000:.1f}ms, required < 25ms"
        assert (
            timing.control_latency < 0.025
        ), f"Control took {timing.control_latency*1000:.1f}ms, required < 25ms"

        # Verify motor command was received
        assert len(motor_controller.received_commands) > 0, "No motor commands received"
        assert (
            motor_controller.received_commands[-1].linear.x > 0
        ), "Invalid motor command received"
    ...
    def test_navigation_data_loss_recovery(
        ...
        navigation_monitor.record_perception_complete(
            test_id, None, degraded_lidar, None
        )
        ...
        timing = navigation_monitor.complete_measurement(test_id)
        state = (
            navigation_monitor.state_tracking[-1]
            if navigation_monitor.state_tracking
            else None
        )
        ...
        assert (
            state.motor_command is not None
        ), "Motor command should still be generated"
        assert (
            state.motor_command.linear.x < 0.5
        ), "Speed should be reduced with degraded sensors"
    ...
    def test_navigation_data_corruption_handling(
        ...
        assert (
            len(motor_controller.received_commands) >= 0
    ...
    def test_navigation_loop_under_load(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            navigation_monitor.record_perception_complete(
                test_id, test_image, test_lidar, None
            )
        ...
        assert (
            timing.total_latency < 0.200
        ), f"Navigation under load took {timing.total_latency*1000:.1f}ms, required < 200ms"
        assert (
            len(motor_controller.received_commands) > 0
        ), "Navigation should work under load"
...
class TestNavigationDataIntegrity:
    ...
    def test_coordinate_system_consistency(self):
        ...
        assert np.allclose(
            transformed_robot, robot_coords, atol=0.1
        ), "Camera to robot transformation failed"
        assert np.allclose(
            transformed_world, world_coords, atol=0.1
        ), "Robot to world transformation failed"
    ...
    def test_timestamp_synchronization(self):
        ...
        for sensor1, time1 in sensor_times.items():
            for sensor2, time2 in sensor_times.items():
                ...
                ), f"Timestamp synchronization failed: {sensor1} vs {sensor2} diff {time_diff*1000:.1f}ms"
    ...
    def test_data_precision_preservation(self):
        ...
        assert np.allclose(
            processed_position, precise_position, atol=1e-6
        ), "Position precision lost"
        assert np.allclose(
            processed_orientation, precise_orientation, atol=1e-6
        ), "Orientation precision lost"
