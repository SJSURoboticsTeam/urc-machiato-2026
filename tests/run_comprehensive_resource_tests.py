...
def run_statistical_performance_tests(iterations: int, confidence: float) -> Dict[str, Any]:
...
def generate_performance_report(all_results: Dict[str, Any], output_file: str) -> Dict[str, Any]:
    ...
    report['test_summary'] = {
        ...
        'failed_tests': sum(1 for r in test_results.values() if not r.get('success', True)),
        'overall_success_rate': sum(1 for r in test_results.values() if r.get('success', False)) / len(test_results) if test_results else 0
    }
                ...
                if 'resource_optimization_effectiveness' in test_name:
                    ...
                    if 'mean' in cpu_stats and 'mean' in memory_stats:
                        opt_effectiveness[test_name] = {
                            ...
                            'memory_reduction_potential': memory_stats.get('std_dev', 0)
                        }

            report['performance_metrics']['resource_optimization_effectiveness'] = opt_effectiveness
    ...
    if success_rate >= 0.9:
        recommendations.append("✅ Excellent: Resource optimization system is highly reliable (>90% test success)")
    elif success_rate >= 0.8:
        recommendations.append("⚠️ Good: Resource optimization system is reliable (80-90% test success)")
    else:
        recommendations.append("❌ Critical: Resource optimization system needs attention (<80% test success)")

    # Resource optimization effectiveness
    opt_metrics = report['performance_metrics']['resource_optimization_effectiveness']
    if opt_metrics:
        avg_cpu_reduction = sum(m.get('cpu_reduction_potential', 0) for m in opt_metrics.values()) / len(opt_metrics)
        if avg_cpu_reduction > 10:
            recommendations.append(f"✅ Excellent: Resource optimization provides significant CPU savings ({avg_cpu_reduction:.1f}%)")
        elif avg_cpu_reduction > 5:
            recommendations.append(f"⚠️ Moderate: Resource optimization provides some CPU savings ({avg_cpu_reduction:.1f}%)")
        else:
            recommendations.append("⚠️ Limited: Resource optimization provides minimal CPU savings (<5%)")

    # Fallback functionality
    fallback_result = test_results.get('fallback_functionality', {})
    if fallback_result.get('success'):
        recommendations.append("✅ Robust: Fallback mechanisms work correctly when dependencies are missing")
    else:
        recommendations.append("❌ Critical: Fallback mechanisms failed - system may not work without all dependencies")

    # Recovery mechanisms
    recovery_result = test_results.get('recovery_mechanisms', {})
    if recovery_result.get('success'):
        recommendations.append("✅ Reliable: Recovery mechanisms activate correctly under resource pressure")
    else:
        recommendations.append("❌ Critical: Recovery mechanisms failed - system may not handle resource pressure")
    ...
    if integration_result.get('success'):
        ...
        recommendations.append("❌ Critical: Component integration failed - system components may conflict")
...
def main():
    """Main test runner."""
    parser = argparse.ArgumentParser(description='URC 2026 Resource Optimization Test Suite')
    ...
    parser.add_argument('--skip-slow', action='store_true',
                       help='Skip slow statistical tests')
    parser.add_argument('--quick', action='store_true',
                       help='Run quick functional tests only')
    ...
    try:
        ...
        if not args.quick:
            ...
            all_results['test_results']['statistical_performance'] = run_statistical_performance_tests(
                args.iterations, args.confidence
            )
