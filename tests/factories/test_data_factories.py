...
if FACTORY_AVAILABLE:
    ...
    class RoverStateFactory(URCBaseFactory):
        ...
        # Core state data
        battery_level = LazyFunction(lambda: fake.pyfloat(min_value=0.0, max_value=100.0, precision=2))
        communication_status = LazyFunction(lambda: fake.random_element(elements=['connected', 'degraded', 'lost']))
        system_health = LazyFunction(lambda: fake.random_element(elements=['nominal', 'degraded', 'critical', 'emergency']))
    ...
    class MissionFactory(URCBaseFactory):
        ...
        distance_traveled = LazyAttribute(lambda o: sum(
            np.sqrt((o.waypoints[i+1]['x'] - o.waypoints[i]['x'])**2 +
                   (o.waypoints[i+1]['y'] - o.waypoints[i]['y'])**2)
            for i in range(len(o.waypoints)-1)
        ) if len(o.waypoints) > 1 else 0)

        samples_collected = LazyAttribute(lambda o: fake.random_int(min=0, max=len(o.waypoints)) if o.type == 'sample_collection' else 0)
    ...
    class CommandFactory(URCBaseFactory):
        ...
        def parameters(self):
            ...
            if self.command == 'start_mission':
                base_params['mission_type'] = fake.random_element(['autonomous_navigation', 'sample_collection'])
                base_params['waypoints'] = WaypointFactory.create_batch(fake.random_int(min=2, max=5))
    ...
    def create_time_series_data(points: int = 100, frequency: float = 1.0,
        ...
        if pattern == 'sinusoidal':
            values = [10.0 + 2.0 * np.sin(2 * np.pi * 0.1 * t) + 0.1 * np.random.normal() for t in times]
        elif pattern == 'linear':
    ...
    def create_test_mission(**kwargs):
        ...
        return {
            ...
            'waypoints': [{'name': 'wp1', 'x': 0, 'y': 0}, {'name': 'wp2', 'x': 10, 'y': 5}],
            **kwargs
        }
