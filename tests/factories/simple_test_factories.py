...
class TestDataFactory:
    ...
    def create_rover_state(**overrides) -> Dict[str, Any]:
        ...
        if FAKER_AVAILABLE:
            base_data = {
                ...
                "communication_status": fake.random_element(
                    ["connected", "degraded", "lost"]
                ),
                "system_health": fake.random_element(
                    ["nominal", "degraded", "critical"]
                ),
                "current_mode": fake.random_element(
                    ["BOOT", "CALIBRATION", "IDLE", "TELEOPERATION", "AUTONOMOUS"]
                ),
                ...
                "angular_velocity_z": round(
                    fake.pyfloat(min_value=-0.5, max_value=0.5), 2
                ),
                "mission_status": fake.random_element(
    ...
    def create_mission(**overrides) -> Dict[str, Any]:
        ...
        waypoints = [
            TestDataFactory.create_waypoint() for _ in range(random.randint(2, 5))
        ]
        ...
        if FAKER_AVAILABLE:
            base_data = {
                ...
                "type": fake.random_element(
                    ["autonomous_navigation", "sample_collection", "delivery"]
                ),
                "waypoints": waypoints,
...
def create_time_series_data(
    ...
    if pattern == "sinusoidal":
        values = [
            10.0 + 2.0 * math.sin(2 * math.pi * 0.1 * t) + 0.1 * random.gauss(0, 1)
            for t in times
        ]
    elif pattern == "linear":
