...
class ComprehensiveSystemTester(Node):
    ...
    def hardware_cmd_callback(self, msg):
        ...
        perf_interval = self.config.get("testing", {}).get(
            "performance_monitor_interval", 0.2
        )
        self.test_timer = self.create_timer(phase_interval, self.run_test_phases)
        self.performance_timer = self.create_timer(
            perf_interval, self.monitor_performance
        )
    ...
    def run_test_phases(self):
        ...
        if self.current_test_phase < len(test_phases):
            phase_name = (
                ...
                .__name__.replace("test_phase_", "")
                .replace("_", " ")
                .title()
            )
            self.get_logger().info(
                f"Running Test Phase {self.current_test_phase + 1}: {phase_name}"
            )
            test_phases[self.current_test_phase]()
    ...
    def test_phase_1_mission_execution(self):
        ...
        waypoints = [{"x": 2.0, "y": 0.0}, {"x": 2.0, "y": 2.0}, {"x": 0.0, "y": 2.0}]
        ...
        if (
            self.received_mission_status
            and self.received_mission_status.get("status") == "completed"
        ):
            self.add_test_result(
                "Waypoint Navigation", True, "Mission completed successfully"
            )
        else:
            self.add_test_result(
                "Waypoint Navigation", False, "Mission did not complete"
            )
        ...
        if (
            self.received_mission_status
            and self.received_mission_status.get("status") == "stopped"
        ):
            self.add_test_result(
                "Mission Interruption", True, "Mission stopped successfully"
            )
        else:
        ...
        if progress_values and max(progress_values) >= 99.0:
            self.add_test_result(
                "Progress Tracking",
                True,
                f"Progress reached {max(progress_values):.1f}%",
            )
        else:
    ...
    def test_phase_2_sensor_integration(self):
        ...
        if self.received_odom and self.received_imu and self.received_gps:
            ...
            timestamps = [
                self.received_odom.header.stamp.sec
                + self.received_odom.header.stamp.nanosec * 1e-9,
                self.received_imu.header.stamp.sec
                + self.received_imu.header.stamp.nanosec * 1e-9,
                self.received_gps.header.stamp.sec
                + self.received_gps.header.stamp.nanosec * 1e-9,
            ]
            ...
            if max_diff < 1.0:
                self.add_test_result(
                    ...
                    True,
                    f"Max timestamp difference: {max_diff:.3f}s",
                )
            else:
                self.add_test_result(
                    "Data Synchronization",
                    False,
                    f"Timestamp difference too large: {max_diff:.3f}s",
                )
        else:
            self.add_test_result(
                "Data Synchronization", False, "Missing sensor data for sync check"
            )
    ...
    def test_phase_3_slam_localization(self):
        ...
        if self.received_slam_pose:
            ...
            self.add_test_result(
                "SLAM Pose", True, f"Pose: ({pos.x:.2f}, {pos.y:.2f}, {pos.z:.2f})"
            )
        else:
        ...
        if self.received_map_data:
            ...
            if robot:
                ...
                self.add_test_result(
                    "Map Data Flow", False, "Map data missing robot information"
                )
        else:
            self.add_test_result(
                "Map Data Flow", False, "No frontend map data received"
            )
        ...
        if self.received_odom and self.received_slam_pose:
            ...
            distance = (
                (odom_pos.x - slam_pos.x) ** 2 + (odom_pos.y - slam_pos.y) ** 2
            ) ** 0.5
            if distance < 2.0:  # Allow some drift
                self.add_test_result(
                    "Coordinate Consistency",
                    True,
                    f"Position difference: {distance:.3f}m",
                )
            else:
                self.add_test_result(
                    "Coordinate Consistency",
                    False,
                    f"Position difference too large: {distance:.3f}m",
                )
        else:
            self.add_test_result(
                "Coordinate Consistency", False, "Missing data for coordinate check"
            )
    ...
    def test_phase_4_network_communication(self):
        ...
        for topic in topics_to_check:
            if topic in active_topics:
                self.add_test_result(
                    f"ROS2 Topic {topic}", True, "Topic active and publishing"
                )
            else:
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "bash",
                    "-c",
                    'echo "test" | nc -q1 localhost 8765 >/dev/null',
                ],
                capture_output=True,
                timeout=5,
            )
            if result.returncode == 0:
                self.add_test_result(
                    "WebSocket Interface", True, "WebSocket server responding"
                )
            else:
                self.add_test_result(
                    "WebSocket Interface", False, "WebSocket server not responding"
                )
        except Exception as e:
            self.add_test_result(
                "WebSocket Interface", False, f"WebSocket test failed: {e}"
            )
        ...
        if response_time < 0.5:  # Less than 500ms
            self.add_test_result(
                "Command Response Time", True, f"Response time: {response_time:.3f}s"
            )
        else:
            self.add_test_result(
    ...
    def test_phase_5_performance_testing(self):
        ...
        if self.cpu_usage:
            ...
            if avg_cpu < 50.0:
                self.add_test_result(
                    "CPU Usage", True, f"Average: {avg_cpu:.1f}%, Peak: {max_cpu:.1f}%"
                )
            else:
                self.add_test_result(
                    "CPU Usage", False, f"CPU usage too high: {avg_cpu:.1f}%"
                )
        else:
        ...
        if self.memory_usage:
            ...
            if avg_memory < 70.0:
                ...
                self.add_test_result(
                    "Memory Usage", False, f"Memory usage too high: {avg_memory:.1f}%"
                )
        else:
            self.add_test_result(
                "Memory Usage", False, "No memory usage data collected"
            )
        ...
        for topic, rate in rates.items():
            if rate > 5:  # At least 5 Hz for sensor data
                self.add_test_result(
                    f"Publication Rate {topic}", True, f"{rate:.1f} Hz"
                )
            else:
                self.add_test_result(
                    f"Publication Rate {topic}", False, f"Rate too low: {rate:.1f} Hz"
                )
    ...
    def test_phase_6_error_handling(self):
        ...
        if self.received_odom:  # Basic functionality check
            self.add_test_result(
                ...
                True,
                "System handled invalid command gracefully",
            )
        else:
            self.add_test_result(
                "Invalid Command Handling", False, "System failed after invalid command"
            )
        ...
        if self.memory_usage and max(self.memory_usage) < 90.0:
            self.add_test_result(
                "Resource Limits",
                True,
                f"Max memory usage: {max(self.memory_usage):.1f}%",
            )
        else:
            self.add_test_result(
                "Resource Limits", False, "Memory usage approaching limits"
            )
    ...
    def test_phase_7_integration_testing(self):
        ...
        workflow_complete = (
            self.received_mission_status
            and self.received_map_data
            and self.received_odom
            and self.received_imu
        )

        if workflow_complete:
            self.add_test_result(
                "End-to-End Workflow", True, "Complete mission workflow successful"
            )
        else:
            self.add_test_result(
                "End-to-End Workflow", False, "Mission workflow incomplete"
            )
        ...
        if self.received_odom and self.received_map_data:
            ...
            if time_diff < 0.5:  # Within 500ms
                self.add_test_result(
                    ...
                    True,
                    f"Time sync: {time_diff:.3f}s difference",
                )
            else:
                self.add_test_result(
                    "Multi-Component Sync",
                    False,
                    f"Time sync poor: {time_diff:.3f}s difference",
                )
        else:
            self.add_test_result(
                "Multi-Component Sync", False, "Missing data for sync test"
            )
    ...
    def generate_final_report(self):
        ...
        report.append(
            f"Total Test Duration: {time.time() - self.system_start_time:.1f} seconds"
        )
        report.append("")
        report.append(
