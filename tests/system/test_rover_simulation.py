...
class RoverSimulationTester(Node):
    ...
    def test_phase_0_check_simulation_startup(self):
        ...
        try:
            ...
            found_nodes = [
                node for node in nodes if any(req in node for req in required_nodes)
            ]

            if len(found_nodes) >= 2:
                self.add_test_result(
                    "Simulation nodes detected", True, f"Found nodes: {found_nodes}"
                )
                self.test_phase = 1
            else:
                self.add_test_result(
                    "Simulation nodes check", False, f"Only found: {found_nodes}"
                )
    ...
    def test_phase_2_test_mission_commands(self):
        ...
        command = {"command": "start_waypoint_mission", "waypoints": waypoints}
        ...
        self.add_test_result(
            "Mission command sent",
            True,
            f"Sent waypoint mission with {len(waypoints)} waypoints",
        )
        self.test_phase = 3
    ...
    def test_phase_3_test_data_flow(self):
        ...
        if self.received_map_data:
            ...
            if "x" in robot and "y" in robot:
                ...
                self.add_test_result(
                    "Map data structure", False, "Missing robot position"
                )
        else:
        ...
        if self.received_mission_status:
            ...
            self.add_test_result(
                "Mission status flow", False, "No mission status received"
            )
    ...
    def print_final_results(self):
        ...
        categories = {"Simulation": [], "Sensors": [], "Mission": [], "Data Flow": []}

        for result in self.test_results:
            if (
                "simulation" in result["name"].lower()
                or "node" in result["name"].lower()
            ):
                categories["Simulation"].append(result)
            elif (
                "odom" in result["name"].lower()
                or "imu" in result["name"].lower()
                or "sensor" in result["name"].lower()
            ):
                categories["Sensors"].append(result)
            elif (
                "mission" in result["name"].lower()
                or "waypoint" in result["name"].lower()
            ):
