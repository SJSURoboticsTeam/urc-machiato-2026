...
class TestCompetitionScenario(unittest.TestCase):
    ...
    def test_full_competition_mission(self):
        ...
        for waypoint in gnss_waypoints:
            with self.subTest(waypoint=f"GNSS_{waypoint['id']}"):
                ...
                self.assertLessEqual(
                    accuracy,
                    self.mission_requirements["gnss_accuracy"],
        ...
        for target in aruco_targets:
            with self.subTest(target=f"AR_{target['id']}"):
                ...
                self.assertLessEqual(
                    accuracy,
                    self.mission_requirements["aruco_accuracy"],
        ...
        for equipment in equipment_locations:
            with self.subTest(equipment=f"EQUIP_{equipment['id']}"):
                ...
                if "sequence" in equipment:
                    # Autonomous typing
                    success = self.system.perform_autonomous_typing(
                        equipment["sequence"]
                    )
                    self.assertTrue(
                        success, f"Failed to type sequence: {equipment['sequence']}"
                    )
                else:
                    # Equipment interaction
                    success = self.system.interact_with_equipment(
                        equipment["equipment"]
                    )
                    self.assertTrue(
                        success, f"Failed to interact with: {equipment['equipment']}"
                    )
        ...
        self.assertLessEqual(
            total_time,
            self.mission_requirements["time_limit"],
        ...
        self.assertEqual(
            len(completed_waypoints),
            self.mission_requirements["waypoints"],
    ...
    def test_performance_requirements(self):
        ...
        self.assertLessEqual(
            typing_time, 30.0, f"Typing too slow: {typing_time:.1f}s for 5 characters"
        )
        ...
        self.assertLessEqual(
            avg_response, 0.1, f"Average response too slow: {avg_response:.3f}s"
        )
        self.assertLessEqual(
            max_response, 0.5, f"Max response too slow: {max_response:.3f}s"
        )
    ...
    def test_robustness_under_failure(self):
        ...
        for scenario in failure_scenarios:
            with self.subTest(scenario=scenario):
                ...
                self.assertTrue(safety_engaged, f"Safety not engaged during {scenario}")

                # System should remain stable (not crash)
                self.assertTrue(system_stable, f"System unstable during {scenario}")

                # Recovery should be possible
                recovery_success = self.system.attempt_recovery()
                self.assertTrue(recovery_success, f"Recovery failed for {scenario}")
