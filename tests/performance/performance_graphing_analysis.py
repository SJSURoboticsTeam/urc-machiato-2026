...
class PerformanceGraphGenerator:
    ...
    def generate_comprehensive_dashboard(self):
        ...
        fig.suptitle('URC 2026 Mars Rover - Performance Analysis Dashboard', fontsize=16, fontweight='bold')
        ...
        dashboard_file = self.output_dir / f"performance_dashboard_{int(datetime.now().timestamp())}.png"
    ...
    def _plot_performance_overview(self, ax):
        ...
        for test_name, test_runs in self.data.items():
            if test_runs and isinstance(test_runs[0], dict) and 'p99_ms' in test_runs[0]:
                ...
                mean_vals = [run.get('mean_ms', 0) for run in test_runs if 'mean_ms' in run]
        ...
        if categories:
            ...
            bars1 = ax.bar(x - width/2, p99_values, width, label='P99 Latency', alpha=0.8, color='red')
            bars2 = ax.bar(x + width/2, mean_values, width, label='Mean Latency', alpha=0.8, color='blue')
    ...
    def _plot_latency_distribution(self, ax):
        ...
        if all_latencies:
            ax.hist(all_latencies, bins=20, alpha=0.7, edgecolor='black', color='skyblue')
            ...
            ax.axvline(mean_lat, color='red', linestyle='--', alpha=0.8, label=f'Mean: {mean_lat:.2f}ms')
            ax.axvline(median_lat, color='green', linestyle='--', alpha=0.8, label=f'Median: {median_lat:.2f}ms')
            ax.legend()
    ...
    def _plot_requirements_compliance(self, ax):
        ...
        actual = [req['actual'] if req['actual'] is not None else 0 for req in requirements.values()]

        x = np.arange(len(categories))
        width = 0.35

        bars1 = ax.bar(x - width/2, required, width, label='Required (ms)', alpha=0.8, color='red')
        bars2 = ax.bar(x + width/2, actual, width, label='Actual (ms)', alpha=0.8, color='green')
        ...
        for i, (req, act) in enumerate(zip(required, actual)):
            if act > 0:
                ...
                ax.text(x[i], max(req, act) + 0.5, compliance, ha='center', va='bottom', fontsize=12)
        ...
        for test_name, test_runs in self.data.items():
            ...
            if len(latencies) > 1:
                test_variability[test_name] = {
                    ...
                    'cv': (statistics.stdev(latencies) / statistics.mean(latencies)) * 100 if len(latencies) > 1 else 0
                }
    ...
    def generate_chaos_analysis_graphs(self):
        ...
        ax.text(0.5, 0.5, 'Chaos Engineering Analysis\n\n(Not implemented in current test run)',
               ha='center', va='center', transform=ax.transAxes, fontsize=14)
        ax.set_title('Chaos Engineering Impact Analysis')
        ax.axis('off')

        chaos_file = self.output_dir / f"chaos_analysis_{int(datetime.now().timestamp())}.png"
    ...
    def generate_executive_summary_report(self):
        ...
        summary['test_summary'] = {
            ...
            'success_rate': (successful_tests / total_measurements * 100) if total_measurements > 0 else 0
        }
        ...
        summary['performance_highlights'] = {
            ...
            'overall_performance_trend': 'good' if len(best_performers) > len(worst_performers) else 'needs_improvement'
        }
        ...
        summary['compliance_status'] = {
            ...
            'compliance_rate': (compliant_count / len(compliance_checks) * 100) if compliance_checks else 0,
        ...
        if compliance_rate >= 80:
            recommendations.append("✅ Excellent performance - system meets most requirements")
        elif compliance_rate >= 60:
            recommendations.append("⚠️ Good performance - minor optimizations needed")
        else:
            recommendations.append("❌ Performance issues detected - significant improvements required")

        if worst_performers:
            recommendations.append(f"🔧 Focus optimization on: {', '.join([name for name, _ in worst_performers[:2]])}")

        if successful_tests < total_measurements * 0.8:
            recommendations.append("🧪 Increase test reliability and reduce measurement variability")

        summary['recommendations'] = recommendations

        # Save executive summary
        summary_file = self.output_dir / f"executive_summary_{int(datetime.now().timestamp())}.json"
    ...
    def print_executive_summary(self, summary: Dict[str, Any]):
        ...
        print("\n📏 COMPLIANCE STATUS:")
        print(f"  Requirements Met: {comp.get('compliant_requirements', 0)}/{comp.get('total_requirements', 0)}")
...
def run_chaos_tests():
    ...
    try:
        ...
        chaos_results = {
            ...
            'success_rate': (len(result.successes) if hasattr(result, 'successes') else 0) / result.testsRun * 100 if result.testsRun > 0 else 0
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='URC 2026 Performance Graphing and Analysis')
    parser.add_argument('--input-reports', nargs='+',
                       help='Input performance report JSON files')
    parser.add_argument('--output-dir', type=str, default='performance_graphs',
                       help='Output directory for graphs and reports (default: performance_graphs)')
    parser.add_argument('--find-reports', action='store_true',
                       help='Automatically find all performance report files')
    parser.add_argument('--run-chaos-tests', action='store_true',
    ...
    if args.find_reports:
        ...
        if report_files:
            ...
            report_files.sort(key=lambda x: x.split('_')[-1].replace('.json', ''), reverse=True)
    ...
    if args.run_chaos_tests:
        ...
        # Save chaos results
        chaos_file = Path(args.output_dir) / f"chaos_test_results_{int(datetime.now().timestamp())}.json"
