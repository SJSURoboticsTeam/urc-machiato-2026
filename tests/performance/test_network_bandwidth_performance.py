...
class NetworkBandwidthPerformanceTest(unittest.TestCase):
    ...
    def _start_bandwidth_monitoring(self):
        ...
        self.monitor_thread = threading.Thread(target=self._bandwidth_monitor_worker, daemon=True)
    ...
    def _test_vision_data_transmission(self) -> Dict[str, Any]:
        ...
        data_rate_mbps = (total_data_size * 8) / (1024 * 1024 * test_duration)
        avg_compression_ratio = statistics.mean(compressed_sizes) if compressed_sizes else 1.0
    ...
    def _generate_vision_messages(self) -> List[Dict[str, Any]]:
        ...
        if np.random.random() < 0.5:  # 50% chance for 10Hz when called at 15Hz
            terrain_data = {
                ...
                'width': 100,
                'height': 100,
                'data': np.random.choice([0, 1, 2, 3], size=(100, 100)).tolist()  # Terrain classes
            }
    ...
    def _compress_message(self, message: Dict[str, Any]) -> int:
        ...
        try:
            ...
            return int(self._estimate_message_size(message) * 0.7)  # Assume 70% compression
        ...
        min_compressed_size = min(compressed_sizes) if compressed_sizes else len(serialized)
    ...
    def _analyze_bandwidth_usage(self, results_by_scenario: Dict[str, Any]):
        ...
        total_bandwidth = sum(result.get('data_rate_mbps', 0) for result in results_by_scenario.values())
        avg_bandwidth = total_bandwidth / len(results_by_scenario) if results_by_scenario else 0

        # Find peak bandwidth usage
        peak_bandwidth = max((result.get('data_rate_mbps', 0) for result in results_by_scenario.values()), default=0)

        # Analyze compression effectiveness
        avg_compression_ratio = statistics.mean(self.compression_ratios) if self.compression_ratios else 1.0
        ...
        for scenario_name, result in results_by_scenario.items():
            scenario_display = scenario_name.replace('_', ' ').replace('test_', '').title()
        ...
        requirements = {
            ...
            'Average Compression Ratio': (avg_compression_ratio, self.target_compression_ratio),
            'Best Compression Ratio': (best_compression_ratio, self.target_compression_ratio * 0.8),
        }
        ...
        if all_passed:
            ...
            print("   Communication system suitable for competition bandwidth constraints")
        else:
        ...
        self.assertLess(avg_bandwidth, self.target_bandwidth_mbps * 1.5,
                       ".2f")
        self.assertLess(avg_compression_ratio, 0.8,
                       ".2f")
    ...
    def test_packet_loss_recovery_performance(self):
        ...
        for loss_rate in loss_rates:
            ...
            avg_recovery_time = recovery_time / messages_lost if messages_lost > 0 else 0
            ...
            recovery_results[f'{loss_rate*100:.1f}%_loss'] = {
                ...
                'recovery_rate': recovered_messages / messages_lost if messages_lost > 0 else 1.0
            }
        ...
        avg_recovery_times = [r['avg_recovery_time_ms'] for r in recovery_results.values()]
