...
class ResourceBudgetingRegressionTester:
    ...
    def __init__(
        self,
        cpu_limit_ghz: float = 2.0,
        ram_limit_gb: float = 16.0,
        ...
        self.resource_limits = ResourceLimits(
            cpu_cores=max(
                1, int(cpu_limit_ghz / 0.5)
            ),  # Assume 0.5GHz per core baseline
            ram_gb=ram_limit_gb,
    ...
    def run_comprehensive_resource_budgeting(
        ...
        print("=" * 60)
        print(
            f"Resource Constraints: {self.cpu_limit_ghz}GHz CPU, {self.ram_limit_gb}GB RAM"
        )
        ...
        try:
            ...
            regression_analysis = self._analyze_performance_regressions(
                baselines, stress_results
            )

            # Phase 4: Resource budgeting analysis
            print("\n💰 Phase 4: Resource Budgeting Analysis...")
            resource_budget = self._analyze_resource_budgeting(
                baselines, stress_results
            )
    ...
    def _establish_performance_baselines(
                ...
                if isinstance(test_data, dict):
                    for metric_name, metric_value in test_data.items():
                        if (
                            ...
                            and "ms" in metric_name
                        ):
                            baselines[test_name].measurements[metric_name].append(
                                metric_value
                            )
        ...
        for test_name, baseline in baselines.items():
            for metric_name, values in baseline.measurements.items():
                if values:
                    baseline.measurements[f"{metric_name}_mean"] = statistics.mean(
                        values
                    )
                    baseline.measurements[f"{metric_name}_median"] = statistics.median(
                        values
                    )
                    baseline.measurements[f"{metric_name}_p95"] = sorted(values)[
                        int(len(values) * 0.95)
                    ]
                    baseline.measurements[f"{metric_name}_p99"] = sorted(values)[
                        int(len(values) * 0.99)
                    ]
                    baseline.measurements[f"{metric_name}_stdev"] = (
    ...
    def _run_cpu_stress_test(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "p95_cpu_usage": sorted(cpu_usage_samples)[
                int(len(cpu_usage_samples) * 0.95)
            ],
            "within_limits": max(cpu_usage_samples)
    ...
    def _run_memory_stress_test(self) -> Dict[str, Any]:
        ...
        return {
            "peak_memory_percent": (
                max(memory_usage_samples) if memory_usage_samples else 0
            ),
            "avg_memory_percent": (
                statistics.mean(memory_usage_samples) if memory_usage_samples else 0
            ),
            "within_limits": (
                max(memory_usage_samples) <= 90 if memory_usage_samples else True
            ),
    ...
    def _run_combined_stress_test(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cpu_within_limits": max(cpu_samples)
            <= self.performance_bounds["cpu_usage_percent"],
    ...
    def _run_network_stress_test(self) -> Dict[str, Any]:
        ...
        if len(network_samples) > 1:
            time_diff = (
                network_samples[-1]["timestamp"] - network_samples[0]["timestamp"]
            )
            bytes_sent = (
                network_samples[-1]["bytes_sent"] - network_samples[0]["bytes_sent"]
            )
            bytes_recv = (
                network_samples[-1]["bytes_recv"] - network_samples[0]["bytes_recv"]
            )
            ...
            return {
                ...
                "within_bandwidth_limit": (sent_rate_mbps + recv_rate_mbps)
                <= self.resource_limits.network_bandwidth_mbps,
            }

        return {"error": "Insufficient network samples"}

    def _analyze_performance_regressions(
        self, baselines: Dict[str, PerformanceBaseline], stress_results: Dict[str, Any]
    ) -> Dict[str, Any]:
                            ...
                            regression_analysis["regressions_detected"].append(
                                {
                                    ...
                                    "regression_percent": (
                                        (baseline_p99 - bound_value) / bound_value
                                    )
                                    * 100,
                                }
                            )
        ...
        regression_analysis["bottleneck_analysis"] = {
            ...
            "resource_contention": stress_results.get("combined_stress", {}).get(
                "avg_cpu_usage", 0
            )
            > 70,
        }
        ...
        if regression_analysis["regressions_detected"]:
            regression_analysis["recommendations"].append(
                "❌ Performance regressions detected - investigate bottlenecks"
            )

        if regression_analysis["bottleneck_analysis"]["cpu_bottleneck"]:
            regression_analysis["recommendations"].append(
                "⚠️ CPU bottleneck detected - consider optimization or resource increase"
            )

        if regression_analysis["bottleneck_analysis"]["memory_bottleneck"]:
            regression_analysis["recommendations"].append(
                "⚠️ Memory bottleneck detected - monitor memory usage patterns"
            )

        if not regression_analysis["regressions_detected"]:
            regression_analysis["recommendations"].append(
                "✅ No performance regressions detected - system performing within bounds"
            )
    ...
    def _analyze_resource_budgeting(
        ...
        resource_budget = {
            "cpu_cores_required": max(
                1, int(sum(cpu_requirements) / 0.8)
    ...
    def _calculate_efficiency_score(
        ...
        regressions = len(
            self._analyze_performance_regressions(baselines, stress_results)[
                "regressions_detected"
            ]
        )
    ...
    def _generate_resource_budgeting_report(
        self,
        ...
        report_file = (
            self.output_dir / f"resource_budgeting_report_{int(time.time())}.json"
        )
    ...
    def generate_performance_summary(self, report: Dict[str, Any]) -> str:
        ...
        summary_lines.append(
            f"  CPU: {limits.get('cpu_ghz', 0)}GHz ({limits.get('cpu_cores', 0)} cores)"
        )
        ...
        summary_lines.append("\n💰 RESOURCE BUDGETING:")
        summary_lines.append(
            f"  CPU Cores Required: {budget.get('cpu_cores_required', 0)}"
        )
        summary_lines.append(
            f"  Memory Required: {budget.get('memory_gb_required', 0)}GB"
        )
        summary_lines.append(
            f"  Power Budget: {budget.get('power_budget_watts', 0):.1f}W"
        )
        summary_lines.append(
            f"  Efficiency Score: {budget.get('resource_efficiency_score', 0):.1f}/100"
        )
        # Recommendations
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="URC 2026 Resource Budgeting & Regression Tests"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="resource_regression_results",
