...
class SystemIntegrationMonitor:
            ...
            try:
                ...
                for component, subsystems in self.system_components.items():
                    for subsystem_name, subsystem_data in subsystems.items():
                        metrics_store.update_system_status(
                            component, subsystem_name, subsystem_data
                        )
    ...
    def _monitor_behavior_trees(self):
        ...
        try:
            ...
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    if (
                        "behavior" in " ".join(proc.info["cmdline"] or []).lower()
                        or "bt" in proc.info["name"].lower()
                    ):
            ...
            for bt_name in list(self.system_components["behavior_trees"].keys()):
                if (
                    current_time
                    - self.system_components["behavior_trees"][bt_name].get(
                        "last_check", 0
                    )
                    > 10
                ):
        ...
        try:
            ...
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    ...
                    if (
                        "state" in cmdline.lower()
                        or "machine" in cmdline.lower()
                        or "sm" in proc.info["name"].lower()
                    ):
            ...
            for sm_name in list(self.system_components["state_machines"].keys()):
                if (
                    current_time
                    - self.system_components["state_machines"][sm_name].get(
                        "last_check", 0
                    )
                    > 10
                ):
    ...
    def _monitor_simulator_status(self):
        ...
        try:
            ...
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    ...
                    if (
                        ...
                        or "gzserver" in name
                        or "gzclient" in name
                        or "simulator" in cmdline
                        or "gazebo" in cmdline
                    ):
            ...
            for i, proc in enumerate(sim_processes):
                ...
                real_time_factor = max(
                    0.1, 1.0 - (cpu_usage / 100.0) * 0.5
                )  # Rough estimate
            ...
            for sim_name in list(self.system_components["simulator"].keys()):
                if (
                    current_time
                    - self.system_components["simulator"][sim_name].get("last_check", 0)
                    > 10
                ):
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run(
                    ["pgrep", "-f", "ros2"], capture_output=True, text=True, timeout=2
                )
            ...
            network_connections = len(
                [
                    c
                    for c in self.system_components["network"].values()
            ...
            ipc_connections = len(
                [
                    bt
                    for bt in self.system_components["behavior_trees"].values()
                    if bt.get("active", False)
                ]
            )
    ...
    def _calculate_health_score(self) -> float:
        ...
        sm_active = len(
            [
                sm
                for sm in self.system_components["state_machines"].values()
                if sm.get("active", False)
            ]
        )
        ...
        net_connected = len(
            [
                n
                for n in self.system_components["network"].values()
                if n.get("connected", False)
            ]
        )
        ...
        sim_running = len(
            [
                s
                for s in self.system_components["simulator"].values()
