...
class MissionExecutionPerformanceTest(unittest.TestCase):
    ...
    def _run_mission_execution_performance_test(self):
        ...
        for mission_num in range(self.num_missions):
            ...
            if mission_result:
                ...
                self.path_generation_times.append(mission_result['path_generation_time_ms'])
                self.state_transition_times.extend(mission_result['state_transition_times_ms'])
    ...
    def _create_test_mission(self, mission_id: str) -> Dict[str, Any]:
        ...
        mission_types = ['waypoint_navigation', 'object_search', 'terrain_traversal', 'return_home']
    ...
    def _execute_test_mission(self, mission: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _analyze_mission_results(self):
        ...
        p95_state_transition = statistics.quantiles(self.state_transition_times, n=20)[18]
        ...
        memory_range_mb = max_memory_mb - min(self.memory_usage_readings) if self.memory_usage_readings else 0
        ...
        requirements = {
            ...
            'State Transitions (avg)': (avg_state_transition, self.target_state_transition_ms),
            'State Transitions (95p)': (p95_state_transition, self.target_state_transition_ms * 1.2),
        ...
        elif avg_planning <= self.target_planning_ms and avg_execution <= 5000:  # 5s total
        ...
        if avg_planning > self.target_planning_ms or avg_path_gen > self.target_path_gen_ms:
        ...
        self.assertLess(avg_planning, self.target_planning_ms * 1.5,
                       ".3f")
        self.assertLess(avg_path_gen, self.target_path_gen_ms * 1.5,
                       ".3f")
        self.assertLess(avg_state_transition, self.target_state_transition_ms * 2.0,
                       ".3f")
        self.assertLess(max_memory_mb, self.target_memory_mb * 1.5,
                       ".2f")
...
class MissionPlannerSimulator:
    ...
    def plan_mission(self, mission: Dict[str, Any]) -> Dict[str, Any]:
        ...
        complexity_multiplier = len(mission['waypoints']) / 5.0  # Normalize to 5 waypoints
        ...
        planning_time = self.planning_base_time * complexity_multiplier * mission_type_complexity
