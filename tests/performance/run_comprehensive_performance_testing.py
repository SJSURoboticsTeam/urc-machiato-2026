...
class ComprehensivePerformanceTester:
    ...
    def __init__(self, duration_seconds: int = 300, chaos_level: float = 0.5, output_dir: str = "perf_results"):
    ...
    def _run_phase_1_baseline_tests(self):
        ...
        self._log_performance_metric("binary_protocol_p99", binary_results.get('p99_ms', 0))
        ...
        self._log_performance_metric("motion_control_p99", motion_results.get('p99_ms', 0))
    ...
    def _run_phase_2_integration_tests(self):
        ...
        try:
            from tests.run_comprehensive_resource_tests import run_fallback_tests, run_recovery_tests
    ...
    def _run_phase_4_chaos_tests(self):
        ...
        try:
            from tests.chaos.test_chaos_engineering import TestNetworkChaos, TestServiceChaos
    ...
    def _background_monitoring(self):
        ...
        while self.monitoring_active and (time.time() - start_time) < self.duration_seconds:
            try:
    ...
    def _generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        report_file = self.output_dir / f"comprehensive_performance_report_{int(time.time())}.json"
    ...
    def _perform_statistical_analysis(self) -> Dict[str, Any]:
        ...
        if 'cpu_percent' in self.system_metrics:
            ...
            analysis['cpu_stats'] = {
                ...
                'max': max(cpu_data),
                'stability_score': 100 - statistics.stdev(cpu_data)  # Lower variance = higher stability
            }
        ...
        if 'memory_mb' in self.system_metrics:
            ...
            analysis['memory_stats'] = {
                ...
                'max_mb': max(mem_data),
                'stability_score': 100 - (statistics.stdev(mem_data) / statistics.mean(mem_data) * 10)
            }
    ...
    def _analyze_chaos_impact(self) -> Dict[str, Any]:
        ...
        for event in self.chaos_events:
            ...
            # Analyze CPU impact
            cpu_during_event = [
                cpu for ts, cpu in zip(self.system_metrics.get('timestamp', []),
            ...
            mem_during_event = [
                mem for ts, mem in zip(self.system_metrics.get('timestamp', []),
            ...
            if mem_during_event:
                ...
                baseline_mem = statistics.mean(self.system_metrics.get('memory_mb', [])[:10])  # First 10 readings
    ...
    def _analyze_performance_trends(self) -> Dict[str, Any]:
        ...
        # Analyze CPU trend
        if 'cpu_percent' in self.system_metrics and len(self.system_metrics['cpu_percent']) > 20:
            cpu_data = self.system_metrics['cpu_percent']
            ...
            trends['cpu_trend'] = {
                ...
                'stability': 'stable' if abs(second_avg - first_avg) < 5 else 'unstable'
            }
    ...
    def _create_performance_graphs(self, report: Dict[str, Any]):
        ...
        fig.suptitle('URC 2026 Comprehensive Performance Analysis', fontsize=16, fontweight='bold')
        ...
        if 'cpu_percent' in self.system_metrics:
            axes[0, 0].plot(time_axis, self.system_metrics['cpu_percent'], linewidth=2, alpha=0.8)
            ...
            for event in self.chaos_events:
                ...
                axes[0, 0].axvline(x=event_time, color='red', linestyle='--', alpha=0.7, linewidth=1)

        # 2. Memory Usage Over Time
        if 'memory_mb' in self.system_metrics:
            axes[0, 1].plot(time_axis, self.system_metrics['memory_mb'], linewidth=2, alpha=0.8, color='orange')
            ...
            for event in self.chaos_events:
                event_time = event['start_time'] - base_time
                axes[0, 1].axvline(x=event_time, color='red', linestyle='--', alpha=0.7, linewidth=1)
        ...
        if self.chaos_events:
            event_times = [(event['start_time'] - base_time) for event in self.chaos_events]
            event_durations = [event['duration'] for event in self.chaos_events]
            event_types = [event['type'].replace('_', ' ').title() for event in self.chaos_events]
        ...
        if 'cpu_stats' in analysis:
            ...
            stat_values = [cpu_stats['mean'], cpu_stats['median'], cpu_stats['p95'], cpu_stats['max']]
        ...
        if health_metrics:
            colors = ['green' if x > 80 else 'orange' if x > 60 else 'red' for x in health_metrics]
            bars = axes[2, 1].bar(health_labels, health_metrics, alpha=0.8, color=colors)
        ...
        dashboard_file = self.output_dir / f"performance_dashboard_{int(time.time())}.png"
    ...
    def _create_chaos_impact_graph(self):
        ...
        fig.suptitle('Chaos Engineering Impact Analysis', fontsize=14, fontweight='bold')
        ...
        if 'memory_mb' in self.system_metrics:
            mem_data = self.system_metrics['memory_mb']
            ax2.plot(time_axis, mem_data, alpha=0.7, color='orange', label='Memory Usage')
    ...
    def _create_performance_distribution_graph(self):
        ...
        fig.suptitle('Performance Distribution Analysis', fontsize=14, fontweight='bold')
        ...
        if 'memory_mb' in self.system_metrics:
            mem_data = self.system_metrics['memory_mb']
            axes[0, 1].hist(mem_data, bins=20, alpha=0.7, color='orange', edgecolor='black')
        ...
        if self.chaos_events:
            ...
            axes[1, 1].bar(event_counts.index.str.replace('_', ' ').str.title(),
                          event_counts.values, alpha=0.7, color='red')
    ...
    def _create_time_series_analysis(self):
        ...
        if 'timestamp' in self.system_metrics:
            ...
            if 'cpu_percent' in self.system_metrics:
                ...
                ax1.plot(time_axis, self.system_metrics['cpu_percent'], alpha=0.5, label='Raw CPU')
                ax1.plot(time_axis, cpu_rolling, linewidth=2, label='10-point Rolling Average')
            ...
            if 'memory_mb' in self.system_metrics:
                ...
                ax2.plot(time_axis, self.system_metrics['memory_mb'], alpha=0.5, color='orange', label='Raw Memory')
                ax2.plot(time_axis, mem_rolling, linewidth=2, color='darkorange', label='10-point Rolling Average')
    ...
    def _create_comparative_analysis(self):
        ...
        fig.suptitle('Performance Requirements vs Actual Results', fontsize=14, fontweight='bold')
        ...
        for test_name, results in self.test_results.items():
            if isinstance(results, dict) and 'p99_ms' in results:
                ...
                elif 'end_to_end' in test_name.lower() or 'pipeline' in test_name.lower():
        ...
        actual_values = [req['actual'] if req['actual'] is not None else 0 for req in requirements.values()]

        x = np.arange(len(categories))
        width = 0.35

        bars1 = ax.bar(x - width/2, required_values, width, label='Required (ms)', alpha=0.8, color='red')
        bars2 = ax.bar(x + width/2, actual_values, width, label='Actual (ms)', alpha=0.8, color='green')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='URC 2026 Comprehensive Performance Testing')
    parser.add_argument('--duration', type=int, default=300,
                       help='Test duration in seconds (default: 300)')
    parser.add_argument('--chaos-level', type=float, default=0.5,
                       help='Chaos injection level 0.0-1.0 (default: 0.5)')
    parser.add_argument('--output-dir', type=str, default='perf_results',
                       help='Output directory for results and graphs (default: perf_results)')
    parser.add_argument('--quick', action='store_true',
                       help='Run quick test suite (reduced duration)')
    parser.add_argument('--no-chaos', action='store_true',
                       help='Disable chaos engineering tests')
    ...
    if 'cpu_stats' in analysis:
        ...
        print(f"  Mean: {cpu_stats['mean']:.1f}%, P95: {cpu_stats['p95']:.1f}%, Max: {cpu_stats['max']:.1f}%")
    if 'memory_stats' in analysis:
        mem_stats = analysis['memory_stats']
        print("🧠 Memory Performance:")
        print(f"  Mean: {mem_stats['mean_mb']:.1f}MB, P95: {mem_stats['p95_mb']:.1f}MB, Max: {mem_stats['max_mb']:.1f}MB")
