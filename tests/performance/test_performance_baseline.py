...
class PerformanceBaselineTester:
    ...
    def test_binary_protocol_baseline(self) -> Dict[str, Any]:
        ...
        result["json_comparison"] = {
            ...
            "improvement_factor": sorted(json_latencies)[
                int(len(json_latencies) * 0.99)
            ]
            / result["p99_ms"],
        }
    ...
    def _benchmark_json_protocol(
        self, imu_data: IMUData, iterations: int
    ) -> List[float]:
    ...
    def test_motion_control_baseline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"Motion control p99 latency: {result['p99_ms']:.3f}ms, violations: {result['violation_rate_percent']:.1f}%"
            )
            return result
    ...
    def run_stress_tests(self) -> Dict[str, Any]:
        ...
        stress_results["ipc_bridge_concurrent"] = (
            self._stress_test_ipc_bridge_concurrent()
        )
        ...
        try:
            ...
            def client_worker(thread_id):
                ...
                for i in range(operations_per_thread):
                    cmd = VelocityCommand(
                        linear_x=0.1 * thread_id, angular_z=0.05 * thread_id
                    )
    ...
    def _calculate_statistics(
        self, latencies: List[float], iterations: int
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "p999_ms": (
                latencies[int(len(latencies) * 0.999)]
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        for req in self.REQUIREMENTS:
            ...
            if "p99_ms" in result:
                ...
                compliant = (
                    result["violation_rate_percent"] <= req.max_violation_rate_percent
                )
        ...
        compliant_requirements = sum(
            1 for r in report["requirements"].values() if r["compliant"]
        )
        ...
        if not report["compliance"]["overall_pass"]:
            report["recommendations"].append(
                "Performance requirements not met - investigate bottlenecks"
            )

        # Check for significant improvements
        if (
            "binary_protocol" in self.results
            and "json_comparison" in self.results["binary_protocol"]
        ):
            improvement = self.results["binary_protocol"]["json_comparison"][
                "improvement_factor"
            ]
            if improvement > 3.0:
...
def test_binary_protocol_performance(performance_tester):
    ...
    assert (
        result["p99_ms"] < 1.0
    ), f"Binary protocol too slow: {result['p99_ms']:.3f}ms p99"
    assert (
        result["json_comparison"]["improvement_factor"] > 2.0
    ), "Insufficient improvement over JSON"
...
def test_sensor_timestamp_performance(performance_tester):
    ...
    assert (
        result["p99_ms"] < 1.0
    ), f"Timestamp provider too slow: {result['p99_ms']:.3f}ms p99"
...
def test_motion_control_deadlines(performance_tester):
    ...
    assert (
        result["violation_rate_percent"] < 1.0
    ), f"Too many deadline violations: {result['violation_rate_percent']:.1f}%"
...
def test_end_to_end_pipeline(performance_tester):
    ...
    assert (
        result["p99_ms"] < 5.0
    ), f"End-to-end pipeline too slow: {result['p99_ms']:.3f}ms p99"
...
def test_stress_concurrent_operations(performance_tester):
    ...
    assert (
        stress_result["p99_latency_ms"] < 10.0
    ), f"Concurrent ops too slow: {stress_result['p99_latency_ms']:.3f}ms"
...
def test_stress_memory_usage(performance_tester):
    ...
    assert not memory_result[
        "memory_leak_detected"
    ], f"Memory leak detected: +{memory_result['memory_increase_mb']:.1f}MB"
...
def test_stress_high_frequency(performance_tester):
    ...
    assert (
        hf_result["completion_rate_percent"] > 90.0
    ), f"Low completion rate: {hf_result['completion_rate_percent']:.1f}%"
...
if __name__ == "__main__":
    ...
    print(
        f"Passed: {compliance['compliant_requirements']}/{compliance['total_requirements']}"
    )
