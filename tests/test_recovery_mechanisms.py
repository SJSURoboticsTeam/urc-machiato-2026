...
class RecoveryMechanismsTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.original_cpu_affinity = self.process.cpu_affinity() if hasattr(self.process, 'cpu_affinity') else None
        ...
        try:
            ...
            for _ in range(int(self.recovery_timeout / self.measurement_interval)):
                ...
                if current_components < baseline_components:
                    print(f"✅ Recovery activated: {baseline_components} → {current_components} components")
                    break
            ...
            recovery_effective = (
                final_components < baseline_components or
                final_cpu < max(cpu for cpu, _ in pressure_measurements) or
                final_memory < max(memory for _, memory in pressure_measurements)
            )

            self.assertTrue(recovery_effective, "Recovery mechanism should reduce resource usage")

            # System should still be functional
            essential_status = final_status.get('component_status', {})
            self.assertIsInstance(essential_status, dict, "Component status should be accessible")

            print("✅ CPU pressure recovery test passed"
    ...
    def test_memory_pressure_recovery(self):
        ...
        try:
            ...
            # Get baseline
            baseline_components = len(resource_manager.get_resource_status().get('component_status', {}))
            baseline_cpu, baseline_memory = self._measure_resources()
            ...
            for _ in range(int(self.recovery_timeout / self.measurement_interval)):
                ...
                # Check for recovery
                current_components = len(resource_manager.get_resource_status().get('component_status', {}))

                if current_components < baseline_components:
                    recovery_detected = True
                    print(f"✅ Memory recovery activated: {baseline_components} → {current_components} components")
                    break
            ...
            self.assertLess(memory_growth, 200, "Memory growth should be controlled during recovery")

            # Either recovery activated or memory was managed
            recovery_effective = (
                recovery_detected or
                final_memory <= max_memory_during_pressure * 1.1  # Within 10% of peak
            )

            self.assertTrue(recovery_effective, "Memory recovery should prevent excessive growth")

            print("✅ Memory pressure recovery test passed"
    ...
    def test_combined_resource_pressure_recovery(self):
        ...
        try:
            ...
            resources_reduced = (
                final_cpu < max_cpu * 0.9 or  # At least 10% CPU reduction
                final_memory < max_memory * 0.9  # At least 10% memory reduction
            )

            recovery_effective = recovery_activated or resources_reduced

            self.assertTrue(recovery_effective, "Combined pressure recovery should activate")

            print("✅ Combined pressure recovery test passed"
    ...
    def test_recovery_performance_overhead(self):
        ...
        try:
            ...
            resource_manager.switch_mission_profile('waypoint_navigation')  # Minimal components
            ...
            print("✅ Recovery performance overhead test passed"
        ...
        try:
            ...
            for cycle in range(3):
                ...
                for _ in range(10):  # 5 seconds of monitoring
                    ...
                    components = len(resource_manager.get_resource_status().get('component_status', {}))
            ...
            for cycle_data in stability_measurements:
                cycle_cpus = [cpu for cpu, mem, comp in cycle_data if isinstance(cpu, (int, float))]
                cycle_mems = [mem for cpu, mem, comp in cycle_data if isinstance(mem, (int, float))]
                cycle_comps = [comp for cpu, mem, comp in cycle_data if isinstance(comp, int)]

                if cycle_cpus:
                    cpu_variability.append(statistics.stdev(cycle_cpus) / statistics.mean(cycle_cpus))
                if cycle_mems:
                    memory_variability.append(statistics.stdev(cycle_mems) / statistics.mean(cycle_mems))
                if cycle_comps:
                    component_variability.append(statistics.stdev(cycle_comps) / statistics.mean(cycle_comps))

            # System should stabilize (coefficient of variation < 50%)
            avg_cpu_cv = statistics.mean(cpu_variability) if cpu_variability else 0
            avg_memory_cv = statistics.mean(memory_variability) if memory_variability else 0

            self.assertLess(avg_cpu_cv, 0.5, "CPU usage should stabilize during recovery")
            self.assertLess(avg_memory_cv, 0.5, "Memory usage should stabilize during recovery")

            print("✅ Recovery stability test passed"
