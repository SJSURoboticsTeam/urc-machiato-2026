...
class TestSLAMPerformance:
        ...
        class PerformanceMonitor:
            ...
            def get_summary(self):
                ...
                return {
                    ...
                    'avg_frame_time_ms': statistics.mean(self.frame_times) * 1000 if self.frame_times else 0,
    ...
    def test_frame_processing_rate(self, slam_system, mock_camera_stream, performance_monitor):
        ...
        assert summary['fps'] >= target_fps_min, f"Frame rate too low: {summary['fps']:.1f} FPS (min: {target_fps_min})"
        assert summary['avg_frame_time_ms'] <= 66.7, f"Avg frame time too high: {summary['avg_frame_time_ms']:.1f}ms (target: 66.7ms for 15 FPS)"
        assert summary['max_frame_time_ms'] <= 150.0, f"Max frame time too high: {summary['max_frame_time_ms']:.1f}ms (target: 150ms)"
    ...
    def test_memory_usage_continuously(self, slam_system, performance_monitor):
        ...
        assert summary['max_memory_mb'] < 2048, f"Memory usage too high: {summary['max_memory_mb']:.1f} MB (limit: 2GB)"
        assert summary['avg_memory_mb'] < 1536, f"Average memory too high: {summary['avg_memory_mb']:.1f} MB (target: 1.5GB for Pi 5)"
    ...
    def test_feature_tracking_accuracy(self, slam_system, performance_monitor):
        ...
        for i in range(50):
            ...
            # Add known features
            cv2.rectangle(frame, (100 + i*2, 100), (200 + i*2, 200), (255, 255, 255), 2)
            cv2.circle(frame, (320, 240), 50, (128, 128, 128), -1)
            
            # Store with known transform
            frames_with_transforms.append(frame)
            true_transforms.append([i*0.01, i*0.005, i*0.001])  # Small translations
        ...
        assert min_confidence >= 0.3, f"Minimum confidence too low: {min_confidence:.2f}"
        ...
        if len(poses) > 1:
            ...
            assert avg_position_change > 0.001, "No position changes detected (tracking failed)"
    ...
    def test_pose_estimation_latency(self, slam_system):
        ...
        assert avg_latency <= 50.0, f"Average latency too high: {avg_latency:.1f}ms (target: 50ms for Pi 5)"
        assert max_latency <= 150.0, f"Maximum latency too high: {max_latency:.1f}ms (target: 150ms)"
        assert p95_latency <= 100.0, f"P95 latency too high: {p95_latency:.1f}ms (target: 100ms)"
    ...
    def test_slam_recovery_performance(self, slam_system):
        ...
        tracking_loss_scenarios = [
            "low_texture", "motion_blur", "lighting_change", "occlusion"
        ]
        ...
        for scenario in tracking_loss_scenarios:
            ...
            for i in range(5):
                ...
                assert result['confidence'] < 0.5, f"No tracking loss detected in {scenario}"
            ...
            assert confidence_recovered, f"Failed to recover from {scenario} tracking loss"
        ...
        assert avg_recovery_time <= 2.0, f"Average recovery too slow: {avg_recovery_time:.2f}s"
        assert max_recovery_time <= 5.0, f"Maximum recovery too slow: {max_recovery_time:.2f}s"
    ...
    def test_loop_closure_performance(self, slam_system):
        ...
        assert len(keyframe_events) >= 5, f"Too few keyframes detected: {len(keyframe_events)}"
        ...
        assert avg_processing_time <= 0.05, f"Processing too slow during loop closure: {avg_processing_time*1000:.1f}ms"
        assert max_processing_time <= 0.1, f"Max processing too slow: {max_processing_time*1000:.1f}ms"
    ...
    def _generate_frame_sequence(self, frame_id: int, total_frames: int) -> np.ndarray:
        ...
        cv2.rectangle(frame, (100 + x_offset, 100), (200 + x_offset, 200), (255, 255, 255), 2)
    ...
    def _generate_loop_frame(self, frame_id: int, path_type: str) -> np.ndarray:
        ...
        if frame_id == 0 or (path_type == "return" and frame_id == 29):
            ...
            cv2.putText(frame, "START", (250, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        
        # Add moving features
        cv2.rectangle(frame, (100 + x_offset, 100 + y_offset), 
                     (200 + x_offset, 200 + y_offset), (255, 255, 255), 2)
