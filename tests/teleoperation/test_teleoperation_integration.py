...
class MockTeleoperationPublisher(Node):
    ...
    def publish_mock_data(self):
        ...
        twist_stamped.twist.linear.y = math.sin(t * 0.2) * 0.1 + random.uniform(
            -0.02, 0.02
        )
        twist_stamped.twist.angular.z = math.cos(t * 0.4) * 0.2 + random.uniform(
            -0.05, 0.05
        )
        ...
        battery_state.voltage = (
            20.0 + (self.battery_level / 100.0) * 6.0
        )  # 20-26V range
