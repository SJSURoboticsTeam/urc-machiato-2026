...
class MockHardwareServer:
    ...
    def _initialize_mock_hardware(self):
        ...
        logger.info(
            f"Mock hardware initialized with {len(self.gpio_states)} GPIO pins, {len(self.encoder_counts)} encoders"
        )
    ...
    def _update_sensor_data(self):
        ...
        if random.random() < 0.1:  # 10% chance of GPS update
            ...
            gps_data["satellites"] = max(
                4, gps_data["satellites"] + random.randint(-1, 2)
            )
        ...
        battery_data["percentage"] = max(
            0.0, battery_data["percentage"] - random.gauss(0.01, 0.005)
        )
        battery_data["temperature"] = min(
            40.0, battery_data["temperature"] + random.gauss(0.05, 0.02)
        )
        self.sensor_data["battery"] = MockSensorData(
            "battery", battery_data, current_time
        )
    ...
    def _update_encoder_counts(self):
        ...
        for motor_name, command in self.motor_commands.items():
            ...
            if motor_name in self.encoder_counts:
                ...
                self.encoder_counts[motor_name] += int(
                    velocity_change * 0.1
                )  # 10Hz update
...
def get_system_status():
    ...
    status = {
        ...
        "sensor_data": {
            sensor_type: {"data": data.data, "timestamp": data.timestamp}
