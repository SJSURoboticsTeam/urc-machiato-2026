    ...
    def _test_transition_time_validation(self):
        ...
        try:
            ...
            transition_validation = {
                ...
                "p95_transition_time_ms": statistics.quantiles(
                    all_transition_times, n=20
                )[18],
                ...
                "violation_rate_percent": (
                    len([t for t in all_transition_times if t > 50.0])
                    / len(all_transition_times)
                )
                * 100,
                "meets_50ms_requirement": statistics.quantiles(
                    all_transition_times, n=20
                )[18]
                < 50.0,
            ...
            status = (
                "✅ PASS"
                if transition_validation["meets_50ms_requirement"]
                else "❌ FAIL"
            )
            print(".2f")
            sm.cleanup()
    ...
    def _test_state_consistency_checking(self):
        ...
        try:
            ...
            consistency_analysis = {
                ...
                "invalid_transitions_prevented": len(
                    [
                        r
                        for r in consistency_results
                        if not r["expected_valid"] and not r["actual_valid"]
                    ]
                ),
                "valid_transitions_allowed": len(
                    [
                        r
                        for r in consistency_results
                        if r["expected_valid"] and r["actual_valid"]
                    ]
                ),
                "false_positives": len(
                    [
                        r
                        for r in consistency_results
                        if not r["expected_valid"] and r["actual_valid"]
                    ]
                ),
                "false_negatives": len(
                    [
                        r
                        for r in consistency_results
                        if r["expected_valid"] and not r["actual_valid"]
                    ]
                ),
                "average_validation_time_ms": statistics.mean(
                    [r["transition_time_ms"] for r in consistency_results]
                ),
            ...
            status = (
                "✅ PASS"
                if consistency_analysis["meets_consistency_requirement"]
                else "❌ FAIL"
            )
            print(".1f")
            sm.cleanup()
        ...
        try:
            ...
            for thread_id in range(5):  # 5 concurrent threads
                thread = threading.Thread(
                    target=worker_thread, args=(thread_id, thread_results)
                )
            ...
            thread_safety_analysis = {
                ...
                "error_rate_percent": (
                    (len(errors) / total_transitions) * 100
                    if total_transitions > 0
                    else 0
                ),
                "average_transition_time_ms": statistics.mean(
                    [r["time_ms"] for r in thread_results if r["success"]]
                ),
                "max_transition_time_ms": max(
                    [r["time_ms"] for r in thread_results if r["success"]] or [0]
                ),
                "thread_safety_score": self._calculate_thread_safety_score(
                    thread_results
                ),
                "meets_thread_safety_requirement": len(errors) == 0
                and successful_transitions > total_transitions * 0.95,
                "error_details": errors[:10] if errors else [],  # First 10 errors
            }

            self.test_results["concurrent_access_thread_safety"] = (
                thread_safety_analysis
            )

            status = (
                "✅ PASS"
                if thread_safety_analysis["meets_thread_safety_requirement"]
                else "❌ FAIL"
            )
            print(".1f")
            sm.cleanup()
    ...
    def _test_persistence_and_recovery(self):
        ...
        try:
            ...
            try:
                ...
                data_restored = all(
                    sm2.state_data.get(k) == v for k, v in test_data.items()
                )
                ...
                status = (
                    "✅ PASS"
                    if persistence_analysis["meets_persistence_requirement"]
                    else "❌ FAIL"
                )
                print(".1f")
                sm1.cleanup()
    ...
    def _test_transition_success_rate_monitoring(self):
        ...
        try:
            ...
            success_rate_monitoring = {
                ...
                "meets_success_rate_requirement": (success_count / total_count)
            ...
            self.test_results["transition_success_rate_monitoring"] = (
                success_rate_monitoring
            )

            status = (
                "✅ PASS"
                if success_rate_monitoring["meets_success_rate_requirement"]
                else "❌ FAIL"
            )
            print(".1f")
            sm.cleanup()
        ...
        if "state_consistency_checking" in self.test_results:
            ...
            if isinstance(consistency, dict):
                ...
                analysis["consistency_validation"] = {
                    ...
                    "invalid_transitions_prevented": consistency.get(
                        "invalid_transitions_prevented", 0
                    ),
            ...
            if isinstance(timing, dict):
                if not timing.get("meets_50ms_requirement", False):
                    recommendations.append(
                        "Optimize transition times to meet <50ms requirement"
                    )
                if timing.get("violation_rate_percent", 100) > 5:
                    recommendations.append(
                        "Reduce transition time violations through optimization"
                    )
        ...
        if "state_consistency_checking" in self.test_results:
            ...
            if isinstance(consistency, dict):
                if consistency.get("accuracy_percent", 0) < 95:
                    recommendations.append(
                        "Improve state consistency validation accuracy"
                    )
                if consistency.get("false_positives", 0) > 0:
                    recommendations.append(
                        "Fix false positive state transition validations"
                    )
        ...
        if "concurrent_access_thread_safety" in self.test_results:
            ...
            if isinstance(thread_safety, dict):
                if not thread_safety.get("meets_thread_safety_requirement", False):
                    recommendations.append(
                        "Implement proper thread synchronization for state transitions"
                    )
                if thread_safety.get("error_rate_percent", 100) > 1:
                    recommendations.append(
                        "Reduce concurrent access errors through better locking"
                    )
        ...
        if "persistence_and_recovery" in self.test_results:
            ...
            if isinstance(persistence, dict):
                if not persistence.get("meets_persistence_requirement", False):
                    recommendations.append(
                        "Fix state persistence and recovery mechanisms"
                    )
