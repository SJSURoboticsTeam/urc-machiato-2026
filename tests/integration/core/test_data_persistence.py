...
class TestDataManager:
    ...
    def test_data_caching(self, data_manager, sample_data):
        ...
        data_manager.store_data(
            "telemetry_data", sample_data["telemetry_data"], cache=True
        )
    ...
    def test_concurrent_data_access(self, data_manager, sample_data):
        ...
        async def concurrent_operation(operation_id: int):
            ...
            # Retrieve data
            retrieved = await data_manager.get_data_async(
                f"concurrent_data_{operation_id}"
            )
    ...
    def test_data_querying_and_filtering(self, data_manager, sample_data):
        ...
        results = data_manager.query_data(
            "telemetry_data", filters={"battery.voltage": {"$gt": 22.0}}
        )
        ...
        sorted_results = data_manager.query_data(
            "telemetry_data", sort_by="sequence_id", ascending=True
        )
    ...
    def test_data_encryption(self, data_manager):
        ...
        sensitive_data = {
            "credentials": {"api_key": "secret_key_123", "password": "super_secret"},
            "personal_data": {
    ...
    def test_data_retention_policies(self, data_manager, sample_data):
        ...
        data_manager.store_data(
            "short_term", sample_data["telemetry_data"], retention_days=1
        )
        data_manager.store_data(
            "long_term", sample_data["mission_data"], retention_days=365
        )
...
@pytest.mark.skipif(DataCache is None, reason="DataCache not in data_manager")
class TestDataCache:
...
@pytest.mark.skipif(BackupManager is None, reason="BackupManager not in data_manager")
