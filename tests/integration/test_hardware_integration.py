...
class TestCANBusCommunication:
    ...
    def test_can_message_formatting(self, can_interface_config):
        ...
        current_bytes = struct.pack('<H', int(current_limit * 100))  # Scale to centi-amps
    ...
    def test_can_message_filtering(self, can_interface_config):
        ...
        message_filters = {
            "motor_control": lambda msg: msg.arbitration_id >= 0x200 and msg.arbitration_id <= 0x20F,
            "sensor_data": lambda msg: msg.arbitration_id >= 0x300 and msg.arbitration_id <= 0x30F,
        ...
        test_messages = [
            ...
            Mock(arbitration_id=0x301, data=[10, 20, 30, 40, 50, 60, 70, 80]),  # Sensor data
    ...
    def test_can_bus_reliability(self, can_interface_config):
        ...
        for scenario in error_scenarios:
            ...
            assert recovery_time >= scenario["recovery_time"] * 0.05  # Minimum recovery time
...
class TestMotorControlIntegration:
    ...
    def test_motor_feedback_processing(self, motor_config):
        ...
        # Pack feedback data
        feedback_data = struct.pack('<HhHBB', actual_rpm, int(current_draw * 100),
                                  int(temperature * 10), motor_id, error_flags)
...
class TestSensorDataProcessing:
    ...
    def sensor_config(self):
        ...
        return {
            ...
            "gps": {
                "rate": 10,  # Hz
                "accuracy": {"horizontal": 2.5, "vertical": 5.0}
            },
            "lidar": {
                "rate": 20,  # Hz
                "range": 30.0,  # meters
    ...
    def test_imu_data_validation(self, sensor_config):
        ...
        test_readings = [
            ...
            {"accel": [15.5, -8.2, 10.1], "gyro": [1999.0, 1500.0, -1800.0], "temp": 45.0},
            {"accel": [0.0, 0.0, 0.0], "gyro": [0.0, 0.0, 0.0], "temp": 0.0}  # Invalid reading
        ]

        for reading in test_readings:
            # Validate accelerometer range
            for accel_val in reading["accel"]:
                assert abs(accel_val) <= imu_config["ranges"]["accel"] * 1.1  # Allow 10% margin
...
class TestEmergencySystems:
    ...
    def test_emergency_detection(self, emergency_config):
        ...
        # Check motor current
        if system_state["motor_current"] > emergency_conditions["motor_overcurrent"]["threshold"]:
        ...
        # Check battery level
        if system_state["battery_level"] < emergency_conditions["low_battery"]["threshold"]:
            detected_emergencies.append({
                "type": "low_battery",
    ...
    def test_safety_interlock_systems(self, emergency_config):
        ...
        def check_interlocks(action):
            if action in interlocks:
                return all(system_status.get(condition, False)
...
class TestHardwareSoftwareIntegration:
    ...
    def test_control_loop_timing(self, integration_config):
        ...
        assert jitter <= control_config["jitter_tolerance"] / 1000.0  # Convert to seconds
