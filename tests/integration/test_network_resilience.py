...
class TestNetworkResilience:
    ...
    async def test_packet_loss_resilience(self, network_emulator, resilience_manager):
        ...
        try:
            ...
            print(f"📡 Packet loss test: {success_rate:.1%} success rate under 10% loss")
    ...
    async def test_connection_drop_recovery(self, network_emulator, resilience_manager):
        ...
        try:
            ...
            print(f"🔌 Connection drop test: {successful_attempts}/{len(recovery_attempts)} successful")
    ...
    async def test_connection_pooling_efficiency(self, resilience_manager):
        ...
        with patch('aiohttp.ClientSession', side_effect=mock_session):
            ...
            tasks = []
            for _ in range(10):
                task = resilience_manager.execute_with_resilience(self._make_test_request)
                tasks.append(task)

            await asyncio.gather(*tasks)

            print(f"🔗 Connection pooling: {pool_stats['connections_created']} connections created")

            # Should reuse connections (fewer than requests)
            assert pool_stats["connections_created"] < 10

    @pytest.mark.asyncio
    async def test_network_partition_handling(self, network_emulator, resilience_manager):
        ...
        print(f"🌐 Network partition test: {failed_during_partition}/5 requests failed during partition")

        # Most requests during partition should fail
        assert failed_during_partition >= 3

    @pytest.mark.asyncio
    async def test_bandwidth_throttling_resilience(self, network_emulator, resilience_manager):
        ...
        try:
            ...
            result = await resilience_manager.execute_with_timeout(
                large_data_request,
                timeout=5.0  # Generous timeout for slow connection
            )
            elapsed = time.time() - start_time
    ...
    async def test_service_discovery_resilience(self, resilience_manager):
        ...
        result = await resilience_manager.execute_with_resilience(
            ...
            max_attempts=len(service_endpoints)
        )

        assert "tertiary" in result
        print("🔍 Service discovery resilience working - failed over to tertiary service")

    @pytest.mark.asyncio
    async def test_websocket_connection_resilience(self, network_emulator, resilience_manager):
        ...
        try:
            ...
            successful_connections = sum(1 for a in reconnection_attempts if a["success"])

            print(f"🔌 WebSocket resilience: {successful_connections}/{len(reconnection_attempts)} successful connections")
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_network_resilience(self, network_emulator, resilience_manager):
        ...
        for scenario in test_scenarios:
            ...
            try:
                ...
                overall_results.append({
                    ...
                    "met_expectations": success_rate >= scenario["expected_success_rate"]
                })

                print(f"🌐 {scenario['name']}: {success_rate:.1%} success (expected {scenario['expected_success_rate']:.1%})")
        ...
        print(f"🎯 End-to-end resilience: {scenarios_met}/{len(overall_results)} scenarios met expectations")
