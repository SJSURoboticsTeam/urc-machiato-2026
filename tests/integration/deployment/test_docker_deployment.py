...
"""
    ...
    def test_docker_compose_orchestration(self, temp_docker_dir):
        ...
        # Clean up
        subprocess.run(
            ["docker-compose", "down"], cwd=temp_docker_dir, capture_output=True
        )
    ...
    def test_environment_configuration_in_containers(self, temp_docker_dir):
        ...
        result = subprocess.run(
            [
                ...
                "python3",
                "-c",
                "import os; print(f'ROS_DOMAIN_ID={os.environ.get(\"ROS_DOMAIN_ID\")}'); print(f'ROVER_MODE={os.environ.get(\"ROVER_MODE\")}')",
            ],
            cwd=temp_docker_dir,
            capture_output=True,
            text=True,
    ...
    def test_docker_security_scanning(self, temp_docker_dir):
        ...
        assert (
            "ros-jazzy" not in result.stdout
        )  # Should not expose internal package names
    ...
    def test_container_health_checks(self, temp_docker_dir):
        ...
        assert health_status in [
            "healthy",
            "starting",
        ]  # Should be healthy or still starting
...
"""
    ...
    def test_docker_layer_caching(self, temp_docker_dir):
        ...
        assert result.returncode == 0
        assert (
            second_build_time < first_build_time * 0.8
        )  # Should be significantly faster
        print(
            f"✅ Layer caching working: {first_build_time:.1f}s -> {second_build_time:.1f}s"
        )
