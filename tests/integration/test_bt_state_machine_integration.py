...
class TestBTStateMachineIntegration:
    ...
    def test_bt_queries_state_machine(self, mock_state_machine_service):
        ...
        try:
            ...
            assert "/adaptive_state_machine/get_state" in content, "BT should call correct service"
    ...
    def test_bt_state_machine_service_call_pattern(self):
        ...
        try:
            ...
            # Check for proper service client creation
            assert "create_client<GetSystemState>" in content, "BT should create GetSystemState client"
    ...
    def test_blackboard_input_sources(self):
        ...
        try:
            ...
            # Check that callbacks update blackboard
            assert 'blackboard_->set("robot_x"' in content, "Odom callback should update robot_x"
            assert 'blackboard_->set("slam_x"' in content, "SLAM callback should update slam_x"
    ...
    def test_bt_mission_execution_flow(self):
        ...
        try:
            ...
            assert "tree_->tick()" in content, "BT should execute tree ticks"
            assert "NodeStatus::SUCCESS" in content, "BT should handle success status"
    ...
    async def test_bt_telemetry_integration(self):
        ...
        try:
            ...
            assert "telemetry_publisher_" in content, "BT should have telemetry publisher"
            assert "publish_bt_telemetry" in content, "BT should publish telemetry"
            assert "samples_collected" in content, "Telemetry should include mission progress"
                ...
                if file.endswith('.py') or file.endswith('.cpp'):
                    try:
                        with open(os.path.join(root, file), 'r') as f:
                            content = f.read()
                            if 'adaptive_state_machine' in content and 'get_state' in content:
                                service_exists = True
                                break
                    except:
                        continue
...
class TestBlackboardCommunication:
    ...
    def test_blackboard_thread_safety(self):
        ...
        try:
            ...
            has_thread_safety = any(indicator in content.lower() for indicator in thread_safe_indicators)
    ...
    def test_blackboard_persistence(self):
        ...
        try:
            ...
            assert "BT::Blackboard::Ptr blackboard_;" in content, "Blackboard should be persistent member"

            # Check that values are retained between operations
            assert "set_blackboard_value" in content, "BT should set persistent values"
            assert "get_blackboard_value" in content, "BT should retrieve persistent values"
