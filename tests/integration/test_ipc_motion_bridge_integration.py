...
from src.motion.ipc_motion_bridge import (
    IpcMotionBridge, VelocityCommand, MotionControlStatus,
    MotionControlState, create_motion_bridge_server, create_motion_bridge_client
)
...
class TestIpcMotionBridgeIntegration:
    ...
    def test_bridge_performance_latency(self):
        ...
        try:
            ...
            for i in range(iterations):
                ...
                # Record in profiler
                self.performance_profiler.record_measurement("ipc_command_latency", latency_ms)
    ...
    def test_bridge_state_update_performance(self):
        ...
        server_bridge = create_motion_bridge_server(f"{self.shared_memory_name}_state_perf")
        client_bridge = create_motion_bridge_client(f"{self.shared_memory_name}_state_perf")
        ...
        try:
            ...
            for i in range(iterations):
                ...
                self.performance_profiler.record_measurement("ipc_state_latency", latency_ms)
    ...
    def test_concurrent_bridge_access(self):
        ...
        server_bridge = create_motion_bridge_server(f"{self.shared_memory_name}_concurrent")
        client_bridge = create_motion_bridge_client(f"{self.shared_memory_name}_concurrent")
        ...
        try:
            ...
            def client_worker(worker_id):
                ...
                try:
                    for i in range(100):
                        command = VelocityCommand(
                            linear_x=0.1 * worker_id,
                            angular_z=0.05 * worker_id
                        )
                        success = client_bridge.send_velocity_command(command)
                        if not success:
            ...
            assert len(results) == 3, f"Expected 3 successful completions, got {len(results)}"
    ...
    def test_motion_control_deadline_compliance(self):
        ...
        server_bridge = create_motion_bridge_server(f"{self.shared_memory_name}_deadline")
        client_bridge = create_motion_bridge_client(f"{self.shared_memory_name}_deadline")
        ...
        try:
            ...
            # Assert requirements
            assert p99_latency < target_period_ms, f"p99 latency {p99_latency:.3f}ms exceeds deadline {target_period_ms}ms"
            assert deadline_miss_rate < 1.0, f"Deadline miss rate {deadline_miss_rate:.1f}% too high (target <1%)"
    ...
    def test_performance_profiling_integration(self):
        ...
        server_bridge = create_motion_bridge_server(f"{self.shared_memory_name}_profile")
        client_bridge = create_motion_bridge_client(f"{self.shared_memory_name}_profile")
        ...
        try:
            ...
            # Measure operations
            self.performance_profiler.measure_latency("ipc_command_roundtrip", profiled_command)
            self.performance_profiler.measure_latency("ipc_state_roundtrip", profiled_state)
