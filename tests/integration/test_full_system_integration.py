...
class FullSystemIntegrationTest:
    ...
    def test_full_system_startup(self):
        ...
        if state_started:
            assert self.check_node_exists("/adaptive_state_machine") or "state_machine" in subprocess.run(
                ["ros2", "node", "list"], capture_output=True, text=True
            ).stdout.lower(), "State machine not found"
    ...
    def test_data_availability_state_machine(self):
        ...
        if topic_available:
            echo_result = subprocess.run(
                ["timeout", "2", "ros2", "topic", "echo", "/adaptive_state_machine/state", "--once"],
                capture_output=True,
                text=True,
                timeout=3
            )
    ...
    def test_consistency_check(self):
        ...
        # Check nodes
        consistency_report['nodes']['bt_orchestrator'] = self.check_node_exists("/bt_orchestrator")
        if state_started:
            consistency_report['nodes']['state_machine'] = (
                self.check_node_exists("/adaptive_state_machine") or 
                "state_machine" in subprocess.run(
                    ["ros2", "node", "list"], capture_output=True, text=True
                ).stdout.lower()
            )
        
        # Check critical topics
        consistency_report['topics']['/bt/telemetry'] = self.check_topic_exists("/bt/telemetry")
        consistency_report['topics']['/adaptive_state_machine/state'] = self.check_topic_exists("/adaptive_state_machine/state")
        consistency_report['topics']['/odom'] = self.check_topic_exists("/odom")
        consistency_report['topics']['/slam/pose'] = self.check_topic_exists("/slam/pose")
        
        # Check critical services
        consistency_report['services']['/bt_orchestrator/get_state'] = self.check_service_exists("/bt_orchestrator/get_state")
        consistency_report['services']['/adaptive_state_machine/get_state'] = self.check_service_exists("/adaptive_state_machine/get_state")
        ...
        assert consistency_report['topics']['/bt/telemetry'], "BT telemetry topic not found"
        assert consistency_report['services']['/bt_orchestrator/get_state'], "BT lifecycle service not found"
        
        if state_started:
            assert consistency_report['topics']['/adaptive_state_machine/state'], "State machine state topic not found"
            assert consistency_report['services']['/adaptive_state_machine/get_state'], "State machine service not found"
