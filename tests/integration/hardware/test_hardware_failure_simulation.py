...
class HardwareFailureSimulator:
    ...
    def simulate_motor_failure(self, motor_id: str, failure_type: str = "stall"):
        ...
        failure = {
            ...
            "timestamp": time.time(),
            "severity": (
                "critical" if failure_type in ["stall", "overheat"] else "warning"
            ),
        }
...
class TestHardwareFailureSimulation:
    ...
    def test_motor_failure_simulation(
        self, failure_simulator, mock_hardware_controller
    ):
        """Test motor failure simulation and handling."""
        # Register motor failure handler
        failure_simulator.register_failure_handler(
            "motor", mock_hardware_controller.handle_motor_failure
        )
    ...
    def test_sensor_failure_simulation(
        self, failure_simulator, mock_hardware_controller
    ):
        """Test sensor failure simulation and recovery."""
        # Register sensor failure handler
        failure_simulator.register_failure_handler(
            "sensor", mock_hardware_controller.handle_sensor_failure
        )
    ...
    def test_can_bus_failure_simulation(
        self, failure_simulator, mock_hardware_controller
    ):
        """Test CAN bus failure and communication recovery."""
        # Register CAN failure handler
        failure_simulator.register_failure_handler(
            "can_bus", mock_hardware_controller.handle_can_failure
        )
    ...
    def test_power_failure_simulation(
        ...
        failure_simulator.register_failure_handler(
            "power", mock_hardware_controller.switch_to_backup_power
        )
    ...
    def test_emergency_stop_simulation(
        ...
        failure_simulator.register_failure_handler(
            "emergency_stop", mock_hardware_controller.emergency_stop
        )
    ...
    @pytest.mark.asyncio
    async def test_concurrent_hardware_failures(
        self, failure_simulator, mock_hardware_controller
    ):
    ...
    @pytest.mark.asyncio
    async def test_hardware_hot_swapping(
        self, failure_simulator, mock_hardware_controller
    ):
    ...
    def test_failure_severity_classification(self, failure_simulator):
        ...
        critical_count = sum(
            1 for f in active_failures.values() if f["severity"] == "critical"
        )
        warning_count = sum(
            1 for f in active_failures.values() if f["severity"] == "warning"
        )
    ...
    async def test_hardware_diagnostic_capabilities(self, failure_simulator):
        ...
        for failure in failures:
            ...
            if failure["component"] == "sensor":
                failure_key = (
                    f"{failure['component']}_{failure['type']}_{failure['id']}"
                )
    ...
    def test_failure_pattern_analysis(self, failure_simulator):
        ...
        # Analyze patterns
        motor_failures = [
            f for f in active_failures.values() if f["component"] == "motor"
        ]
        sensor_failures = [
            f for f in active_failures.values() if f["component"] == "sensor"
        ]
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_hardware_failure_scenario(
        self, failure_simulator, mock_hardware_controller
    ):
        ...
        emergency_active = any(
            f["component"] == "emergency_stop" for f in active_failures.values()
        )
    ...
    @pytest.mark.asyncio
    async def test_hardware_redundancy_activation(
        self, failure_simulator, mock_hardware_controller
    ):
