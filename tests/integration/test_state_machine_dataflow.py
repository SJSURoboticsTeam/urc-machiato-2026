...
try:
    ...
    pytest.skip(f"Skipping state machine tests due to import error: {e}", allow_module_level=True)
...
class MockStateMachine(Node):
    ...
    def __init__(self, initial_state: SystemState = SystemState.IDLE):
        ...
        self.state_publisher = self.create_publisher(String, '/state_machine/current', 10)
        self.transition_publisher = self.create_publisher(String, '/state_machine/transition', 10)
    ...
    def is_valid_transition(self, from_state: SystemState, to_state: SystemState) -> bool:
        ...
        valid_transitions = {
            ...
            SystemState.AUTONOMOUS: [SystemState.IDLE, SystemState.TELEOPERATION, SystemState.EMERGENCY_STOP],
            SystemState.TELEOPERATION: [SystemState.IDLE, SystemState.AUTONOMOUS, SystemState.EMERGENCY_STOP],
            SystemState.EMERGENCY_STOP: [SystemState.IDLE, SystemState.TELEOPERATION],
            SystemState.MISSION_PAUSED: [SystemState.AUTONOMOUS, SystemState.IDLE, SystemState.EMERGENCY_STOP],
...
class StateMachineMonitor:
    ...
    def record_transition_data(self, transition_id: str, from_state: SystemState, 
                             to_state: SystemState, snapshot: StateDataSnapshot):
    ...
    def complete_transition_monitoring(self, transition_id: str) -> StateTransition:
        ...
        if measurement['data_before'] and measurement['data_after']:
            consistency = measurement['data_before'].compare_consistency(measurement['data_after'])
    ...
    def capture_data_snapshot(self, state: SystemState, blackboard: MockBlackboard, 
                           bt_orchestrator: MockBTOrchestrator) -> StateDataSnapshot:
...
class TestStateMachineCoordination:
    ...
    def test_state_transition_data_consistency(
        ...
        assert state_machine.current_state == SystemState.AUTONOMOUS, "Should be in AUTONOMOUS state"
        ...
        assert transition.to_state == SystemState.AUTONOMOUS, "Should transition to AUTONOMOUS"
        assert transition.transition_time < 0.050, f"Transition took {transition.transition_time*1000:.1f}ms, required < 50ms"
        
        # Verify BT status updated appropriately
        assert bt_orchestrator.status['running'], "BT should be running in AUTONOMOUS state"
    ...
    def test_concurrent_state_changes(
        ...
        assert len(transitions) <= len(concurrent_requests), f"Too many transitions: {len(transitions)}"
        
        # Current state should be valid
        valid_states = [state for state, _ in concurrent_requests]
        assert state_machine.current_state in valid_states, f"Invalid final state: {state_machine.current_state}"
        ...
        assert final_snapshot.state == state_machine.current_state, "State snapshot mismatch"
        
        print(f"✅ Concurrent state changes handled: {state_machine.current_state.value}")
    ...
    def test_state_machine_blackboard_sync(
        ...
        for key, expected_value in test_data.items():
            ...
            assert actual_value == expected_value, f"Blackboard data {key} changed: {actual_value} != {expected_value}"
        ...
        for key, expected_value in all_data.items():
            actual_value = blackboard.get(key)
            assert actual_value == expected_value, f"Data {key} lost in transition: {actual_value} != {expected_value}"
    ...
    def test_emergency_state_transition_priority(
        ...
        emergency_success = state_machine.transition_to(SystemState.EMERGENCY_STOP, "emergency_trigger")
        ...
        assert state_machine.current_state == SystemState.EMERGENCY_STOP, "Should be in EMERGENCY_STOP state"
        
        # Verify BT stopped in emergency
        assert not bt_orchestrator.status['running'], "BT should be stopped in emergency"
        assert bt_orchestrator.status['tree_status'] == 'STOPPED', "BT status should be STOPPED"
        ...
        assert emergency_snapshot.state == SystemState.EMERGENCY_STOP, "Emergency state not recorded"
        assert not emergency_snapshot.bt_status['running'], "BT running status inconsistent"
    ...
    def test_state_transition_error_handling(
        ...
        invalid_transitions = [
            (SystemState.EMERGENCY_STOP, SystemState.AUTONOMOUS),  # Emergency to autonomous not allowed
            (SystemState.SYSTEM_ERROR, SystemState.AUTONOMOUS),   # Error to autonomous not allowed
        ]
        ...
        for from_state, to_state in invalid_transitions:
            ...
            assert not success, f"Invalid transition {from_state.value} -> {to_state.value} should fail"
            assert state_machine.current_state == from_state, "State should not change on invalid transition"
        ...
        for from_state, to_state in error_recovery_transitions:
            ...
            assert success, f"Valid transition {from_state.value} -> {to_state.value} should succeed"
            assert state_machine.current_state == to_state, "State should change on valid transition"
    ...
    def test_state_transition_performance(
        ...
        for scenario_name, load_level in load_scenarios:
            ...
            start_time = time.time()
            success = state_machine.transition_to(SystemState.AUTONOMOUS, f"test_{scenario_name}")
            ...
            assert transition_time < 0.100, f"Transition under {scenario_name} took {transition_time*1000:.1f}ms, required < 100ms"
...
class TestStateMachineIntegration:
    ...
    def test_state_machine_bt_coordination(self, ros_context, state_machine, bt_orchestrator):
        ...
        for target_state, (expected_status, expected_running) in state_bt_mappings.items():
            # Transition to state
            ...
            assert bt_orchestrator.status['running'] == expected_running, \
    ...
    def test_state_transition_data_propagation(self, ros_context, blackboard, state_machine):
        ...
        for key, expected_value in transition_data.items():
            actual_value = blackboard.get(key)
            assert actual_value == expected_value, f"Data {key} not propagated: {actual_value} != {expected_value}"
            
        # Verify transition metadata was added
        assert "last_transition_time" in blackboard.data, "Transition time not recorded"
        assert "last_transition_state" in blackboard.data, "Transition state not recorded"
