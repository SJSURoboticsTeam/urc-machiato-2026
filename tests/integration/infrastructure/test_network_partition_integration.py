...
class TestNetworkPartitionIntegration:
    ...
    def test_network_partition_detection(self):
        ...
        with patch.object(
            self.detector, "_check_wifi_connected", return_value=True
        ), patch.object(
            self.detector, "_check_internet_reachable", return_value=(False, None)
        ):
    ...
    def test_network_recovery(self):
        ...
        with patch.object(
            ...
            self.detector, "_check_internet_reachable", return_value=(True, 25.0)
        ):
    ...
    def test_network_health_checks(self):
        ...
        with patch.object(
            ...
            self.detector, "_check_internet_reachable", return_value=(True, 15.0)
        ), patch.object(
            self.detector, "_get_wifi_signal_strength", return_value=-45
        ):
        ...
        with patch.object(
            ...
            self.detector, "_check_internet_reachable", return_value=(True, 150.0)
        ), patch.object(
            self.detector, "_get_wifi_signal_strength", return_value=-85
        ):
    ...
    def test_degraded_network_state(self):
        ...
        with patch.object(
            ...
            self.detector, "_check_internet_reachable", return_value=(True, 80.0)
        ):

            time.sleep(2.0)
            status = self.detector.get_network_status()
            # Should be connected but marked as degraded due to high latency
            assert status["network_state"] in [
                NetworkState.CONNECTED.value,
                NetworkState.DEGRADED.value,
            ]
    ...
    def test_partition_duration_tracking(self):
        ...
        with patch.object(
            ...
            )  # Should have been partitioned for > 2 seconds
    ...
    def test_concurrent_callback_handling(self):
        ...
        with patch.object(
            ...
            partition_callbacks = [
                r for r in callback_results if "partition_detected" in r
            ]
    ...
    def test_error_recovery(self):
        ...
        with patch.object(
            self.detector, "_perform_health_check", side_effect=Exception("Test error")
        ):
