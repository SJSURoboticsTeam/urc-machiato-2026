...
class MockWebSocketManager(Node):
    ...
    def connect_client(self, client_id: str) -> bool:
        ...
        if (
            np.random.random()
            < self.network_emulator.current_condition.connection_stability
        ):
...
class MockTelemetryManager(Node):
    ...
    def send_telemetry(self, data: Dict, client_id: str = "dashboard") -> bool:
        """Send telemetry data"""
        message = {"type": "telemetry", "data": data, "timestamp": time.time()}
...
class NetworkResilienceMonitor:
    ...
    def record_recovery_event(
        self, condition: str, recovery_time: float, strategies_used: List[str]
    ):
...
class TestNetworkResilienceDataflow:
    ...
    def test_high_latency_data_flow(
        ...
        ), "Should receive messages despite high latency"

        received_data = received_messages[-1].get("data", {})
        assert (
            received_data["position"] == test_data["position"]
        ), "Position data corrupted"
        assert (
            received_data["battery_level"] == test_data["battery_level"]
        ), "Battery data corrupted"

        # Record test result
        performance = resilience_manager.monitor_network_performance()
        data_integrity = {
            "position_integrity": received_data.get("position")
            == test_data["position"],
            "battery_integrity": received_data.get("battery_level")
            == test_data["battery_level"],
        }

        network_monitor.record_test_result(
            high_latency_condition, True, performance, data_integrity
        )
    ...
    def test_packet_loss_data_recovery(
        ...
        assert (
            received_count >= expected_received - 2
        ), f"Too few messages received: {received_count} vs expected {expected_received}"
        ...
        network_monitor.record_test_result(
            packet_loss_condition, received_count > 5, performance, data_integrity
        )
    ...
    def test_network_partition_recovery(
        ...
        assert (
            len(received_messages) == 0
        ), "Should receive no messages during partition"
        ...
        network_monitor.record_recovery_event(
            "partition", recovery_time, ["reconnection", "message_retransmission"]
        )
    ...
    def test_adaptive_strategies(
        ...
        test_conditions = [
            (
                NetworkCondition("good_network", bandwidth_limit_mbps=100.0),
                ["retransmission"],
            ),
            (
                NetworkCondition("slow_network", bandwidth_limit_mbps=0.5),
                ["retransmission", "compression"],
            ),
            (
                NetworkCondition("lossy_network", packet_loss_rate=0.2),
                ["retransmission", "redundancy"],
            ),
            (
                NetworkCondition(
                    "poor_network", packet_loss_rate=0.3, bandwidth_limit_mbps=0.5
                ),
        ...
        for condition, expected_strategies in test_conditions:
            ...
            for strategy in expected_strategies:
                assert current_strategies.get(
                    strategy, False
                ), f"Strategy {strategy} should be enabled for {condition.name}"
            ...
            if condition.packet_loss_rate < 1.0:
                assert (
                    len(received_messages) > 0
                ), f"Data should get through with strategies for {condition.name}"
    ...
    def test_bandwidth_limit_handling(
        ...
        for bandwidth_mbps, test_name in bandwidth_tests:
            ...
            condition = NetworkCondition(
                name=f"bandwidth_{test_name}", bandwidth_limit_mbps=bandwidth_mbps
            )
            network_emulator.set_condition(condition)
            network_emulator.reset_metrics()
            ...
            if bandwidth_mbps < 100:
                assert (
                    actual_throughput <= bandwidth_mbps * 1.2
                ), f"Throughput {actual_throughput:.2f} exceeds limit {bandwidth_mbps}"

            # Verify some data got through
            assert messages_received > 0, f"No messages received for {test_name}"

            print(
                f"  Throughput: {actual_throughput:.2f} Mbps, Messages: {messages_received}/{messages_sent}"
            )
    ...
    def test_concurrent_connections_under_stress(
        ...
        assert (
            len(clients) >= num_clients * 0.7
        ), f"Too few clients connected: {len(clients)}/{num_clients}"
        ...
        delivery_rate = (
            total_received / total_messages_sent if total_messages_sent > 0 else 0
        )

        assert delivery_rate > 0.5, f"Delivery rate too low: {delivery_rate:.2f}"
        assert (
            len(received_by_client) >= len(clients) * 0.7
        ), "Too few clients received messages"

        print(
            f"✅ Concurrent connections: {total_received}/{total_messages_sent} messages, {delivery_rate:.2f} rate"
        )
        ...
        network_monitor.record_test_result(
            stress_condition, delivery_rate > 0.5, performance, data_integrity
        )
