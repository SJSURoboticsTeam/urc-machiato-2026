...
from simulation.integration.full_stack_simulator import (
    FullStackSimulator, ScenarioType, create_full_stack_simulator
)
...
class TestCompleteCommunicationStack:
    ...
    async def test_emergency_stop_propagation(self, perfect_simulator):
        ...
        assert result.duration_s < 0.1, f"E-stop too slow: {result.duration_s*1000:.1f}ms > 100ms"
        ...
        for motor_id in range(simulator.firmware_sim.num_motors):
            ...
            assert abs(status['velocity_actual']) < 0.01, f"Motor {motor_id} not stopped"
    ...
    async def test_can_bus_overload(self, simulator):
        ...
        print(f"✅ High load test passed with {slcan_stats['success_rate']*100:.1f}% success rate")
    ...
    async def test_network_failure_handling(self, simulator):
        ...
        assert result.messages_received > 0, "No messages received during network failure"
    ...
    async def test_complete_roundtrip(self, perfect_simulator):
        ...
        status_data = {
            ...
            'motors': [simulator.firmware_sim.get_motor_status(i) 
    ...
    async def test_full_test_suite(self, simulator):
        ...
        assert summary['pass_rate'] >= 0.8, f"Pass rate too low: {summary['pass_rate']*100:.1f}%"
        
        print(f"✅ Full test suite: {summary['passed']}/{summary['total_scenarios']} passed")
    ...
    async def test_slcan_encoding_accuracy(self, perfect_simulator):
        ...
        for linear_x, linear_y, angular_z in test_cases:
            # Encode
            frame = simulator.slcan_sim.encode_velocity_command(linear_x, linear_y, angular_z)
            ...
            assert abs(cmd.linear_x - linear_x) < 0.001, f"Linear X mismatch: {cmd.linear_x} != {linear_x}"
            assert abs(cmd.linear_y - linear_y) < 0.001, f"Linear Y mismatch: {cmd.linear_y} != {linear_y}"
            assert abs(cmd.angular_z - angular_z) < 0.001, f"Angular Z mismatch: {cmd.angular_z} != {angular_z}"
    ...
    async def test_concurrent_clients(self, simulator):
        ...
        assert stats['connected_clients'] == 0 or len(simulator.websocket_sim.connected_clients) == 0
