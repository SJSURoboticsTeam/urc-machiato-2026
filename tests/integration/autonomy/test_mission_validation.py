...
class TestURCMissions:
    ...
    async def test_sample_collection_mission(self, mock_system_components):
        ...
        # Set up vision mocks
        vision.detect_samples = AsyncMock(
            return_value=[
    ...
    async def test_delivery_mission(self, mock_system_components):
        ...
        vision.detect_delivery_target = AsyncMock(
            return_value={
                "position": (8, 8, 0),
                "type": "delivery_zone",
                "confidence": 0.95,
    ...
    async def test_follow_me_mission(self, mock_system_components):
        ...
        vision.detect_aruco_markers = AsyncMock(
            return_value=[
                {
                    "id": 42,
                    "position": (3, 3, 0),
        ...
        vision.track_aruco_marker = AsyncMock(
            return_value={
                "position": (3, 3, 0),
                "velocity": (0.5, 0.5, 0),
                "visible": True,
        ...
        # Test mission planning
        params = {"target_marker_id": 42, "follow_distance": 2.0, "max_speed": 1.0}
        plan = await mission.plan_mission(params)
    ...
    async def test_return_to_operator_mission(self, mock_system_components):
        ...
        nav.calculate_path_to_safety = AsyncMock(
            return_value=[(10, 10, 0), (5, 5, 0), (0, 0, 0)]
        )

        # Set up vision mocks
        vision.detect_operator = AsyncMock(
            return_value={"position": (0, 0, 0), "confidence": 0.9}
        )
    ...
    async def test_mission_failure_recovery(self, mock_system_components):
        ...
        nav.navigate_to_position = AsyncMock(
            side_effect=[
    ...
    async def test_mission_performance_metrics(self, mock_system_components):
        ...
        print(
            f"✅ Mission Performance Metrics test completed (execution time: {execution_time:.2f}s)"
        )
    ...
    async def test_concurrent_mission_execution(self, mock_system_components):
        ...
        delivery_task = delivery_mission.execute_mission(
            {"pickup_location": (1, 1), "delivery_location": (7, 7)}
        )
