...
# Add project paths (tests/integration/autonomy -> repo root)
...
class TestBTStateMachineRuntime:
    ...
    def setup_method(self):
        ...
        self.executor = None
        # Initialize ROS2 (only if not already initialized, e.g. by conftest ros2_setup_teardown)
        if not rclpy.ok():
            rclpy.init()
        # Create test node
    ...
    def teardown_method(self):
        ...
        # Clean up ROS2 node only; do not shutdown context (conftest ros2_setup_teardown owns it)
        if self.test_node:
    ...
    def start_state_machine(self) -> bool:
        ...
        try:
            ...
            # Try to start state machine (tests/integration/autonomy -> repo root -> src/core)
