...
class TestBTRuntimeIntegration:
        ...
        try:
            ...
            if "bt_orchestrator" in result.stdout:
                ...
                self.test_node.get_logger().error(
                    f"BT orchestrator not found: {result.stdout}"
                )
                return False
        ...
        if change_state_client.wait_for_service(timeout_sec=5.0):
            ...
            if future.done():
                ...
                self.test_node.get_logger().info(
                    f"Configure transition result: {response}"
                )
    ...
    def test_qos_communication(self):
        ...
        odom_publisher = self.test_node.create_publisher(Odometry, "/odom", qos_profile)
    ...
    def test_telemetry_publishing(self):
        ...
        telemetry_subscriber = self.test_node.create_subscription(
            String, "/bt/telemetry", telemetry_callback, qos_profile
        )
        ...
        self.test_node.get_logger().info(
            f"Received {len(telemetry_received)} telemetry messages"
        )
    ...
    def test_topic_subscriptions(self):
        ...
        for topic in expected_topics:
            ...
            self.test_node.get_logger().info(
                f"Topic {topic} info: {topic_info.stdout[:100]}"
            )
