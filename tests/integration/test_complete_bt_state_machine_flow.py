...
class TestCompleteBTStateMachineIntegration:
    ...
    def test_adaptive_state_machine_service_creation(self):
        ...
        assert os.path.exists(state_machine_file), "Adaptive state machine file should exist"
        ...
        assert "/adaptive_state_machine/get_state" in content, "Should create correct service"
    ...
    def test_bt_orchestrator_state_machine_queries(self):
        ...
        # Check for state machine service calls
        ...
        assert "create_client<GetSystemState>" in content, "BT should create service client"
        assert "wait_for_service" in content, "BT should wait for service availability"
        assert "async_send_request" in content, "BT should send async requests"
    ...
    def test_blackboard_integration_complete(self):
        ...
        for var in required_vars:
            assert f'"{var}"' in content, f"Should initialize {var} in blackboard"

        # Check blackboard updates from ROS2
        assert "odom_callback" in content, "Should have odom callback"
        assert "slam_pose_callback" in content, "Should have SLAM callback"
        assert 'set("robot_x"' in content, "Should update robot_x from odom"
        assert 'set("slam_x"' in content, "Should update slam_x from SLAM"
    ...
    def test_bt_mission_file_completeness(self):
        ...
        for mission_file in required_missions:
            ...
            assert "<root BTCPP_format=" in content, f"{mission_file} should have BT root"
            assert "<BehaviorTree" in content, f"{mission_file} should have BehaviorTree element"
    ...
    def test_bt_state_machine_integration_architecture(self):
        ...
        if os.path.exists(bt_file):
            with open(bt_file, 'r') as f:
                if "blackboard_" in f.read() and "set_blackboard_value" in f.read():
        ...
        print(f"✅ BT-State Machine integration architecture validated: {len(components_validated)}/{len(expected_components)} components")
    ...
    def test_performance_requirements(self):
        ...
        for req_name, req_value in performance_reqs.items():
            ...
            assert "<" in req_value or ">" in req_value, f"Invalid performance requirement format: {req_value}"
    ...
    def test_integration_test_completeness(self):
        ...
        integration_points_tested = [
            "BT ↔ State Machine service calls",
        ...
        # Check that our test file covers these points
        test_file = "tests/integration/test_complete_bt_state_machine_flow.py"

        with open(test_file, 'r') as f:
            test_content = f.read()

        for integration_point in integration_points_tested:
            # Convert to function name pattern
            func_name_pattern = integration_point.lower().replace(" ", "_").replace("↔", "_")
            assert func_name_pattern in test_content, f"Missing test for: {integration_point}"
