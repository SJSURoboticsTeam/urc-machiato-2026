...
class MockSensorFusion(Node):
    ...
    def perform_fusion(self) -> FusionResult:
        ...
        weights = {"gps": 0.3, "imu": 0.2, "odometry": 0.5}
        ...
        for sensor, reading in self.readings.items():
            if reading.sensor_type in ["gps", "odometry"]:
                if reading.sensor_type == "gps":
                    ...
                    positions[sensor] = np.array(
                        [reading.data["latitude"], reading.data["longitude"]]
                    )
                else:
...
class SensorFusionMonitor:
    ...
    def record_fusion_result(self, result: FusionResult):
        ...
        self.sensor_status = {
            sensor: sensor in result.contributing_sensors
...
class TestMultiSensorFusion:
    ...
    def test_sensor_dropout_handling(
        ...
        imu_publisher.publish_imu_data(
            np.array([0.1, 0.0, 9.8]), np.array([0.0, 0.0, 0.01])
        )
        ...
        assert (
            len(result.contributing_sensors) == 3
        ), f"Expected 3 sensors, got {len(result.contributing_sensors)}"
        ...
        for i in range(50):  # Simulate 5 seconds of dropout
            imu_publisher.publish_imu_data(
                np.array([0.1 + 0.01 * i, 0.0, 9.8]), np.array([0.0, 0.0, 0.01])
            )
            odometry_publisher.publish_odometry_data(
        ...
        assert (
            avg_confidence > 0.4
        ), "Confidence should remain reasonable with 2 sensors"

        # Verify GPS is not in contributing sensors
        for result in latest_results:
            assert (
                "gps" not in result.contributing_sensors
            ), "GPS should not contribute during dropout"
            assert "imu" in result.contributing_sensors, "IMU should still contribute"
            assert (
                "odometry" in result.contributing_sensors
            ), "Odometry should still contribute"
    ...
    def test_sensor_conflict_resolution(
        ...
        assert (
            result.fusion_quality < 0.8
        ), "Fusion quality should be lower with conflicts"
        ...
        print(
            f"✅ Sensor conflict resolved: outliers={outliers}, quality={result.fusion_quality:.2f}"
        )
    ...
    def test_temporal_sensor_alignment(
        ...
        if result:
            ...
            assert (
                result.fusion_quality > 0.5
            ), f"Poor temporal alignment: {result.fusion_quality}"

            # Verify timestamps are considered
            contributing_times = [
                sensor_fusion.readings[sensor].timestamp
            ...
            if len(contributing_times) > 1:
                ...
                assert (
                    time_variance < 1.0
                ), f"Poor time synchronization: variance={time_variance:.3f}"
        ...
        for i, reliability in enumerate(gps_reliability_sequence):
            ...
            if result:
                ...
                if reliability < 0.4:
                    ...
                    assert (
                        not gps_contributing
                    ), f"GPS should be excluded at reliability {reliability}"
                else:
                    # GPS should contribute when reasonably reliable
                    assert (
                        gps_contributing
                    ), f"GPS should contribute at reliability {reliability}"
    ...
    def test_fusion_performance_under_noise(
        ...
        for noise_level in noise_levels:
            ...
            # Publish noisy data
            gps_publisher.publish_gps_data(
                37.7749 + gps_noise[0], -122.4194 + gps_noise[1], 100.0
            )
    ...
    def test_sensor_fusion_reliability_metrics(
        ...
        assert (
            0.7 <= availability["gps"] <= 0.8
        ), f"GPS availability unexpected: {availability['gps']}"
        assert (
            0.75 <= availability["imu"] <= 0.85
        ), f"IMU availability unexpected: {availability['imu']}"
        assert (
            0.85 <= availability["odometry"] <= 0.95
        ), f"Odometry availability unexpected: {availability['odometry']}"

        # Verify accuracy metrics are reasonable
        assert (
            0.5 <= accuracy["average_confidence"] <= 1.0
        ), f"Average confidence unexpected: {accuracy['average_confidence']}"
        assert (
            0.0 <= accuracy["min_confidence"] <= accuracy["average_confidence"]
        ), f"Min confidence unexpected: {accuracy['min_confidence']}"

        print(
            f"✅ Reliability metrics: availability={availability}, accuracy={accuracy}"
        )
