...
"""
        ...
        try:
            ...
            if (
                "Publisher count: 1" in result.stdout
                or "Publisher count: 1" in result.stderr
            ):
        ...
        for path in simulator_paths:
            ...
            if os.path.exists(full_path):
                return self.start_node(
                    ["python3", full_path], "sensor_simulator", wait_time=3.0
                )
    ...
    def test_full_system_with_simulator(self):
        ...
        if state_started:
            assert (
                "adaptive_state_machine" in result.stdout
                or "state" in result.stdout.lower()
        ...
        has_odom_publisher = (
            "Publisher count: 1" in odom_info.stdout
            or "Publisher count: 1" in odom_info.stderr
        )
    ...
    def test_blackboard_updates_from_simulator(self):
        ...
        has_subscriber = (
            "Subscription count: 2" in odom_info.stdout
            or "Subscription count: 1" in odom_info.stdout
        )
    ...
    def test_consistency_with_simulator(self):
        ...
        consistency_report["nodes"]["state_machine"] = (
            "adaptive_state_machine" in nodes or "state" in nodes.lower()
        )
        consistency_report["nodes"]["simulator"] = (
            "sensor_simulator" in nodes or "mock_sensor" in nodes.lower()
        )
        ...
        consistency_report["topics"]["/odom"] = (
            "/odom"
            in subprocess.run(
                ["ros2", "topic", "list"], capture_output=True, text=True
            ).stdout
        )
        consistency_report["topics"]["/odom_has_publisher"] = (
            "Publisher count: 1" in odom_info.stdout
            or "Publisher count: 1" in odom_info.stderr
        )
        ...
        consistency_report["topics"]["/slam/pose"] = (
            "/slam/pose"
            in subprocess.run(
                ["ros2", "topic", "list"], capture_output=True, text=True
            ).stdout
        )
        consistency_report["topics"]["/slam_has_publisher"] = (
            "Publisher count: 1" in slam_info.stdout
            or "Publisher count: 1" in slam_info.stderr
        )
        ...
        assert consistency_report["topics"][
            "/odom_has_publisher"
        ], "Odometry has no publisher"
        assert consistency_report["data_flow"][
            "odom_publishing"
        ], "Odometry not publishing data"
