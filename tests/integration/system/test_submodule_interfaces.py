...
class TeleopProtocolTester:
    ...
    def test_message_encoding(self) -> InterfaceTestResult:
        ...
        # #region agent log
        log_debug(
            "test_submodule_interfaces.py:101",
            "Starting message encoding test",
            {"test": "message_encoding"},
            "H3",
        )
        # #endregion
        ...
        try:
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:112",
                "Input velocities",
                {"x_vel": x_vel, "y_vel": y_vel, "rot_vel": rot_vel},
                "H3",
            )
            # #endregion
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:122",
                "Scaled velocities",
                {
                    "x_vel_scaled": x_vel_scaled,
                    "y_vel_scaled": y_vel_scaled,
                    "rot_vel_scaled": rot_vel_scaled,
                },
                "H3",
            )
            # #endregion
            ...
            slcan_frame = (
                f"t{msg_id:03X}6{x_bytes.hex()}{y_bytes.hex()}{rot_bytes.hex()}\r"
            )

            result.details["Expected SLCAN Frame"] = slcan_frame
            result.details["X Velocity (scaled)"] = (
                f"{x_vel_scaled} (0x{x_vel_scaled:04X})"
            )
            result.details["Y Velocity (scaled)"] = (
                f"{y_vel_scaled} (0x{y_vel_scaled:04X})"
            )
            result.details["Rot Velocity (scaled)"] = (
                f"{rot_vel_scaled} (0x{rot_vel_scaled:04X})"
            )

            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:142",
                "Generated SLCAN frame",
                {"slcan_frame": slcan_frame, "msg_id": hex(msg_id)},
                "H3",
            )
            # #endregion
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:153",
                "Decoded velocities",
                {
                    "decoded_x": decoded_x,
                    "decoded_y": decoded_y,
                    "decoded_rot": decoded_rot,
                },
                "H3",
            )
            # #endregion
            ...
            if abs(decoded_rot - rot_vel) > 0.1:
                result.add_error(
                    f"Rot velocity round-trip error: {decoded_rot} != {rot_vel}"
                )
            ...
            if not result.errors:
                ...
                # #region agent log
                log_debug(
                    "test_submodule_interfaces.py:168",
                    "Message encoding test PASSED",
                    {"result": "success"},
                    "H3",
                )
                # #endregion
                ...
                # #region agent log
                log_debug(
                    "test_submodule_interfaces.py:173",
                    "Message encoding test FAILED",
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:180",
                "Exception in message encoding test",
                {"exception": str(e)},
                "H3",
            )
            # #endregion
    ...
    def test_message_id_coverage(self) -> InterfaceTestResult:
        ...
        # #region agent log
        log_debug(
            "test_submodule_interfaces.py:189",
            "Starting message ID coverage test",
            {"test": "message_id_coverage"},
            "H2",
        )
        # #endregion
        ...
        try:
            ...
            if overlap:
                result.add_error(
                    f"Overlapping message IDs found: {[hex(x) for x in overlap]}"
                )
                # #region agent log
                log_debug(
                    "test_submodule_interfaces.py:204",
                    "Overlapping IDs detected",
                    {"overlap": [hex(x) for x in overlap]},
                    "H2",
                )
                # #endregion
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:214",
                "Message ID coverage analysis",
                {
                    "total_ids": len(all_ids),
                    "send_ids": len(send_set),
                    "receive_ids": len(receive_set),
                    "id_range": f"{hex(min(all_ids))}-{hex(max(all_ids))}",
                },
                "H2",
            )
            # #endregion
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:225",
                "Exception in message ID coverage test",
                {"exception": str(e)},
                "H2",
            )
            # #endregion
    ...
    def test_firmware_compatibility(self) -> InterfaceTestResult:
        ...
        # #region agent log
        log_debug(
            "test_submodule_interfaces.py:235",
            "Starting firmware compatibility test",
            {"test": "firmware_compatibility"},
            "H5",
        )
        # #endregion
        ...
        try:
            ...
            firmware_path = Path(
                "/home/durian/urc-machiato-2026/vendor/control-systems/drive/applications/application.cpp"
            )

            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:246",
                "Checking firmware application file",
                {"firmware_path": str(firmware_path), "exists": firmware_path.exists()},
                "H5",
            )
            # #endregion
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:259",
                "Read firmware application",
                {
                    "file_size": len(firmware_code),
                    "line_count": firmware_code.count("\n"),
                },
                "H5",
            )
            # #endregion
            ...
            for check_name, keywords in checks.items():
                ...
                if found:
                    ...
                    # #region agent log
                    log_debug(
                        "test_submodule_interfaces.py:276",
                        f"Check passed: {check_name}",
                        {"check": check_name, "found": True},
                        "H5",
                    )
                    # #endregion
                else:
                    ...
                    # #region agent log
                    log_debug(
                        "test_submodule_interfaces.py:283",
                        f"Check failed: {check_name}",
                        {"check": check_name, "found": False},
                        "H5",
                    )
                    # #endregion
            ...
            if "each loop:" in firmware_code and firmware_code.count("\n") < 25:
                result.add_error(
                    "Firmware application appears to be a stub (not implemented)"
                )
                result.details["Implementation Status"] = "STUB - Needs implementation"
                # #region agent log
                log_debug(
                    "test_submodule_interfaces.py:292",
                    "Firmware is a stub",
                    {"status": "stub", "implementation_needed": True},
                    "H5",
                )
                # #endregion
            else:
                ...
                # #region agent log
                log_debug(
                    "test_submodule_interfaces.py:298",
                    "Firmware has implementation",
                    {"status": "implemented"},
                    "H5",
                )
                # #endregion
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:309",
                "Exception in firmware compatibility test",
                {"exception": str(e)},
                "H5",
            )
            # #endregion
...
class MainCodebaseProtocolTester:
    ...
    def test_protocol_compatibility(self) -> InterfaceTestResult:
        ...
        # #region agent log
        log_debug(
            "test_submodule_interfaces.py:334",
            "Starting protocol compatibility test",
        ...
        try:
            ...
            teleop_ids = set(teleop_tester.SEND_IDS.values()) | set(
                teleop_tester.RECEIVE_IDS.values()
            )
            main_ids = set(self.MESSAGE_IDS.values())

            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:346",
                "Comparing message ID sets",
                {"teleop_ids_count": len(teleop_ids), "main_ids_count": len(main_ids)},
                "H2",
            )
            # #endregion
            ...
            if conflicts:
                ...
                # #region agent log
                log_debug(
                    "test_submodule_interfaces.py:356",
                    "Message ID conflicts found",
                    {"conflicts": [hex(x) for x in conflicts]},
                    "H2",
                )
                # #endregion
            else:
                ...
                # #region agent log
                log_debug(
                    "test_submodule_interfaces.py:362",
            ...
            result.details["Teleoperation ID Range"] = (
                f"{hex(teleop_range[0])} - {hex(teleop_range[1])}"
            )
            result.details["Main Codebase ID Range"] = (
                f"{hex(main_range[0])} - {hex(main_range[1])}"
            )

            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:375",
                "Message ID range comparison",
                {
                    "teleop_range": f"{hex(teleop_range[0])}-{hex(teleop_range[1])}",
                    "main_range": f"{hex(main_range[0])}-{hex(main_range[1])}",
                },
                "H2",
            )
            # #endregion
            ...
            if main_range[0] <= teleop_range[1] and main_range[1] >= teleop_range[0]:
                ...
                # #region agent log
                log_debug(
                    "test_submodule_interfaces.py:385",
                    "Message ID ranges overlap",
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:396",
                "Exception in protocol compatibility test",
                {"exception": str(e)},
                "H2",
            )
            # #endregion
    ...
    def test_hardware_interface_config(self) -> InterfaceTestResult:
        ...
        # #region agent log
        log_debug(
            "test_submodule_interfaces.py:406",
            "Starting hardware interface config test",
            {"test": "hardware_interface_config"},
            "H4",
        )
        # #endregion
        ...
        try:
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:417",
                "Checking hardware interface node",
                {"path": str(hw_interface_path), "exists": hw_interface_path.exists()},
                "H4",
            )
            # #endregion
            ...
            if not hw_interface_path.exists():
                result.add_error(
                    f"Hardware interface node not found: {hw_interface_path}"
                )
                result.passed = False
                return result
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:431",
                "Read hardware interface node",
                {"file_size": len(hw_interface_code)},
                "H4",
            )
            # #endregion
            ...
            checks = {
                "CAN Serial Import": "can_serial" in hw_interface_code
                or "CanSerial" in hw_interface_code,
                "Twist Mux": "mux" in hw_interface_code.lower()
                and "priority" in hw_interface_code.lower(),
            ...
            for check_name, passed in checks.items():
                if passed:
                    ...
                    # #region agent log
                    log_debug(
                        "test_submodule_interfaces.py:448",
                        f"Check passed: {check_name}",
                        {"check": check_name, "found": True},
                        "H4",
                    )
                    ...
                    # #region agent log
                    log_debug(
                        "test_submodule_interfaces.py:455",
                        f"Check failed: {check_name}",
                        {"check": check_name, "found": False},
                        "H4",
                    )
            ...
            result.details["Device Paths Found"] = (
                found_devices if found_devices else "None"
            )

            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:467",
            ...
            # #region agent log
            log_debug(
                "test_submodule_interfaces.py:478",
                "Exception in hardware interface config test",
                {"exception": str(e)},
                "H4",
            )
            # #endregion
...
class InterfaceTestSuite:
    ...
    def run_all_tests(self):
        ...
        # #region agent log
        log_debug(
            "test_submodule_interfaces.py:501",
        ...
        # #region agent log
        log_debug(
            "test_submodule_interfaces.py:517",
            "All tests completed",
        ...
        # #region agent log
        log_debug(
            "test_submodule_interfaces.py:539",
            "Test suite complete",
            {
                "total_tests": len(self.results),
                "passed": passed,
...
def main():
    ...
    # #region agent log
    log_debug(
        "test_submodule_interfaces.py:557",
        "Test suite starting",
        {"session": SESSION_ID, "timestamp": time.time()},
        "H1",
    )
    # #endregion
    ...
    # #region agent log
    log_debug(
        "test_submodule_interfaces.py:565",
        "Test suite finished",
        {"session": SESSION_ID, "success": success, "timestamp": time.time()},
        "H1",
    )
    # #endregion
