...
# Add repo root and src to path (tests/integration/system -> ../../..)
...
class EmergencySystemsOptimizationTester:
        ...
        try:
            ...
            response_time_analysis = {
                ...
                "violation_rate_percent": (
                    len([t for t in response_times if t > 10.0]) / test_iterations
                )
                * 100,
                "meets_10ms_requirement": statistics.quantiles(response_times, n=20)[18]
                < 10.0,
                "meets_5ms_target": statistics.quantiles(response_times, n=20)[18]
                < 5.0,  # Aggressive target
            ...
            status = (
                "✅ PASS"
                if response_time_analysis["meets_10ms_requirement"]
                else "❌ FAIL"
            )
            print(".2f")
            if response_time_analysis["violations_over_10ms"] > 0:
                print(
                    f"  ⚠️ {response_time_analysis['violations_over_10ms']} violations detected"
                )
    ...
    def _test_override_priority_verification(self):
        ...
        try:
            ...
            test_scenarios = [
                {
                    "name": "motion_command_during_emergency",
                    "motion_cmd": True,
                    "emergency_active": True,
                },
                {
                    "name": "navigation_command_during_emergency",
                    "nav_cmd": True,
                    "emergency_active": True,
                },
                {
                    "name": "arm_command_during_emergency",
                    "arm_cmd": True,
                    "emergency_active": True,
                },
                {
                    "name": "normal_operation_after_clear",
                    "motion_cmd": True,
            ...
            for scenario in test_scenarios:
                ...
                scenario_result = {
                    ...
                    "override_working": (
                        scenario.get("emergency_active", False) and commands_blocked > 0
                    )
                    or (
                        not scenario.get("emergency_active", False)
                        and commands_allowed > 0
                    ),
                }
            ...
            emergency_scenarios = [
                r for r in override_test_results if r["emergency_active"]
            ]
            normal_scenarios = [
                r for r in override_test_results if not r["emergency_active"]
            ]
            ...
            override_priority_analysis = {
                ...
                "emergency_override_effective": all(
                    r["override_working"] for r in emergency_scenarios
                ),
                "normal_operation_allowed": all(
                    r["override_working"] for r in normal_scenarios
                ),
                "override_priority_score": self._calculate_override_priority_score(
                    override_test_results
                ),
                "meets_override_requirement": all(
                    r["override_working"] for r in override_test_results
                ),
                "detailed_results": override_test_results,
            }

            self.test_results["override_priority_verification"] = (
                override_priority_analysis
            )

            status = (
                "✅ PASS"
                if override_priority_analysis["meets_override_requirement"]
                else "❌ FAIL"
            )
            print(".1f")
            safety_system.cleanup()
    ...
    def _test_recovery_procedure_validation(self):
        ...
        try:
            ...
            for iteration in range(test_iterations):
                ...
                emergency_triggered = safety_system.emergency_stop(
                    f"recovery_test_{iteration}"
                )
                ...
                recovery_cycle_result = {
                    ...
                    "complete_recovery_cycle": (
                        normal_state_verified
                        and emergency_active
                        and system_recovered
                        and post_recovery_commands_allowed
                    ),
                }
            ...
            successful_cycles = sum(
                1 for r in recovery_test_results if r["complete_recovery_cycle"]
            )
            total_cycles = len(recovery_test_results)

            recovery_activation_times = [
                r["emergency_activation_time_ms"] for r in recovery_test_results
            ]
            ...
            recovery_procedure_analysis = {
                ...
                "emergency_activation_avg_ms": statistics.mean(
                    recovery_activation_times
                ),
                "emergency_activation_p95_ms": statistics.quantiles(
                    recovery_activation_times, n=20
                )[18],
                "recovery_time_avg_ms": statistics.mean(recovery_times),
                "recovery_time_p95_ms": statistics.quantiles(recovery_times, n=20)[18],
                "meets_recovery_requirement": (successful_cycles / total_cycles)
                > 0.95,  # >95% success rate
                "recovery_time_acceptable": statistics.quantiles(recovery_times, n=20)[
                    18
                ]
                < 5000.0,  # <5 seconds
                "emergency_response_fast": statistics.quantiles(
                    recovery_activation_times, n=20
                )[18]
                < 50.0,  # <50ms
                "detailed_results": recovery_test_results[
                    :10
                ],  # First 10 cycles for detail
            }

            self.test_results["recovery_procedure_validation"] = (
                recovery_procedure_analysis
            )
            ...
            status = (
                "✅ PASS"
                if (
                    recovery_procedure_analysis["meets_recovery_requirement"]
                    and recovery_procedure_analysis["recovery_time_acceptable"]
                )
                else "❌ FAIL"
            )
            print(".1f")
    ...
    def _test_false_trigger_rate_monitoring(self):
        ...
        try:
            ...
            print(
                f"  Monitoring for false triggers over {monitoring_duration_seconds} seconds..."
            )
            ...
            false_trigger_analysis = {
                ...
                "false_trigger_rate_percent": (len(false_triggers) / total_checks)
                * 100,
                "meets_requirement": (len(false_triggers) / total_checks) * 100
                < 0.1,  # <0.1% requirement
                "triggers_per_hour": (
                    (len(false_triggers) / (actual_duration / 3600))
                    if actual_duration > 0
                    else 0
                ),
                "mean_time_between_false_triggers": (
                    actual_duration / len(false_triggers)
                    if false_triggers
                    else float("inf")
                ),
                "false_trigger_details": (
                    false_triggers[:5] if false_triggers else []
                ),  # First 5 for analysis
            }

            self.test_results["false_trigger_rate_monitoring"] = false_trigger_analysis

            status = (
                "✅ PASS" if false_trigger_analysis["meets_requirement"] else "❌ FAIL"
            )
            print(".4f")
            if false_triggers:
    ...
    def _test_hardware_interlock_verification(self):
        ...
        try:
            ...
            test_scenarios = [
                {
                    "name": "emergency_stop_brake_engagement",
                    "emergency": True,
                    "brake_expected": True,
                },
                {
                    "name": "normal_operation_brake_release",
                    "emergency": False,
                    "brake_expected": False,
                },
                {
                    "name": "recovery_brake_release",
                    "emergency": True,
                    "recovery": True,
                    "brake_expected": False,
                },
            ...
            correct_interlocks = sum(
                1 for r in interlock_test_results if r["interlock_correct"]
            )
            ...
            hardware_interlock_analysis = {
                ...
                "emergency_brake_engagement": all(
                    r["brake_engaged"]
                    for r in interlock_test_results
                    if r["scenario"].startswith("emergency")
                ),
                "recovery_brake_release": all(
                    not r["brake_engaged"]
                    for r in interlock_test_results
                    if "recovery" in r["scenario"]
                ),
                "normal_operation_brake_release": all(
                    not r["brake_engaged"]
                    for r in interlock_test_results
            ...
            self.test_results["hardware_interlock_verification"] = (
                hardware_interlock_analysis
            )

            status = (
                "✅ PASS"
                if hardware_interlock_analysis["meets_interlock_requirement"]
                else "❌ FAIL"
            )
            safety_system.cleanup()
    ...
    def _calculate_override_priority_score(self, override_results: List[Dict]) -> float:
        ...
        if emergency_scenarios:
            emergency_working = sum(
                1 for r in emergency_scenarios if r["override_working"]
            )
            emergency_bonus = (
                10.0 if emergency_working == len(emergency_scenarios) else 0.0
            )
        else:
            emergency_bonus = 0.0
        ...
        final_score = max(
            0.0, min(100.0, base_score + emergency_bonus - normal_penalty)
        )
        return final_score
        ...
        if "override_priority_verification" in self.test_results:
            ...
            if isinstance(override, dict):
                ...
                analysis["override_priority"] = {
                    ...
                    "emergency_override_effective": override.get(
                        "emergency_override_effective", False
                    ),
                    "normal_operation_allowed": override.get(
        ...
        if "recovery_procedure_validation" in self.test_results:
            ...
            if isinstance(recovery, dict):
                ...
                analysis["recovery_performance"] = {
                    ...
                    "meets_success_requirement": recovery.get(
                        "meets_recovery_requirement", False
                    ),
                    "recovery_time_acceptable": recovery.get(
                        "recovery_time_acceptable", False
                    ),
                    "optimization_needed": success_rate < 98.0
                    or recovery_time > 3000.0,
                }
            ...
            if isinstance(timing, dict):
                if not timing.get("meets_10ms_requirement", False):
                    recommendations.append(
                        "Optimize emergency response time to meet <10ms requirement"
                    )
                if timing.get("violation_rate_percent", 100) > 1:
                    recommendations.append(
                        "Reduce response time violations through hardware/software optimization"
                    )
        ...
        if "override_priority_verification" in self.test_results:
            ...
            if isinstance(override, dict):
                if not override.get("emergency_override_effective", False):
                    recommendations.append(
                        "Fix emergency override to block all commands during emergency"
                    )
                if not override.get("normal_operation_allowed", False):
                    recommendations.append(
                        "Ensure normal commands work when emergency is cleared"
                    )
        ...
        if "false_trigger_rate_monitoring" in self.test_results:
            ...
            if isinstance(false_triggers, dict):
                if not false_triggers.get("meets_requirement", False):
                    recommendations.append(
                        "Reduce false trigger rate to <0.1% through better filtering"
                    )
                if false_triggers.get("triggers_per_hour", 999) > 1:
                    recommendations.append(
                        "Implement additional trigger validation logic"
                    )
        ...
        if "hardware_interlock_verification" in self.test_results:
            ...
            if isinstance(interlock, dict):
                if not interlock.get("meets_interlock_requirement", False):
                    recommendations.append(
                        "Fix hardware interlock verification for brake engagement"
                    )

        if not recommendations:
            recommendations.append(
                "✅ Emergency systems optimizations are performing well"
            )
