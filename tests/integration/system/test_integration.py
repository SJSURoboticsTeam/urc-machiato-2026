...
class IntegrationTests(unittest.TestCase):
        ...
        try:
            ...
            for mission in self.test_mission_profiles:
                with self.subTest(mission=mission):
                    ...
                    self.assertTrue(
                        rm_success, f"Resource manager failed to switch to {mission}"
                    )
            ...
            print(
                "✅ MissionResourceManager + FeatureFlags integration working correctly"
            )
        ...
        try:
            ...
            for mission in self.test_mission_profiles:
                with self.subTest(mission=mission):
                    ...
                    self.assertTrue(
                        bt_success, f"Behavior tree failed to switch to {mission}"
                    )
                    ...
                    self.assertEqual(
                        rm_status.get("mission_profile"),
                        bt_status.get("mission_profile"),
            ...
            try:
                ...
                self.fail(
                    f"Behavior tree execution failed during integration test: {e}"
                )
    ...
    def test_monitoring_resource_manager_integration(self):
        ...
        pytest.importorskip(
            "simulation.tools.monitoring_dashboard",
            reason="simulation.tools.monitoring_dashboard not available",
        )
        ...
        try:
            ...
            for mission, data in mission_data.items():
                self.assertEqual(
                    data["mission"], mission, f"Mission {mission} not properly tracked"
                )
        ...
        try:
            ...
            for mission in self.test_mission_profiles:
                with self.subTest(mission=mission):
                    ...
                    # Resource manager
                    switch_results["rm"] = systems[
                        "resource_manager"
                    ].switch_mission_profile(mission)
                    ...
                    for system, success in switch_results.items():
                        self.assertTrue(
                            success, f"{system} failed to switch to {mission}"
                        )
                    ...
                    self.assertEqual(
                        rm_mission,
                        mission,
                        f"Resource manager mission mismatch: {rm_mission}",
                    )
                    self.assertEqual(
                        ff_mission,
                        mission,
                        f"Feature flags mission mismatch: {ff_mission}",
                    )
                    self.assertEqual(
                        bt_mission,
                        mission,
                        f"Behavior tree mission mismatch: {bt_mission}",
                    )
                    ...
                    self.assertGreater(
                        rm_components, 0, "Should have some components enabled"
                    )
                    self.assertLess(
                        rm_components,
                        10,
                        "Should not have excessive components enabled",
                    )
        ...
        try:
            ...
            for from_mission, to_mission in transitions:
                with self.subTest(transition=f"{from_mission}->{to_mission}"):
                    ...
                    rm_mission = (
                        systems["rm"].get_resource_status().get("mission_profile")
                    )
                    ff_mission = systems["ff"].current_mission_profile
                    bt_mission = (
                        systems["bt"].get_resource_status().get("mission_profile")
                    )

                    self.assertEqual(
                        rm_mission,
                        to_mission,
                        f"RM mission inconsistency: {rm_mission}",
                    )
                    self.assertEqual(
                        ff_mission,
                        to_mission,
                        f"FF mission inconsistency: {ff_mission}",
                    )
                    self.assertEqual(
                        bt_mission,
                        to_mission,
                        f"BT mission inconsistency: {bt_mission}",
                    )

                    # Check component/feature relationship
                    rm_components = (
                        systems["rm"].get_resource_status().get("component_status", {})
                    )
                    ...
                    for comp in major_components:
                        if comp in rm_components:
                            ...
                            related_features = [
                                ...
                                if comp.replace("_", "_") in f or f in comp
                            ]
                            # At least one related feature should be enabled (simplified check)
                            feature_consistency = (
                                len(related_features) > 0 or len(ff_features) > 0
                            )
                            self.assertTrue(
                                feature_consistency,
    ...
    def test_system_stability_during_transitions(self):
        ...
        try:
            ...
            for i in range(transitions):
                ...
                mission = self.test_mission_profiles[
                    i % len(self.test_mission_profiles)
                ]
            ...
            self.assertLess(
                max_memory_spike, 50.0, "Memory spikes should be controlled"
            )

            print(
                "✅ System stability during transitions verified"
    ...
    def test_concurrent_access_safety(self):
        ...
        try:
            ...
            self.assertIsInstance(
                rm_status, dict, "Resource manager should still be functional"
            )
