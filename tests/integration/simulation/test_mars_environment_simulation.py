...
class MarsTerrainSimulator:
    ...
    def update_weather(self):
        ...
        self.weather_conditions["wind_speed"] = max(
            0, min(20, self.weather_conditions["wind_speed"])
        )

        # Occasional dust storms
        if random.random() < 0.05:  # 5% chance per update
            self.weather_conditions["dust_storm"] = not self.weather_conditions[
                "dust_storm"
            ]
    ...
    def simulate_physics(
        self, position: Tuple[float, float], velocity: Tuple[float, float], dt: float
...
class MarsCommunicationSimulator:
    ...
    def simulate_communication(
        ...
        time.sleep(
            delay * 0.1
        )  # Scale down for testing (real delay would be much longer)
    ...
    def get_communication_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            + random.uniform(-self.jitter_range, self.jitter_range),
        ...
        for sample_site in sample_sites:
            ...
            if not nav_success:
                result["errors"].append(
                    f"Failed to navigate to sample site {sample_site['id']}"
                )
                break
            ...
            if collection_success:
                ...
                result["errors"].append(
                    f"Failed to collect sample at site {sample_site['id']}"
                )
        ...
        if collection_result["success"]:
            ...
            if nav_success:
                result["waypoints_completed"] = (
                    collection_result["waypoints_completed"] + 1
                )
...
class TestMarsEnvironmentSimulation:
    ...
    def test_sample_collection_mission(self, urc_mission_simulator):
        ...
        print(
            f"🧪 Sample collection mission: {result['samples_collected']} samples, {result['success']}"
        )
    ...
    def test_navigation_mission(self, urc_mission_simulator):
        ...
        if result["success"]:
            assert result["waypoints_completed"] == len(
                urc_mission_simulator.terrain.terrain_map["waypoints"]
            )

        print(
            f"🧭 Navigation mission: {result['waypoints_completed']} waypoints, {result['success']}"
        )
    ...
    def test_delivery_mission(self, urc_mission_simulator):
        ...
        print(
            f"📦 Delivery mission: {result['samples_delivered']} samples delivered, {result['success']}"
        )
        ...
        for mission_type in mission_types:
            try:
                ...
                if "samples_collected" in result:
                    competition_results["total_samples_collected"] += result[
                        "samples_collected"
                    ]
    ...
    def test_communication_blackout_handling(self, urc_mission_simulator):
        ...
        urc_mission_simulator.communication.blackout_probability = (
            0.1  # 10% blackout chance
        )
    ...
    def test_battery_life_simulation(self, urc_mission_simulator):
        ...
        result = urc_mission_simulator.execute_mission(
            "delivery"
    ...
    @pytest.mark.parametrize(
        "mission_type", ["sample_collection", "navigation", "delivery"]
    )
    def test_mission_reliability_under_stress(
        self, urc_mission_simulator, mission_type
    ):
        ...
        for run in range(3):
            ...
            urc_mission_simulator.terrain.weather_conditions["dust_storm"] = (
                run % 2 == 0
            )
        ...
        success_rate = sum(1 for r in reliability_results if r["success"]) / len(
            reliability_results
        )
        avg_duration = sum(r["duration"] for r in reliability_results) / len(
            reliability_results
        )
        ...
        print(
            f"🔄 {mission_type} reliability: {success_rate:.1%} success rate, {avg_duration:.1f}s avg duration"
        )
