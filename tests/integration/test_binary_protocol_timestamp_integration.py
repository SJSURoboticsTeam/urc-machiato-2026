...
class TestBinaryProtocolTimestampIntegration:
    ...
    def test_binary_protocol_imu_roundtrip(self):
        ...
        imu_data = IMUData(
            ...
            orientation_x=0.1, orientation_y=0.2, orientation_z=0.3, orientation_w=0.9
        )
        ...
        assert len(binary_data) == BinarySensorProtocol.HEADER_SIZE + BinarySensorProtocol.IMU_SIZE

        # Verify message type and sequence
        assert BinarySensorProtocol.get_message_type(binary_data) == SensorMessageType.IMU
    ...
    def test_end_to_end_sensor_pipeline(self):
        ...
        raw_sensor_data = {
            "accel_x": 1.5, "accel_y": -0.3, "accel_z": 9.7,
        ...
        binary_message = BinarySensorProtocol.encode_imu(imu_data, timestamped_data.sequence_number)
        ...
        assert BinarySensorProtocol.get_sequence_number(binary_message) == timestamped_data.sequence_number
    ...
    def test_performance_binary_vs_timestamped_pipeline(self):
        ...
        for i in range(iterations):
            # Timestamp
            timestamped = self.timestamp_provider.tag_sensor_data(SensorType.IMU, raw_data)
        ...
        assert time_per_iteration_ms < 1.0, f"Pipeline too slow: {time_per_iteration_ms:.3f}ms/iteration"
    ...
    def test_sensor_latency_calibration(self):
        ...
        self.timestamp_provider.calibrate_sensor_latency(SensorType.IMU, 8.5)  # 8.5ms measured
