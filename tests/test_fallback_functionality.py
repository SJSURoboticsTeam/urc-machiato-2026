...
class FallbackFunctionalityTests(unittest.TestCase):
    ...
    def test_computer_vision_fallback(self):
        ...
        with self._mock_missing_libraries(['torch', 'torchvision', 'tensorflow', 'detectron2']):
            # Import computer vision node - should load with fallbacks
            try:
                from src.autonomy.perception.computer_vision.autonomy_computer_vision import computer_vision_node
    ...
    def test_terrain_analysis_fallback(self):
        ...
        with self._mock_missing_libraries(['tensorflow']):
            try:
                ...
                from src.autonomy.core.terrain_intelligence.terrain_ml_analyzer import TerrainFeatureExtractor
                ...
                from src.autonomy.perception.computer_vision.lightweight_vision import LightweightTerrainAnalyzer
                ...
                terrain_image = np.full((240, 320, 3), [194, 178, 128], dtype=np.uint8)  # Sandy color
    ...
    def test_performance_impact_of_fallbacks(self):
        ...
        with self._mock_missing_libraries(['torch', 'tensorflow', 'detectron2']):
            ...
            memory_reduction = baseline_measurements['memory_mb'] - fallback_measurements['memory_mb']

            # Memory usage should be lower or similar (not significantly higher)
            self.assertLessEqual(fallback_measurements['memory_mb'], baseline_measurements['memory_mb'] * 1.1)
            ...
            print(".1f"                      f"Baseline memory: {baseline_measurements['memory_mb']:.1f}MB, "
        ...
        for module_name in critical_modules:
            with self.subTest(module=module_name):
                try:
                    ...
                    with self._mock_missing_libraries(['torch', 'tensorflow', 'pydantic_settings']):
