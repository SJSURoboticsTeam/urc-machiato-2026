...
class TestCrossSystemIntegration:
    ...
    def integrated_system(self, ros2_init_shutdown):
        ...
        def start_with_comm(targets):
            ...
            comm_manager.send_command_via_redundant_channel(
                {"type": "start_autonomous_navigation", "targets": targets}
            )
            return result
    ...
    def test_mission_start_integration(self, integrated_system):
        ...
        assert (
            nav_orch["active"] == True
        ), "Navigation should be active after mission start"
        assert nav_orch["current_mode"] == "autonomous", "Should be in autonomous mode"
        assert (
            len(nav_orch["targets"]["gnss_locations"]) == 1
        ), "Should have GNSS targets"
        ...
        assert (
            call_args["type"] == "start_autonomous_navigation"
        ), "Should send correct command type"
    ...
    def test_emergency_stop_integration(self, integrated_system):
        ...
        emergency.trigger_emergency_stop(
    ...
    def test_communication_failover_during_mission(self, integrated_system):
        ...
        assert (
            nav_orch["active"] == True
        ), "Mission should remain active during failover"
    ...
    def test_led_status_system_integration(self, integrated_system):
        ...
        ), "LED should set autonomous mode"
        ...
        ), "LED should switch to teleoperation"
    ...
    def test_gnss_compliance_integration(self, integrated_system):
        ...
        assert (
            bridge.gnss_compliance["wgs84_verified"] == True
        ), "GNSS should be verified"
        assert (
            bridge.gnss_compliance["last_coordinates"]["lat"] == 38.4068
        ...
        assert (
            bridge.gnss_compliance["wgs84_verified"] == False
        ), "Invalid GNSS should fail verification"
    ...
    def test_band_configuration_integration(self, integrated_system):
        ...
        assert (
            band_config["900mhz"]["current_subband"] == "low"
        ), "Subband should be low"
        assert (
            band_config["900mhz"]["sub_bands"]["low"]["active"] == True
        ), "Low subband should be active"

        # Verify spectrum compliance tracking
        spectrum = bridge.spectrum_compliance
        assert (
            spectrum["current_band"] == "900mhz"
        ), "Spectrum compliance should track band"
        assert isinstance(
            spectrum["bandwidth_usage_mhz"], float
        ), "Bandwidth should be tracked"
    ...
    def test_mission_abortion_integration(self, integrated_system):
        ...
        assert (
            nav_orch["current_mode"] == "teleoperation"
        ), "Should switch to teleoperation on abort"
    ...
    def test_system_health_monitoring_integration(self, integrated_system):
        ...
        assert nav_orch["current_mode"] == "autonomous", "Should be in autonomous mode"
        assert (
            comm.send_command_via_redundant_channel.called
        ...
        assert (
            nav_orch["current_mode"] == "autonomous"
        ), "LED status should be synchronized"
