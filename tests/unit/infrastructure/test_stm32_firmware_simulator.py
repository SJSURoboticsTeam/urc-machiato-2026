...
class TestSTM32FirmwareSimulator:
    ...
    def simulator(self):
        ...
        sim = STM32FirmwareSimulator(
            {"num_motors": 6, "simulate_faults": False, "control_loop_hz": 100}
        )
        yield sim
        sim.stop()
    ...
    def test_chassis_velocities_rotation(self, simulator):
        ...
        motor_velocities = [m.velocity_setpoint for m in simulator.motors.values()]
        assert any(v > 0 for v in motor_velocities) or any(
            v < 0 for v in motor_velocities
        )
    ...
    def test_random_fault_injection(self, fault_simulator):
        ...
        faulted_motors = [
            m for m in simulator.motors.values() if m.fault != MotorFaultType.NONE
        ]
