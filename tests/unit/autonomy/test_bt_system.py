...
class TestBTActionServers:
    ...
    def test_navigate_action_server_creation(self, ros_context):
        ...
        action_client = ActionClient(
            node, action_msgs.NavigateToPose, "/bt/navigate_to_pose"
        )
        ...
        ), "ActionClient has required methods"
    ...
    def test_execute_mission_action_server_creation(self, ros_context):
        ...
        # Create action client
        action_client = ActionClient(
            node, action_msgs.ExecuteMission, "/bt/execute_mission"
        )
    ...
    async def test_mission_action_communication(self, ros_context):
        ...
        if not server_available:
            pytest.skip(
                "BT mission action server not available - run integrated system first"
            )
...
class TestBTTelemetry:
    ...
    def test_bt_telemetry_topic(self, ros_context):
        ...
        subscription = node.create_subscription(
            String, "/bt/telemetry", telemetry_callback, 10
        )
        ...
        if len(received_messages) > 0:
            ...
            assert hasattr(
                received_messages[0], "data"
            ), "Telemetry message has data field"
