...
class MockBehaviorTree:
    """Mock behavior tree for testing (can also be used as a composite node)."""
    ...
    def add_node(self, node):
        """Add a node to the tree (MockBTNode or MockBehaviorTree)."""
...
class TestActionNodeFailures:
    ...
    def test_action_node_execution_failure(self, mock_tree):
        ...
        assert (
            "navigate_to_waypoint" in mock_tree.failures
        ), "Should record failing node"
...
class TestDecoratorFailures:
    ...
    def test_inverter_decorator_logic_error(self):
        ...
        assert (
            inverter_success.execute() == "FAILURE"
        ), "Should invert SUCCESS to FAILURE"
        assert (
            inverter_failure.execute() == "SUCCESS"
...
class TestStatePersistenceFailures:
    ...
    def test_state_save_failure_corruption(self):
        ...
        try:
            state_manager.save_state(
                "critical_state", {"waypoint": "wp1", "progress": 50}
            )
            ...
            ), "Should not corrupt existing state on save failure"
    ...
    def test_state_load_corruption_recovery(self):
        ...
        def validate_state(state):
            ...
            if not isinstance(state.get("mission_progress"), (int, float)) or not (
                0 <= state.get("mission_progress", 0) <= 100
            ):
    ...
    def test_concurrent_state_access_corruption(self):
        ...
        if actual != expected:
            assert (
                True
            ), f"Concurrent access caused corruption: expected {expected}, got {actual}"
...
class TestRecoveryMechanismFailures:
    ...
    def test_recovery_state_corruption(self):
        ...
        assert isinstance(
            validated_state["recovery_attempts"], int
        ), "Should fix corrupted attempt counter"
        assert isinstance(
            validated_state["recovery_history"], list
        ), "Should fix corrupted history"
