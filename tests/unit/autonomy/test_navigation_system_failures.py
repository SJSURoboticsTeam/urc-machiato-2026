...
# Add source paths (repo root and src)
...
class TestPathPlanningFailures:
    ...
    def test_blocked_path_no_alternative_route(self, path_planner):
        ...
        with patch.object(path_planner, "_create_navigation_graph", return_value=None):
            ...
            assert path == [
                start,
                goal,
            ], "Should return fallback path when planning fails"
    ...
    def test_impossible_route_terrain_obstacles(self, path_planner):
        ...
        with patch.object(path_planner, "get_terrain_cost", return_value=float("inf")):
            ...
            assert path == [
                start,
                goal,
            ], "Should fallback to direct path for impassable terrain"
    ...
    def test_path_planner_memory_exhaustion(self, path_planner):
        ...
        with patch.object(
            ...
            assert path == [
                start,
                goal,
            ], "Should fallback gracefully on memory exhaustion"
    ...
    def test_dstar_lite_convergence_failure(self):
        ...
        dstar.costmap = {
            (x, y): 1.0 for x in range(-100, 101) for y in range(-100, 101)
        }
    ...
    def test_buffer_management_improvements(self, sync_engine):
        ...
        assert (
            valid_camera_id in sync_engine.camera_buffers
        ), "Should auto-register new camera"
        ...
        assert (
            buffer_stats["max_size"] > 100
        ), f"Buffer should expand adaptively, got {buffer_stats['max_size']}"
...
class TestLocalizationFailures:
    ...
    def test_complete_gps_loss_no_fallback(self, gnss_processor):
        ...
        ), "Should indicate localization failure"
    ...
    def test_gps_spoofing_detection_failure(self, gnss_processor):
        ...
        if hasattr(gnss_processor, "_detect_spoofing"):
            assert gnss_processor._detect_spoofing(
                result1, result2
            ), "Should detect GPS spoofing"
    ...
    def test_sensor_fusion_timing_violations(self, gnss_processor):
        ...
        gps_data = {"latitude": 40.0, "longitude": -74.0, "timestamp": time.time()}
        ...
        if result:
            assert (
                result.get("quality", 1.0) < 0.5
            ), "Should degrade fusion quality for timing violations"
...
class TestWaypointValidationFailures:
    ...
    def test_invalid_coordinates_rejection(self, navigation_node):
        ...
        for waypoint in invalid_waypoints:
            result = nav_node._validate_waypoint(type("Waypoint", (), waypoint)())
            assert (
                not result.success
            ), f"Should reject invalid waypoint: {waypoint['name']}"
    ...
    def test_dynamic_obstacle_blocking_waypoint(self):
        ...
        waypoint_blocked = any(
            math.sqrt(
                (obs["position"][0] - waypoint["position"][0]) ** 2
                + (obs["position"][1] - waypoint["position"][1]) ** 2
            )
            <= obs["radius"]
...
class TestMotionControlFailures:
    ...
    def test_encoder_feedback_loss(self, motion_controller):
        ...
        odometry_valid = any(encoder.get_position() is not None for encoder in encoders)
...
class TestEmergencyStopFailures:
    ...
    def test_false_emergency_stop_triggers(self, safety_monitor):
        ...
        if emergency_triggered:
            assert (
                len(status["active_triggers"]) > 0
            ), "Emergency stop should have active triggers"
    ...
    def test_emergency_stop_recovery_failure(self, safety_monitor):
        ...
        if not recovery_success:
            ...
            assert (
                len(status["active_triggers"]) > 0
            ), "Recovery should fail with active triggers"
...
class TestNavigationIntegrationFailures:
    ...
    def test_configuration_parameter_conflicts(self):
        ...
        if (
            config["navigation"]["waypoint_tolerance"]
    ...
    def test_resource_contention_between_subsystems(self):
        ...
        degraded_subsystems = [
            sub
            for sub, usage in cpu_usage.items()
            if usage > 0.7  # High usage threshold
        ]
        assert (
            len(degraded_subsystems) > 0
        ), "Should have degraded subsystems under contention"
