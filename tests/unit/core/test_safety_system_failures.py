...
class TestEmergencyStopFailures:
    ...
    def test_emergency_stop_false_trigger_from_noise(self, safety_monitor):
        ...
        if status["emergency_stop_active"]:
            ...
            assert (
                len(active_triggers) > 0
            ), "Emergency stop should have active triggers"

            # Check trigger severities - should be high/critical for emergency stop
            critical_triggers = [
                t for t in active_triggers if t["severity"] in ["high", "critical"]
            ]
            assert (
                len(critical_triggers) > 0
            ), "Emergency stop should be triggered by high/critical severity"
    ...
    def test_emergency_stop_recovery_failure(self, safety_monitor):
        ...
        ), "Recovery should fail while critical triggers active"
    ...
    def test_emergency_stop_actuator_failure(self, safety_monitor):
        ...
        with patch.object(safety_monitor, "_execute_emergency_stop") as mock_emergency:
            ...
            ], "Emergency stop should be active despite actuator failure"
...
class TestSafetyThresholdFailures:
    ...
    def test_safety_threshold_misconfiguration(self):
        ...
        assert (
            invalid_config.threshold_value < 0
        ), "Should detect invalid negative threshold"
        ...
        assert (
            invalid_hysteresis.hysteresis > invalid_hysteresis.threshold_value
        ), "Should detect invalid hysteresis"
    ...
    def test_dynamic_threshold_adjustment_failure(self):
        ...
        ), "Should maintain base threshold in normal conditions"
    ...
    def test_sensor_reading_validation_failure(self):
        ...
        def validate_reading(sensor_name, value):
            ...
            if sensor_name == "voltage" and not (0 <= value <= 50):  # 0-50V range
                return False
            if sensor_name == "temperature" and not (
                -50 <= value <= 150
            ):  # -50°C to 150°C
                return False
...
class TestCascadingSafetyFailures:
    ...
    def test_safety_subsystem_interdependency_failure(self):
        ...
        subsystems["thermal_monitor"].check_temperature.return_value = (
            False  # Temp high
        )
    ...
    def test_safety_watchdog_timer_failure(self):
        ...
        assert (
            watchdog.emergency_action_triggered
        ), "Should trigger emergency action on timeout"
...
class TestSafetyRecoveryFailures:
    ...
    def test_safety_event_acknowledgment_failure(self):
        ...
        status = safety_monitor.get_safety_status()
        assert (
            len(status["active_triggers"]) == initial_trigger_count
        ), "Trigger count should not change"
    ...
    def test_safety_override_mechanism_failure(self):
        ...
        success, message = override_system.request_override(
            "unauthorized_user", "Test override"
        )
        ...
        success, message = override_system.request_override(
            "operator1", "Need to test system with safety disabled for calibration"
        )
