...
pytest.skip(
    "ObservabilitySystem tests require isolated Prometheus CollectorRegistry",
...
class TestObservabilitySystem:
    ...
    def test_metrics_collection(self, observability_system):
        ...
        observability_system.increment_counter(
            "test_counter", {"service": "navigation"}
        )
        ...
        observability_system.record_histogram(
            "test_histogram", 1.5, {"operation": "waypoint"}
        )
    ...
    def test_tracing_functionality(self, observability_system):
        ...
        # Create a span
        with observability_system.start_span(
            "test_operation", {"user": "operator"}
        ) as span:
    ...
    def test_structured_logging(self, observability_system):
        ...
        observability_system.logger.info(
            "Test info message", user="operator", action="start_mission"
        )
        observability_system.logger.warning(
            "Test warning message", component="navigation", error="gps_timeout"
        )
        observability_system.logger.error(
            "Test error message", service="communication", code="CONNECTION_LOST"
        )

        # Test log with correlation ID
        observability_system.logger.info(
            "Test with correlation", correlation_id="test-123", data={"key": "value"}
        )
    ...
    def test_health_check_registration(self, observability_system):
        ...
        def communication_health():
            return {
                "status": "degraded",
                "details": {"websocket": "connected", "ros2": "timeout"},
            }

        observability_system.register_health_check("navigation", navigation_health)
        observability_system.register_health_check(
    ...
    def test_circuit_breaker_integration(self, observability_system):
        ...
        observability_system.register_circuit_breaker(
            "navigation_service", failure_threshold=3, recovery_timeout=5
        )
    ...
    def test_error_tracking(self, observability_system):
        ...
        try:
            ...
            observability_system.track_error(
                e, {"component": "test", "operation": "validation"}
            )
    ...
    def test_custom_metrics(self, observability_system):
        ...
        counter = observability_system.create_counter(
            "custom_counter", "Custom test counter", ["label"]
        )
        counter.labels(label="test").inc()

        # Create custom gauge
        gauge = observability_system.create_gauge(
            "custom_gauge", "Custom test gauge", ["component"]
        )
        gauge.labels(component="test").set(42.0)

        # Create custom histogram
        histogram = observability_system.create_histogram(
            "custom_histogram", "Custom test histogram", ["operation"]
        )
    ...
    def test_metrics_aggregation(self, observability_system):
        ...
        for i in range(10):
            observability_system.record_histogram(
                "response_time", i * 0.1, {"endpoint": "api"}
            )
            observability_system.increment_counter(
...
class TestObservabilitySystemIntegration:
    ...
    def test_end_to_end_observability_flow(self, integrated_system):
        ...
        with obs.start_span(
            "mission_execution", {"mission": "sample_collection"}
        ) as span:
            ...
            obs.logger.info(
                "Mission completed", mission="sample_collection", result="success"
            )
