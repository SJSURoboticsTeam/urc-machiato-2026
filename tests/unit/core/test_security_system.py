...
class TestSecurityManager:
    ...
    def test_user_authentication(self, security_manager, sample_credentials):
        ...
        auth_result = security_manager.authenticate(
            sample_credentials["username"], sample_credentials["password"]
        )
    ...
    def test_role_based_access_control(self, security_manager):
        ...
        assert (
            security_manager.check_permission(
                "mission_operator", "mission_start", permissions
            )
            is True
        )
        assert (
            security_manager.check_permission("viewer", "mission_start", permissions)
            is False
        )
        assert (
            security_manager.check_permission("admin", "system_shutdown", permissions)
            is True
        )
        assert (
            security_manager.check_permission(
                "mission_operator", "system_shutdown", permissions
            )
            is False
        )
    ...
    def test_input_validation(self, security_manager):
        ...
        invalid_commands = [
            ...
            },  # XSS attempt
            {
                ...
                "mission_type": "drop table users;",
            },  # SQL injection
            {
                "action": "navigate",
                "waypoint": {"lat": "invalid", "lon": -108.987654},
            },  # Type mismatch
        ]
    ...
    def test_api_key_authentication(self, security_manager):
        ...
        # Create API key
        api_key, secret = security_manager.create_api_key(
            "test_service", ["read", "write"]
        )
        ...
        invalid_auth = security_manager.authenticate_api_key(
    ...
    def test_request_signing(self, security_manager):
        ...
        is_invalid = security_manager.verify_request_signature(
    ...
    def test_rate_limiting(self, security_manager):
        ...
        for i in range(10):
            allowed = security_manager.check_rate_limit(
                client_id, "api_requests", limit=100, window=60
            )
    ...
    def test_multi_factor_authentication(self, security_manager, sample_credentials):
        ...
        # Verify MFA code
        verified = security_manager.verify_mfa_code(
            sample_credentials["username"], mfa_code
        )
        assert verified is True

        # Test invalid MFA code
        invalid_verified = security_manager.verify_mfa_code(
            sample_credentials["username"], "000000"
        )
    ...
    def test_security_policy_enforcement(self, security_manager):
        ...
        assert (
            security_manager.check_policy_compliance(user_compliant, policies) is True
        )
        assert (
            security_manager.check_policy_compliance(user_non_compliant, policies)
            is False
        )
...
class TestAccessControl:
    ...
    def test_role_based_permissions(self, access_control):
        ...
        access_control.define_role_permissions(
            "operator", ["mission:*", "telemetry:read"]
        )
...
class TestAuditLogger:
    ...
    def test_audit_log_retention(self, audit_logger):
        ...
        audit_logger.log_event(
            "old_event", "test", timestamp=time.time() - 86400
        )  # 1 day ago
