...
# Import waypoint navigation mission
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../../missions'))
from waypoint_navigation_mission import WaypointNavigationMission, WaypointState
...
class TestWaypointNavigationMission:
    ...
    def waypoint_mission(self, mock_node):
        ...
        with patch('waypoint_navigation_mission.Node', return_value=mock_node):
    ...
    def test_calculate_distance_to_waypoint(self, waypoint_mission):
        ...
        distance = waypoint_mission._calculate_distance_to_waypoint(current_pos, waypoint)
    ...
    def test_calculate_heading_error(self, waypoint_mission):
        ...
        error = waypoint_mission._calculate_heading_error(current_heading, target_heading)
...
class TestWaypointNavigationPerformance:
    ...
    def test_distance_calculation_performance(self, waypoint_mission, performance_monitor):
    ...
    @pytest.mark.performance
    def test_waypoint_validation_performance(self, waypoint_mission, performance_monitor):
        """Test performance of waypoint validation."""
        performance_monitor.start()

        # Create many waypoints
        waypoints = [
            {"position": [i * 0.1, i * 0.1], "heading": 0.0}
            for i in range(100)
        ]
