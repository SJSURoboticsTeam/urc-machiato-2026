...
class ProximityMonitor(Node):
    ...
    def _setup_subscribers(self):
        ...
        self.create_subscription(
            LaserScan, '/scan', self._lidar_callback,
            self.qos_sensor, callback_group=self.create_callback_group()
        )

        # Ultrasonic sensors (multiple possible)
        for i in range(4):  # Assume up to 4 ultrasonic sensors
            self.create_subscription(
                Range, f'/ultrasonic_{i}/range', self._ultrasonic_callback,
                self.qos_sensor, callback_group=self.create_callback_group()
            )

        # Depth camera
        self.create_subscription(
            Image, '/camera/depth/image_raw', self._depth_callback,
            self.qos_sensor, callback_group=self.create_callback_group()
        )

        self.create_subscription(
            CameraInfo, '/camera/camera_info', self._camera_info_callback,
            self.qos_reliable, callback_group=self.create_callback_group()
        )
    ...
    def _check_proximity(self):
        ...
        if not self._has_fresh_safety_data():
            self.logger.warn('No fresh safety sensor data available - cannot ensure safety',
        ...
        if violations:
            ...
            most_severe_zone, reading = max(violations,
                                          key=lambda x: self._severity_priority(x[0].severity))
