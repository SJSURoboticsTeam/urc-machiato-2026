...
def generate_launch_description():
    ...
    namespace_arg = DeclareLaunchArgument(
        "namespace", default_value="autonomy", description="Top-level namespace for all nodes"
    )
    ...
    enable_vision_arg = DeclareLaunchArgument(
        "enable_vision", default_value="true", description="Enable computer vision subsystem"
    )

    enable_slam_arg = DeclareLaunchArgument("enable_slam", default_value="true", description="Enable SLAM subsystem")
    ...
    startup_handler = RegisterEventHandler(
        OnProcessStart(
            ...
            on_start=[
                ...
                TimerAction(period=2.0, actions=[LogInfo(msg="All subsystems initialized")]),
            ],
        )
    )
...
def create_subsystem_launch(subsystem_name: str):
    ...
    return IncludeLaunchDescription(
        PythonLaunchDescriptionSource([FindPackageShare(pkg_name), f"/launch/{subsystem_name}.launch.py"]),
        launch_arguments={
            "namespace": LaunchConfiguration("namespace"),
            "use_sim_time": LaunchConfiguration("use_sim_time"),
        }.items(),
