...
def generate_launch_description():
    ...
    declare_world_cmd = DeclareLaunchArgument(
        "world", default_value="urc_desert_terrain", description="Full path to world model file to load"
    )
    ...
    declare_z_position_cmd = DeclareLaunchArgument(
        "z_pose", default_value="0.0", description="Initial z position of the robot"
    )

    declare_yaw_position_cmd = DeclareLaunchArgument(
        "yaw_pose", default_value="0.0", description="Initial yaw of the robot"
    )
    ...
    declare_enable_gps_cmd = DeclareLaunchArgument(
        "enable_gps", default_value="true", description="Enable GPS sensor if true"
    )

    declare_sensor_noise_level_cmd = DeclareLaunchArgument(
        "sensor_noise_level", default_value="medium", description="Sensor noise level: low, medium, high"
    )
    ...
    start_spawner_cmd = Node(
        ...
        arguments=[
            ...
            "rover2025",
            "-file",
            os.path.join(model_dir, "rover2025_gazebo.urdf"),
            "-x",
            x_pose,
            "-y",
            y_pose,
    ...
    start_sensor_config_cmd = Node(
        ...
        output="screen",
        parameters=[{"sensor_noise_level": sensor_noise_level, "enable_gps": enable_gps, "use_sim_time": use_sim_time}],
    )
