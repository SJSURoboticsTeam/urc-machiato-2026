...
- ``test_launch_file_integration.py`` for launch_testing tests

ROS2 Jazzy Testing Best Practices
==================================

For ROS2 Jazzy projects, we use a **hybrid testing approach** combining pytest and launch_testing:

**Framework Selection:**
- **pytest**: Use for unit tests, logic validation, fast feedback
- **launch_testing**: Use for launch file tests, system integration, lifecycle tests

**Key Benefits:**
- Automatic process management with launch_testing
- Integration with ``colcon test`` workflow
- Better launch file validation
- ROS2 lifecycle testing support

**Quick Reference:**
- See :doc:`../ROS2_JAZZY_TESTING_BEST_PRACTICES` for complete guide
- Example: ``tests/integration/test_launch_file_integration.py``
- CMakeLists.txt example: ``docs/examples/CMakeLists_testing_example.cmake``

**Installation:**
.. code-block:: bash

   sudo apt install ros-jazzy-launch-testing ros-jazzy-launch-testing-ros

**Running Tests:**
.. code-block:: bash

   # Pytest tests (current approach)
   pytest tests/unit/ -v
   
   # Launch testing tests
   pytest -m launch_testing -v
   
   # ROS2 colcon tests (recommended)
   colcon test
