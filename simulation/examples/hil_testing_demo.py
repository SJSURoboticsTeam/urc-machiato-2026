...
def hil_modes_demo():
    ...
    firmware_component = manager.initialize_component(
        "firmware", mode=ComponentMode.AUTO
    )
    ...
    slcan_component = manager.initialize_component(
        "slcan", mode=ComponentMode.SIMULATED
    )
...
def mixed_testing_demo():
    ...
    for name, component in components.items():
        if component:
            mode = (
                "REAL"
                if hasattr(component, "serial_port") or hasattr(component, "device")
                else "SIMULATED"
            )
            print(f"  {name}: {mode}")
...
def comparison_demo():
    ...
    if real_firmware and sim_firmware:
        ...
        if hasattr(real_firmware, "get_motor_status"):
            ...
            print(
                f"  Diff: {abs(real_status['velocity_actual'] - sim_status['velocity_actual']):.3f} rad/s"
            )
        ...
        comparison = manager.record_comparison(
            ...
            {
                "real": (
                    real_status["velocity_actual"] if "real_status" in locals() else 0
                ),
