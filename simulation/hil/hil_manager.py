...
class HILManager:
    ...
    def initialize_component(
        self, component_type: ComponentType, mode: ComponentMode = ComponentMode.AUTO
    ) -> bool:
        ...
        try:
            ...
            if mode in [ComponentMode.REAL, ComponentMode.AUTO]:
                ...
                if real_component:
                    ...
                    self.logger.info(
                        f"{component_type.value} initialized with REAL hardware"
                    )
                    return True
            ...
            if mode in [ComponentMode.SIMULATED, ComponentMode.AUTO]:
                if self.fallback_to_simulation or mode == ComponentMode.SIMULATED:
                    ...
                    self.logger.info(
                        f"{component_type.value} initialized with SIMULATION"
                    )
                    return True
    ...
    def record_comparison(self, real_data: Dict[str, Any], sim_data: Dict[str, Any]):
    ...
    def _calculate_difference(
        ...
        for key in real_data:
            if key in sim_data:
                if isinstance(real_data[key], (int, float)) and isinstance(
                    sim_data[key], (int, float)
                ):
