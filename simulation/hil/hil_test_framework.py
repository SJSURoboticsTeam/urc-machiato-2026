...
class HILTestFramework:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.hardware_registry = get_hardware_registry(
            self.config.get("hardware_registry", {})
        )
    ...
    def _load_test_scenarios(self):
        ...
        websocket_test = TestScenario(
            ...
            components_required={ComponentType.WEBSOCKET_SERVER: ComponentMode.AUTO},
            test_steps=[
    ...
    def run_test_scenario(self, scenario_name: str) -> TestExecution:
        ...
        try:
            ...
            self.logger.info(
                f"Test scenario '{scenario_name}' completed: {execution.result.value}"
            )

        return execution

    def _initialize_test_components(
    ...
    def _execute_test_steps(self, scenario: TestScenario, execution: TestExecution):
        ...
        for i, step in enumerate(scenario.test_steps):
            ...
            try:
                ...
                self.logger.info(
                    f"Step {i+1}/{len(scenario.test_steps)} completed in {step_duration:.2f}s"
                )
    ...
    def _start_websocket_server_step(
        self, step: Dict[str, Any], execution: TestExecution
    ):
        ...
        for expected_key, expected_value in scenario.expected_results.items():
            ...
            if actual_value is not None:
                ...
                if isinstance(expected_value, bool):
                    ...
                    validation_results[expected_key] = str(actual_value) == str(
                        expected_value
                    )
            else:
    ...
    def get_test_report(self) -> Dict[str, Any]:
        ...
        passed_tests = len(
            [e for e in self.test_executions if e.result == TestResult.PASS]
        )
        failed_tests = len(
            [e for e in self.test_executions if e.result == TestResult.FAIL]
        )
        error_tests = len(
            [e for e in self.test_executions if e.result == TestResult.ERROR]
        )
