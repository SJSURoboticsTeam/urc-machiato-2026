...
class STM32FirmwareSimulator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.logger.info(f"STM32 firmware simulator initialized with {self.num_motors} motors")
    ...
    def set_velocity_command(self, motor_id: int, velocity: float) -> bool:
        ...
        if abs(velocity) > self.MAX_VELOCITY:
            self.logger.warning(f"Velocity {velocity} exceeds limit {self.MAX_VELOCITY}")
    ...
    def set_chassis_velocities(self, linear_x: float, linear_y: float, angular_z: float) -> bool:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'motors_faulted': sum(1 for m in self.motors.values() if m.fault != MotorFaultType.NONE),
    ...
    def _update_motor(self, motor: MotorState, dt: float):
        ...
        if motor.current > self.MAX_CURRENT:
            ...
            self.logger.error(f"Motor {motor.motor_id} overcurrent: {motor.current:.2f}A")
        ...
        if motor.temperature > self.MAX_TEMPERATURE:
            ...
            self.logger.error(f"Motor {motor.motor_id} overtemperature: {motor.temperature:.1f}°C")
    ...
    def _update_encoder(self, motor_id: int, motor: MotorState, dt: float):
        ...
        encoder.position = motor.position + random.gauss(0, encoder.noise_std / encoder.resolution * 2 * math.pi)
