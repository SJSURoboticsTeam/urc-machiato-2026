...
class ControlSystemsHIL:
    ...
    def start(self):
        ...
        self.control_thread = threading.Thread(
            target=self._hil_control_loop, daemon=True
        )
    ...
    def _generate_can_feedback(self):
        ...
        linear_x = (
            motor_0_status["velocity_actual"] * 0.1
        )  # Wheel to chassis conversion
        linear_y = 0.0
        angular_z = 0.0

        # Encode feedback frame
        feedback_frame = self.slcan_sim.encode_velocity_command(
    ...
    def _update_performance_metrics(self, loop_duration_us: float):
        ...
        if self.loop_times:
            ...
            if len(self.loop_times) > 1:
                ...
                variance = sum((t - mean) ** 2 for t in self.loop_times) / len(
                    self.loop_times
                )
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "queue_sizes": {"tx": len(self.can_tx_queue), "rx": len(self.can_rx_queue)},
        }
