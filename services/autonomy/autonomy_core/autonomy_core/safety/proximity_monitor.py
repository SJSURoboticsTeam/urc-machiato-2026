...
try:
    ...
    # Fallback - aoi_tracker may not be available
    AOIConfig = None
    AOITracker = None
...
# Unified blackboard client: Python modules live under repo ``shared/core/`` (``core.*`` imports).
def _ensure_shared_core_on_path() -> bool:
    """Insert repo ``shared/`` onto sys.path so ``core.unified_blackboard_client`` resolves."""
    marker = "unified_blackboard_client.py"
    for p in sys.path:
        if p and os.path.isfile(os.path.join(p, "core", marker)):
            return True
    here = Path(__file__).resolve()
    for parent in here.parents:
        shared = parent / "shared" / "core" / marker
        if shared.is_file():
            root_shared = str(parent / "shared")
            if root_shared not in sys.path:
                sys.path.insert(0, root_shared)
            return True
    env_root = os.environ.get("URC_REPO_ROOT", "").strip()
    if env_root:
        root_shared = os.path.join(env_root, "shared")
        if os.path.isfile(os.path.join(root_shared, "core", marker)):
            if root_shared not in sys.path:
                sys.path.insert(0, root_shared)
            return True
    return False
...
try:
    ...
    BlackboardKeys = None
    UnifiedBlackboardClient = None  # type: ignore[misc,assignment]
...
class ProximityMonitor(Node):
    ...
    def __init__(self):
        ...
        # Initialize unified blackboard client for obstacle data
