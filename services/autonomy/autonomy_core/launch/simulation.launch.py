...
def generate_launch_description():
    ...
    env_setup = GroupAction(
        [
            ...
            SetEnvironmentVariable(
                "GAZEBO_MODEL_PATH", os.path.join(os.getcwd(), "src/simulation/models")
            ),
        ]
    )

    # Log startup
    startup_log = LogInfo(msg=["Starting URC 2026 Simulation with world: ", world])
        ...
        PythonLaunchDescriptionSource(
            [
                PathJoinSubstitution(
                    [
                        (
                            get_package_share_directory("autonomy_core")
                            if os.path.exists(
                                get_package_share_directory("autonomy_core")
                                if os.path.exists(os.path.join(os.getcwd(), "install"))
                                else os.path.join(
                                    os.getcwd(), "src/autonomy/autonomy_core"
                                )
                            )
