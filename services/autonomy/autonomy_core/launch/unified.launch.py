...
Hardware LiDAR (Unitree L2): ``proximity_monitor`` expects ``sensor_msgs/LaserScan`` on
``/scan``. The L2 driver publishes ``PointCloud2``; start the bridge before or with the stack::

    ros2 launch autonomy_core unitree_l2_pc2_to_scan.launch.py

Blackboard services ``/blackboard/get_value`` and ``/blackboard/set_value`` are provided by
the C++ ``bt_orchestrator`` (package ``autonomy_bt``), not by this file alone. Start with
``ros2 run autonomy_bt bt_orchestrator`` and lifecycle configure/activate, or use
``scripts/launch/integrated_system.launch.py`` where applicable.
