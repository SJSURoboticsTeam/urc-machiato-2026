...
export const useStateMachine = (ros) => {
  ...
  useEffect(() => {
    ...
    console.log('Setting up real state machine topic subscriptions...');

    // Current state subscriber - handle JSON strings from ROS2 state machine bridge
  ...
  const requestStateTransition = async (targetState, reason = 'frontend_request', force = false) => {
    // Try ROS2 topic publishing first, fall back to demo mode
    ...
    try {
      // Create publisher for transition requests
      const transitionPublisher = createPublisher(ros, '/state_machine/transition_request', 'std_msgs/String');

      // Publish transition request to ROS2 topic
      const transitionRequest = {
        transition: targetState.toLowerCase().replace('_', '_to_') + '_transition', // Convert to transition name
        reason: reason,
        timestamp: Date.now(),
        source: 'frontend'
      };
      ...
      transitionPublisher.publish({
        data: JSON.stringify(transitionRequest)
      });

      console.log('Published state transition request:', transitionRequest);

      // Return success response (actual result will come via subscription)
      return {
        success: true,
        message: `Transition request sent: ${targetState}`,
        newState: targetState, // Will be updated when subscription receives confirmation
        timestamp: new Date().toISOString()
      };
