...
export const MissionTab = () => {
  ...
  const commandPublisherRef = useRef(null);

  // Real mission data from ROS2
  const [realMissionStatus, setRealMissionStatus] = useState(null);
  const [realMissionProgress, setRealMissionProgress] = useState(null);
  const [realMissionTelemetry, setRealMissionTelemetry] = useState(null);

  // Setup ROS2 subscriptions when connected
  useEffect(() => {
    if (!ros) return;

    console.log('Setting up mission ROS2 subscriptions...');
    ...
    subscribersRef.current.status = createSubscriber(
      ...
      (message) => {
        try {
          ...
          setRealMissionStatus(data);
        } catch (error) {
          console.warn('Failed to parse mission status:', error);
        }
      }
    );

    // Mission progress subscriber
    subscribersRef.current.progress = createSubscriber(
      ...
      (message) => {
        try {
          const data = JSON.parse(message.data);
          setRealMissionProgress(data);
          // Update active mission from real data
          if (data.name && data.progress !== undefined) {
            setActiveMission({
              id: data.mission_id,
              name: data.name,
          ...
          console.warn('Failed to parse mission progress:', error);
        }
      }
    );

    // Mission telemetry subscriber
    subscribersRef.current.telemetry = createSubscriber(
      ...
      (message) => {
        try {
          const data = JSON.parse(message.data);
          setRealMissionTelemetry(data);
        } catch (error) {
          console.warn('Failed to parse mission telemetry:', error);
        }
      }
    );

    // Command publisher
    commandPublisherRef.current = createPublisher(
      ros,
      '/mission/commands',
      'std_msgs/String'
    );
  ...
  const handleStartMission = () => {
    ...
    // Also transition system state
    requestStateTransition(SystemState.AUTONOMOUS, 'Mission started from UI');

    console.log('Published mission start command:', missionConfig);
  };
