...
import { DataSourceFactory } from '../services/DataSourceFactory';
...
const useMock = typeof import.meta !== 'undefined' && import.meta.env && String(import.meta.env.VITE_USE_MOCK) === '1';
  ...
  useEffect(() => {
    if (useMock) {
      ...
      source
        ...
        .catch((err) => {
          console.warn('[TelemetryContext] Mock data source connect failed:', err);
          setSystemStatus((prev) => ({ ...prev, can: 'error' }));
        });
      const unsubscribe = source.subscribe<Record<string, unknown>>('telemetry', (data) => {
        const result = validateTelemetryPayload(data);
        if (result.valid && result.sanitized) {
          setTelemetryState(result.sanitized);
        }
      });
      return () => {
        unsubscribe();
