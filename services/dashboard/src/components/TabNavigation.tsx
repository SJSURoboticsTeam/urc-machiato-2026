...
/**
 * Tab navigation with indicators for active missions, errors, and tests.
 */
export const TabNavigation: React.FC<TabNavigationProps> = ({ activeTab, setActiveTab }) => {
  const { activeMission, errorCount, runningTests } = useUIContext();
  ...
  const tabs: TabConfig[] = [
    { id: 'overview', label: 'Overview', icon: LayoutDashboard },
    {
      ...
      label: 'Mission',
      icon: Target,
      badge: activeMission != null ? `${Math.round(activeMission.progress ?? 0)}%` : null
    },
    { id: 'network', label: 'Cognition', icon: GitBranch },
    ...
    { id: 'blackboard', label: 'Blackboard', icon: BookOpen },
      ...
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            ...
            <span>{tab.label}</span>
            {tab.badge != null && (
              <span className={`px-1.5 py-0.5 text-xs rounded ${isActive ? 'bg-zinc-700 text-zinc-200' : 'bg-zinc-700/50 text-zinc-400'}`}>
                {tab.badge}
              </span>
            )}
          </button>
