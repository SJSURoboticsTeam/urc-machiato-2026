...
export function useNetworkData(): UseNetworkDataReturn {
  ...
  const { dataSource } = useTelemetryContext();
  ...
  // Prefer data source (mock mode) when available; otherwise use shared ROS connection
  useEffect(() => {
    if (dataSource && dataSource.getSourceType() === 'mock') {
      setConnections((prev) => ({ ...prev, ros2: 'operational', can: 'operational' }));
      const unsubscribe = dataSource.subscribe<Record<string, unknown>>('/can/sensor_data', (data) => {
        const status = (data?.status as string) ?? 'normal';
        setConnections((prev) => ({ ...prev, can: status === 'degraded' ? 'degraded' : 'operational' }));
        setCanData((prev) => ({
          ...prev,
          bus_status: {
            ...prev.bus_status,
            messages_per_sec: prev.bus_status.messages_per_sec + 1,
            error_count: prev.bus_status.error_count
          }
        }));
        setNetworkNodes((currentNodes) => ({
          ...
          can_bus: {
            ...
            data: {
              status: status === 'degraded' ? 'degraded' : 'operational',
