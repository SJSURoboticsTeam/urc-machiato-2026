...
# Ensure repo root on path so "src" package resolves (used when run from tests or scripts)
_here = Path(__file__).resolve().parent
_repo_root = _here.parent.parent  # simulation/ros2 -> simulation -> repo root
if str(_repo_root) not in sys.path:
    sys.path.insert(0, str(_repo_root))

from shared.core.ros2_mock import Twist, PoseStamped, NavSatFix
...
class ROS2MessageAdapter:
    ...
    @staticmethod
    def dict_to_twist_with_frame(
        data: Dict[str, Any], frame_id: str = "base_link"
    ) -> Twist:
    ...
    @staticmethod
    def dict_to_pose_stamped(
        data: Dict[str, Any], frame_id: str = "map"
    ) -> PoseStamped:
