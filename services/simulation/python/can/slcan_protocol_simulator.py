...
class SLCANFrame:
    ...
    def __post_init__(self):
        ...
        if self.data_length > 8:
            raise ValueError(
                f"CAN data length cannot exceed 8 bytes, got {self.data_length}"
            )
        if len(self.data) != self.data_length:
            raise ValueError(
                f"Data length mismatch: expected {self.data_length}, got {len(self.data)}"
            )
...
class SLCANProtocolSimulator:
    ...
    def decode_velocity_command(self, slcan_frame: str) -> Optional[VelocityCommand]:
        ...
        try:
            ...
            if parsed.message_id != CANMessageID.SET_CHASSIS_VELOCITIES.value:
                self.logger.debug(
                    f"Frame is not velocity command: 0x{parsed.message_id:03X}"
                )
                return None
            ...
            if parsed.data_length != 6:
                self.logger.error(
                    f"Invalid data length for velocity command: {parsed.data_length}"
                )
                return None
    ...
    def parse_frame(self, slcan_frame: str) -> Optional[SLCANFrame]:
        ...
        try:
            ...
            if msg_type in [
                SLCANMessageType.STANDARD_DATA,
                SLCANMessageType.STANDARD_RTR,
            ]:
            ...
            if len(data_hex) != data_len * 2:
                self.logger.error(
                    f"Data length mismatch: expected {data_len*2} chars, got {len(data_hex)}"
                )
                self.stats["malformed_frames"] += 1
                return None
    ...
    def encode_frame(self, message_id: int, data: bytes, extended: bool = False) -> str:
    ...
    def inject_error(self, frame: str) -> str:
        ...
        error_type = random.choice(
            ["corrupt_byte", "truncate", "wrong_checksum", "invalid_id"]
        )
...
def create_slcan_simulator(profile: str = "default") -> SLCANProtocolSimulator:
    ...
    profiles = {
        ...
        "noisy": {"simulate_errors": True, "error_rate": 0.05, "serial_delay_ms": 15.0},
    }
