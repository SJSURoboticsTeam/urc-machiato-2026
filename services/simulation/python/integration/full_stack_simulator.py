...
from services.simulation.python.network.websocket_server_simulator import WebSocketServerSimulator
from services.simulation.python.can.slcan_protocol_simulator import (
    SLCANProtocolSimulator,
    VelocityCommand,
)
from services.simulation.python.firmware.stm32_firmware_simulator import STM32FirmwareSimulator
...
class FullStackSimulator:
    ...
    async def run_scenario(
        self, scenario_type: ScenarioType, params: Optional[Dict[str, Any]] = None
    ) -> ScenarioResult:
        """Run a test scenario.
    ...
    async def _scenario_basic_velocity(self, params: Dict[str, Any]) -> bool:
        ...
        # Send drive command
        await self.websocket_sim.receive(
    ...
    async def _scenario_emergency_stop(self, params: Dict[str, Any]) -> bool:
        ...
        client_id = await self.websocket_sim.connect()
        await self.websocket_sim.receive(
            "driveCommands", {"linear": 1.0, "angular": 0.5}, client_id
        )
    ...
    async def _scenario_network_failure(self, params: Dict[str, Any]) -> bool:
        ...
        success_count = 0
        for i in range(10):
            result = await self.websocket_sim.receive(
                "driveCommands", {"linear": 0.5, "angular": 0.0}, client_id
            )
            if result:
                success_count += 1
    ...
    async def _scenario_firmware_fault(self, params: Dict[str, Any]) -> bool:
        ...
        await self.websocket_sim.receive(
            "driveCommands", {"linear": 1.0, "angular": 0.0}, client_id
        )
    ...
    async def _scenario_recovery(self, params: Dict[str, Any]) -> bool:
        ...
        # Send new command
        result = await self.websocket_sim.receive(
            "driveCommands", {"linear": 0.3, "angular": 0.0}, new_client_id
        )

        return result

    def _log_event(
        self, event_type: str, data: Dict[str, Any], path: CommunicationPath
    ):
