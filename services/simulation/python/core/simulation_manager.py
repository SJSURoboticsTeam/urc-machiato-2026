...
try:
    ...
    print(f"Bridge simulation components not available: {e}")
    BRIDGE_SIMULATION_AVAILABLE = False

# Import physics backends
try:
    from simulation.gazebo.gazebo_integration import GazeboIntegration
...
class SimulationManager:
    ...
    def __init__(self):
        ...
        # self.monitor = SimulationMonitor(self)  # Disabled for now
    ...
    def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        with self.tracer.trace_context(
            ...
            try:
                ...
                # Initialize monitoring (disabled for now)
                ...
                if not self._initialize_physics_backend(physics_config):
                    self.logger.error("Failed to initialize physics backend")
                    return False
                ...
                if bridges_config and BRIDGE_SIMULATION_AVAILABLE:
                    self._initialize_bridges(bridges_config)
                    self.logger.info("Bridge simulation initialized")
                ...
                self.logger.error(
                    f"Simulation initialization failed: {e}"
                )
                import traceback
                traceback.print_exc()
                return False
    ...
    def step(self, dt: float) -> Dict[str, Any]:
        ...
        with self.tracer.trace_context(
            ...
            try:
                ...
                # Update physics backend first
                with self.tracer.trace_context("physics_step", dt=dt):
                    physics_state = self.physics_backend.step(dt) if self.physics_backend else {}
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "physics_backend": {
                "type": self.physics_backend_type,
                "status": self.physics_backend.get_status() if self.physics_backend else None
            },
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        # Add monitoring statistics (disabled for now)
    ...
    def _initialize_physics_backend(self, physics_config: Dict[str, Any]) -> bool:
        """Initialize the selected physics backend.
        ...
        """
        try:
            backend_type = physics_config.get("backend", "gazebo")
            self.physics_backend_type = backend_type
            
            self.logger.info(f"Initializing physics backend: {backend_type}")
            
            if backend_type == "gazebo":
                if not GAZEBO_AVAILABLE:
                    self.logger.error("Gazebo backend not available")
                    return False
                    
                self.physics_backend = GazeboIntegration(physics_config)
                
            elif backend_type == "isaac":
                if not ISAAC_SIM_AVAILABLE:
                    self.logger.error("Isaac Sim backend not available")
                    return False
                    
                self.physics_backend = IsaacIntegration(physics_config)
                
            else:
                self.logger.error(f"Unknown physics backend: {backend_type}")
                return False
            
            # Initialize the backend
            if not self.physics_backend.initialize():
                self.logger.error(f"Failed to initialize {backend_type} backend")
                return False
            
            self.logger.info(f"Physics backend '{backend_type}' initialized successfully")
            return True
            
        except Exception as e:
            self.logger.error(f"Failed to initialize physics backend: {e}")
            return False
