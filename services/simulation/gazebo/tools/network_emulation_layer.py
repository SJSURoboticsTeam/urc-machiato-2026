...
class NetworkEmulationLayer(Node):
    ...
    def generate_latency(self) -> float:
        ...
        base_latency = random.uniform(
            self.current_profile.latency_ms[0], self.current_profile.latency_ms[1]
        )
    ...
    def process_network_conditions(self):
        ...
        if self.current_profile.connection_drops:
            if self.connection_state:
                ...
                if random.random() < 0.001:  # 0.1% chance per 100ms
                    ...
                    self.logger.warn(
                        f"Network connection dropped for {drop_duration:.1f}s"
                    )
            else:
                ...
                if current_time - self.last_connection_drop >= random.uniform(
                    ...
                    self.logger.info(
                        f"Network connection restored after {downtime:.1f}s"
                    )
        ...
        for delayed_msg in messages_to_deliver:
            latency = (
                current_time - delayed_msg["original_time"]
            ) * 1000  # Convert to ms
    ...
    def publish_metrics(self):
        ...
        if self.message_latencies:
            ...
            jitter = (
                statistics.stdev(self.message_latencies)
    ...
    def get_network_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "uptime_percentage": (
                100.0
                - (
                    self.statistics.total_downtime_sec
...
def run_network_emulation_demo():
    ...
    try:
        self.get_logger().info("Testing different network conditions...")
        ...
        self.get_logger().info(f"  Total Messages Sent: {stats['messages_sent']}")
        self.get_logger().info(f"  Messages Received: {stats['messages_received']}")
        self.get_logger().info(f"  Messages Dropped: {stats['messages_dropped']}")
        self.get_logger().info(f"  Packet Loss Rate: {stats['packet_loss_rate']:.1f}%")
        self.get_logger().info(
            f"  Average Latency: {stats['average_latency_ms']:.1f}ms"
        )
        self.get_logger().info(f"  Connection Drops: {stats['connection_drops']}")
        self.get_logger().info(f"  Total Downtime: {stats['total_downtime_sec']:.1f}s")
        self.get_logger().info("\n NETWORK EMULATION DEMONSTRATION COMPLETE")
        self.get_logger().info("Real-world network conditions successfully simulated!")
