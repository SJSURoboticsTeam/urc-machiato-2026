...
class SensorFailureRecoveryTester(Node):
    ...
    def __init__(self):
        ...
        self.failure_timer = self.create_timer(
            self.failure_interval, self.inject_failure
        )
    ...
    def inject_failure(self):
        ...
        self.current_failures = [
            f
            for f in self.current_failures
            if current_time <= f.start_time + f.duration
        ]

        # Publish current failure status
        active_failures = [
            f"{f.sensor_type.value}:{f.failure_mode.value}"
            for f in self.current_failures
        ]
        status_msg = String()
        ...
        for sensor, timeout in self.sensor_timeouts.items():
            ...
            if current_time - last_update > timeout:
                ...
                if not self.degraded_mode:
                    ...
                    self.get_logger().warn(
                        f"SENSOR TIMEOUT: {sensor} - Entering degraded mode"
                    )
    ...
    def complete_test(self):
        ...
        test_results = {
            ...
            "overall_results": {
                ...
                "graceful_degradation": all(
                    m.graceful_degradation for m in self.recovery_metrics
                ),
        ...
        self.get_logger().info("SENSOR FAILURE RECOVERY TEST RESULTS")

        self.get_logger().info(f"Failures Injected: {len(self.failure_history)}")
        self.get_logger().info(f"Successful Recoveries: {successful_recoveries}")
        self.get_logger().info(
            f"Navigation Continuity: {' YES' if self.navigation_active else ' NO'}"
        )
        self.get_logger().info(
            f"Graceful Degradation: {' YES' if test_results['overall_results']['graceful_degradation'] else ' NO'}"
        )
        self.get_logger().info("\nFAILURE DETAILS:")
        for i, failure in enumerate(self.failure_history):
            recovery = (
                ""
                if i < len(self.recovery_metrics)
                and self.recovery_metrics[i].graceful_degradation
                else ""
            )
        self.get_logger().info(
            f"  {recovery} {failure.sensor_type.value} {failure.failure_mode.value} ({failure.duration:.0f}s)"
        )
